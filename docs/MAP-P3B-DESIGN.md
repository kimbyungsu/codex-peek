...
- **배포 세대 검사의 컨텍스트 인식(2차 정정 — 자기 경로 확인만으로는 '섞인 세대'를 못 잡는다)**: 자동
  ...
  키 집합은 EXPECTED_DEPLOY_FILES(20파일 — install.js·hook-setup.ts와 3카피 패리티. 설계 시점 19 →
  P7에서 map-probe.js 추가로 20. 정본은 코드의 3카피 목록이며 패리티 테스트가 일치를 강제)와 **정확 일치**해야
  한다(부재·축소·잉여·지문 불일치=거부). 레포 실행은 현행 레포↔설치본 전수 대조 유지.
- **manifest 기록 조건(4차 정정 — '디스크에 있는 것'의 정본화 금지)**: manifest는 **설치본=배포 원본이
  검증된 상태에서만** 기록한다. install.js=자기가 방금 복사한 세대. 확장=①전체 재배치(20파일 전부 번들에서
...
- **배포 잠금(6차 정정 도입·9차 프로토콜 확정 — 검사기 자체의 reader-writer TOCTOU)**: 20파일 순차 대조
