...
**3차 묶음(2026-09-12 · 사용자 동의 "모두 동의해")**: ① 꽉 찬 지도(활성 60)에서 내려간 파일이 다시 바뀌면 되살리기가 정본 상한에 막혀 소화 기준점이 영구 정체하던 빈틈 → 되살리기도 새 칸 진입으로 취급해 후보 1개를 내보내고 들어온다(`evictForSlot`·후보 규칙은 `pickVictim` 공용). 후보 없음(전부 보호)·교체 꺼짐=**미룸**(`deferred`, 실패 아님 → 기준점 전진 · 다음 변경 때 재시도). 내보낸 뒤 되살리기 실패=즉시 되돌림. ② 배경 작업 완료 알림은 사람의 발화가 아니므로 턴 앵커를 새로 찍지 않는다(`isSystemNotificationHook` → contract-inject 가 직전 앵커 ts·스냅샷 지문을 잇고 라이브 라운드도 리셋하지 않음). 판별 권위는 구조 표식: 훅 입력 origin.kind → transcript 의 같은 원문 user 레코드의 origin.kind(verify-guard 와 동일 권위·꼬리 2MB 만 판독) → 기록에 없을 때만 '봉투만으로 된 본문' 규칙(봉투 밖에 사람의 글이 있으면 발화 — 구현 검증 1판 blocker 수용) — 캠페인 회차 상한(5)·선별 관문·화면 '준비' 고착의 공통 원인. ③ 회전 시험은 60칸 지도를 한 번 만들어 복제(`fullRepo`). ④ 오래 잠든 칸의 보관 이동은 삭제 아닌 별도 파일 이동으로만 검토(ab-5) — 미착수.
