...
- **개정(2026-09-11 · HARNESS-STRUCTURE-2026-09-11 §B1 · 사용자 결정 2a89097203a7ce9b)**: evidence-unseen 의 표시 등급=정보(info) — 배너·상태바 경보 아님. 재확인 신호·동결·발송·해소 규칙은 불변. 검증자가 적은 파일·줄·상세를 실제 파일과 대조하는 새 경보 `citation-mismatch`(저장소 결속·토큰 원문 미기록)는 그 문서 §B1 이 정본.
...
- **구간 적격성과 선택 순서**(확인 검증 보완 반영 — 이 순서가 규범):
  ...
  2. 적격 후보 중에서 `crypto.randomBytes` 기반으로 선택한다(가변 길이 32회). 빗나가면 결정론
     폴백: 길이 64·보폭 32 격자를 전부 스윕한다 — 격자 위 적격 후보가 있으면 반드시 발견하며,
     오프셋 전수(보폭 1)는 아니다(대형 파일 비용). '격자 밖에만 적격 후보가 있는' 병리적 경우는
     no-safe-span=경보 유지의 안전한 실패 방향으로 떨어진다(위 수용 위험과 같은 발견법 한계 부류).
