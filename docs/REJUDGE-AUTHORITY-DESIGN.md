...
작성 2026-08-29 · v1(blocker 4·보완 3)+v2(blocker 3·보완 1)+v3(blocker 1·보완 3)+v4(blocker 2)+v5(blocker 2·보완 2) 설계 검증 전량 반영 개정 · 상태: **설계 검증 5/5 소진 — v6은 5회차 지적 반영분이며 미도장(다음 설계 검증 캠페인 첫 회차가 v6을 검증) · 사용자 승인 대상**
...
## §8 사용자 결정 필요 항목 (설계 검증 뒤 한 번에)
1. 구현자 `rule-propose`(검증 교훈→수칙 상신의 현행 유일 경로) 폐지 vs 보조 채널 잔존('구현 담당 제안' 표기 의무).
2. 반증 없는 `boundary-contest` 재소환의 처리: 자동 강등(`contest-unproven` — 본 설계 제안·ab-3 봉합 전제 충족) vs 경고만.
3. D의 보관 파일 3칸 확장 시기(3차) — 그 전까지 전제·제외는 코어(항상)에만 존재.
4. 큐레이션이 검증 job 앞에 붙어 최악 ~34분이 될 수 있음 — (a) 그대로 허용(유계·생략 조건으로 드묾) vs (b) 큐레이션 예산을
   대시보드 대기시간 안으로 강제(초과분은 다음 job로 이월).
