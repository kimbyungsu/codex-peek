...
> 섞여 들어와 이력에서 제거했다(내용·경로는 의도적으로 미기재). **기존 클론은 `git fetch origin` 후
> `git reset --hard origin/main`으로 맞출 것**(로컬 커밋이 있으면 rebase — 재작성 구간은 2026-07-13~14의 7커밋,
