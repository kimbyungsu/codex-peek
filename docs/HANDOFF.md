...
4-5. **(방향 확정·v1 구현 2026-07-09) Scout Health — 전역 임계값을 프로젝트별 관찰 신호로 대체.** 사용자 결정:
   ...
   잔여(다음 묶음): 게이트 차단 문구에 신호 인용 · Reconciler advisory R0~R2(후보 생성기+LLM 제안 큐+확인 후
   적재 — 장부가 점화됐으므로 '축적 뒤' 유보는 해제, 단 자동 적용은 계속 금지) · 건강도 통합 뷰(Scout Health 층).
5. **(관찰) 관측 장부 실데이터 축적** — confirmed/user_dispute가 쌓이면: DERIVE_V1 임계(현재 최약 1회) 데이터 기반
   조정 + 장부 학습 반영 후 지도 명중률 재실측(ab-retro) → 60% 넘으면 게이트 기본 승격 재논의(사전등록 §4). **→ 2026-07-09 재실측 완료(§4 말미): 기억 주입 70.5%로 60% 최초 돌파 — 재논의 발동·사용자 결정 대기.**
