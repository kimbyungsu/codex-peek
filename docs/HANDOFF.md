...
5. **(관찰) 관측 장부 실데이터 축적** — confirmed/user_dispute가 쌓이면: DERIVE_V1 임계(현재 최약 1회) 데이터 기반
   ...
   **P3 복권·수명 범위(2026-07-09 사용자 질문으로 확정된 4약점 — 임계 튜닝과 같은 턴에 다룰 것)**:
   (a) disputed 항목은 이후 검증이 재확인해도 confirmed 기록 자체가 차단(codex-bridge.js flagLedgerConfirms
   dead-set — 복권 '규칙'이 없는 게 아니라 복권 '재료'가 안 쌓임. 복권 도입 시 이 차단부터 '기록은 하되 승격만
   막기'로 완화 필요) (b) 이벤트 2400건 초과 트림(상한 2000 유지)에서 틀림 이벤트만 잘리고 재제안이 남으면
   '틀림' 딱지가 조용히 소실(부활) — 트림 시 상태 결정 이벤트(dispute/ban 등) 보존 규칙 후보 (c) tombstone은
   스키마·UI 라벨만 있고 쓰는 곳 없음(파일 소멸 자동 감지 미배선) (d) 꾸러미 틀림 각주("같은 결론을 다시 내지
   마라" — scope-package.ts)는 문구가 달라도 결론 단위로 재발견을 억제하는데 만료·재검토 조건이 없음 — 코드가
   바뀌어 사실이 된 결합을 계속 누를 수 있음(sig는 원문 정규화 해시라 신규 항목 생성 자체는 안 막힘).
