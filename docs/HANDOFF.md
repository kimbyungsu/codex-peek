...
- **다음 단계 = D5 A/B 소급 실측**: 완료된 실제 변경들에 두 팔 지도를 소급 생성 → 명중·치명 누락(실사고 라벨 별도)·소음 비교.
  판정 사전 등록: **차이가 오차 수준이면 self 채택**(무료), DeepSeek 유의미 우위면 DeepSeek.
  ⚠ 두 러너는 '지금 작업트리' 기준이라 소급 실측엔 과거 커밋 재현 절차(체크아웃/워크트리) 설계가 선행.
- **2026-07-07 실측 결과(러너 `scripts/scope-ab-retro.js` — worktree로 부모 시점 복원+씨앗 1파일 diff만 적용, 커밋 6건·정답쌍 29)**:
  ...
  ⚠ 이 PC에 DeepSeek 키 없음 → **두 팔 비교는 미완**(키 있는 환경에서 `--arm deepseek`로 같은 러너 재실행). 표본 6커밋은 작음 —
  판 뒤집기용이 아니라 방향 확인용. 씨앗 1파일 재현은 실사용(작업트리 전체가 씨앗)보다 세 레벨 모두에게 불리한 보수적 조건.
  L3 텍스트 채점은 '경로 포함 또는 8자 이상 basename 포함' 규칙이라 hit/소음이 일부 부풀 수 있음.
...
1. **D5 A/B 소급 실측** — self 팔 완료(§4 결과 기록 참조). 남은 반쪽: DeepSeek 키 있는 환경에서 `node scripts/scope-ab-retro.js . --n 6 --arm deepseek`
   재실행 → 두 팔 비교(사전 등록: 오차 수준이면 self 채택). 러너 불변식은 `tests/ab-retro.test.js`가 고정.
2. 실측 결과에 따라 팔 확정 → 지도 high 항목 구조화 + Codex 검증 요청에 지도 동봉(Phase 3).
3. stable MAP(프로젝트 확정 지식층 — 파일채널·의미 결합 기록) 구축. 탐색자 지도의 ⑥MAP patch 후보가 입력.
