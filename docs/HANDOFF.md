...
2. ~~Phase 3(지도 high 구조화 + 검증 동봉)~~ — **구현 완료 2026-07-07**(§3 '검증 동봉' 참조). ⚠ 배포 사본(~/.codex-bridge)엔
   미반영 — 다음 release/재배치 때 실사용 반영(버전 불변 원칙). 효과 실측(동봉 후 검증이 지도 경로를 실제 지적하는 빈도)은 반영 후.
3. **(현재 1순위)** stable MAP(프로젝트 확정 지식층 — 파일채널·의미 결합 기록) 구축. 탐색자 지도의 ⑥MAP patch 후보가 입력.
   §6.5 'stable MAP 2층' 설계(확정층/제안층 분리 + reconcile 승인) 준수.
