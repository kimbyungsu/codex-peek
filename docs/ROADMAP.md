...
- **구현 결정 (2026-06-19 · 사용자 우선 · 메모리 회수)**: 후보 세션 목록 각 줄에 **🗑 버튼** → **확인 모달**(세션 id·첫 발화 표시) → 처리. 동기: 브릿지가 `--allow-new`로 자동생성한 세션은 Codex 확장 UI에 안 보여 직접 폴더를 뒤져야 정리 가능했음 → **대시보드가 유일한 정리 창구**.
  ...
  - **구현 보강 (Codex 검증 019ed785 · 2026-06-19)**:
    ...
    - **sessions 폴더 견고화(V11) 선행**: 삭제/휴지통/숨김은 `SESSIONS_DIR`에서 동작하므로, 그 폴더가 **확장·브릿지·실제 codex 셋 다 동일하게** 해석되는지 먼저 보장해야 함. 현재는 `env.CODEX_HOME‖~/.codex` 하드코딩(바이너리와 달리 auto-track 없음). 견고한 방식: codex 실행 직후 **실제 새 rollout이 떨어진 루트를 넓게 탐지**해 `~/.codex-bridge/codex-sessions-dir.txt`에 기록(바이너리 `codex-bin.txt`와 대칭) → 확장·브릿지 양쪽이 읽음. (현재 'SESSIONS_DIR만 스캔→역산'은 SESSIONS_DIR가 틀리면 실패하므로 **별도 탐지** 필요 — 검증 확인.)

### 5.2 역할 교체 (구현자 ↔ 검증자)
- **단기(경량 모드, 현 구조로 가능)**: "Codex 구현(bridge로 구동) → Claude 검토(읽기전용)". 검증자 읽기전용 원칙 + 별도 git worktree/임시 브랜치로 격리. 두 에이전트가 같은 워크트리 동시 수정 금지.
- **장기(대칭, 재설계)**: §6.

### 5.3 주입 제어 분리 — 사용자 계약 주입모드 · 기본지침 명시성 · UI (2026-06-19 합의·확정)
...
4. UI 3카드 분리 + 플랜 **라이브 표시**(active.json에 `permission_mode` 기록; 단 비원자=V7 영향) + 저장 피드백.
5. README 정정.
...
- **취약 높음 — V11(최우선·세션삭제 선행)**: `CODEX_HOME → SESSIONS_DIR·INDEX_FILE` (`codex-bridge.js:36-39`, `extension.ts:7-9`) + 의존하는 `findRolloutById`·`recentRollouts`·`newestRolloutSince`·watcher 전부. 루트 틀리면 후보/연결/`--allow-new`/삭제가 모두 헛돎. → `codex-sessions-dir.txt` 실측 탐지(바이너리 대칭).
...
**우선순위**: V11(세션삭제 선행) ≫ homedir override/문서화·require 가드 ≈ V7 원자적 저장(이 파일들 포함) ≫ dev 하드코딩 정리. workspace 키는 잔여 엣지만 문서화.
