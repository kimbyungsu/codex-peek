...
| ~~V4~~ ✅해결 | Stop 훅 **1회만 강제** | `verify-guard.js` bumpAttempts/MAX_ATTEMPTS | **해결(2026-06-21)**: 옛 코드는 `stop_hook_active`면 무조건 통과 → '다시 멈추기'로 1회 후 바이패스. 이제 재진입에서도 검증을 재확인하고, 한 턴 최대 MAX_ATTEMPTS(3)회까지 차단(`verify-attempts/<session>.json` 카운터, 새 턴이면 발화 ts로 리셋, 검증되면 즉시 통과+리셋). 무한정지 방지(3회 후 종료 허용)는 유지하되 '바이패스 가능한 1회'를 '바운드된 재검증 루프'로 격상. 테스트 V4 3케이스. |
...
## 9. 진행 로그
- **2026-06-21 — V4(Stop 1회만 강제) 해결**: 옛 `verify-guard.js`는 `stop_hook_active`(재진입)면 무조건 통과 → 검증 1회 차단 후 '다시 멈추기'로 바이패스 가능(품질 게이트가 아닌 1회 리마인더). 수정: 재진입에서도 verified 재확인 + 한 턴 차단 횟수 카운터(`verify-attempts/<session>.json`, atomicWrite) — `bumpAttempts(session, lastUserTs)`가 이전 턴 ts면 리셋, `!needVerify||verified`면 `clearAttempts`+통과, 아니면 `n=bump` 후 `n>MAX_ATTEMPTS(3)`이면 무한정지 방지로 통과(stderr 안내), 그 전이면 차단(`n/MAX` 표기). 안전밸브(무한정지 방지)는 유지하되 '바이패스되는 1회'를 '바운드된 재검증 루프(최대 3회)'로 격상. 카운터 키=claudeSession(V1과 동일 폴백). 테스트 3케이스(재진입 반복 차단→4회차 통과 / 검증 시 통과+리셋 / 새 턴 리셋). 잔여: V9·결정2.
