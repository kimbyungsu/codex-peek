...
- [ ] **한방 설치 부트스트랩 `codex-bridge install`**: 브릿지 home 생성 → JS 런타임 복사 → 기존 Claude 설정 백업 → 훅 안전 병합 → VSIX 설치 → codex 실행파일/home 탐지 → doctor → 성공·실패 리포트, + 제거/복원. **오픈소스 채택의 최대 장벽 = 복잡한 최초 설치**(현재 확장설치+JS배치+훅병합+동기화 수동). 참고: 사용자가 Memento 훅용 idempotent 설치기(`install-memento-context-hooks.sh`) 운영 경험 있음.
- [ ] **엉뚱 폴더/cwd 방어**: `ask`를 링크 없는 폴더(코드 레포 등)에서 돌리면 엉뚱한 워크스페이스로 새 세션 생성·오염(이번 세션 실사고 019ee492). 낯선 사용자는 더 자주 밟음. `--allow-new` 직전 '의도치 않은 컨텍스트' 경고/차단 or 워크스페이스 식별 강화. (싸고 정합성 효과 큼)
- [ ] **원자적 저장(V7)**: 자체 폴더 파일(links.json·sessions-meta.json·contracts 등) tmp+rename으로 — 현재 직접 writeFileSync 다수라 다중 창/중단 시 손상 위험.

**Tier 2 — 낯선 환경 적응 (다른 PC/셋업)**
- [ ] **자체폴더 home 일치 보장**: `CODEX_BRIDGE_HOME` override + **doctor가 확장 home vs 훅 home '양쪽 비교'해 불일치를 경고**(WSL/Remote/Container/포터블에서 확장호스트≠훅 home). 핵심은 '경로 찾기'보다 **확장·훅이 같은 브릿지 home을 보는지 확인**.
- [ ] **세션폴더 실측 탐지(V11 잔여) + codex 실행파일/home 수동 고정 UI**: (a) codex 실행 직후 '실제 rollout 떨어진 루트' 관찰→`codex-sessions-dir.txt`(비표준 세션경로 견고) (b) 자동탐지 깨질 때 폴더 선택창(showOpenDialog, 현재 없음)으로 home/실행파일 직접 지정 + 현재값/출처/마지막확인/상태 표시. **(a)와 (b)는 보완 관계** — 수동 고정만으론 자동탐지 잔여를 못 메움(Codex 검증 확인).
