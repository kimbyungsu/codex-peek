...
## 9. 진행 로그
- **2026-06-23 — 대시보드 디자인 정리 + 설치기 한방화 + 매듭(v0.1.20)**: 사용자 피드백 3건. ①장식 이모지 전부 제거(🌉 제목·🧠🔍🔁🔗⚙️🧩 헤더·🔌🗑🚀🎉📁): 제목은 파랑→초록 그라데이션 워드마크(`.brand`), 헤더 의미는 색 accent 바(파랑=Claude·초록=Codex·회색=기본)·`.to` 색칩·`.mono`로, 무결성 심각도는 `.sevdot`(빨강/주황 색점), 연결표시는 색 동그라미(●). ②섹션 경계 가독성: `h2.sec`에 border-top 구분선. ③검증 대기시간을 독립 `h2.sec` 섹션으로 분리 + 저장을 inline secondary→독립 행 기본 버튼으로(시인성). **설치기 한방화**: `*.vsix`는 git 제외라 신규 클론엔 vsix가 없어 `node install.js`가 확장을 건너뛰던 문제 → tryInstallVsix가 vsix 없으면 `npm run package`로 자동 빌드 후 설치(실측: clean:vsix 후 `node install.js`가 0.1.20 빌드+설치 확인). **스크린샷 갱신**: 로컬 화면 캡처(PowerShell System.Drawing)→대시보드 패널만 크롭→`docs/dashboard.png`(새 디자인, 71KB). README UI 설명도 de-emoji·대기시간·창격리 반영, 한방설치 자동빌드 명시. 검증: npm test(173단언)·lint·compile 통과, 0.1.20 빌드·설치. **Codex 교차검증은 사용량 한도(리셋)로 보류 — 코드 자기점검(이모지 0·배선 유지)만.** 사용자가 reload로 새 디자인 육안 확인함.
