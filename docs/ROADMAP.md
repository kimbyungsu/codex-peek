...
| ~~V2~~ ✅해결(git) | 파일변경 감지가 **툴 호출에 한정** | `verify-guard.js` gitChangedMaxMtime | **해결(2026-06-21, git 한정)**: 도구 감지(Write/Edit) 외에, git 저장소면 `git status --porcelain`로 바뀐 파일의 최신 mtime을 봐서 '사용자 발화 이후 변경'이면 트리거(Bash `sed -i`·`cat>`·생성기 등 수단 무관). 키워드 나열(취약) 대신 실제 변경을 봄. mtime은 V1 proof-freshness에도 합류(Bash 편집 후 재검증 강제). 삭제(rm·git rm)는 부모 디렉터리 mtime으로 감지. **잔여**: 비-git은 도구 감지로 폴백, gitignore된 파일·git status 10s 타임아웃 시 폴백. 테스트 V2 6케이스(삭제·대조 포함). |
