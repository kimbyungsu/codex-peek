...
**우선순위(2026-06-21 갱신)**: ~~V11~~·~~엉뚱 폴더/cwd 방어~~·~~homedir override(CODEX_BRIDGE_HOME)+단일 BRIDGE_DIR~~·~~V7 원자적 저장~~·~~명령 quoting~~·~~normWs NFC~~ **모두 구현됨**(§8.1·본 §10 본문 참조) → 남은 것: **V11 잔여(세션폴더 실측 탐지)** ≫ `.js` codex 확장 home 탐지 보완 ≫ dev 하드코딩 정리 ≫ **설치기(node 절대경로 pin·런타임 JS 복사·훅 경로 갱신)**. workspace 키 잔여 엣지는 가드로 해결됨.
