...
- [ ] **🔴 명령 호출 견고화(런타임·영향 큼)**: 훅 등록(`settings.example.json`)과 검증/안내 문구(`contract-lib.buildVerifyDirective`·`verify-guard`·`codex-guard`)가 `node <경로>`를 **따옴표 없이 + 맨 `node`(PATH 의존)**로 씀 → **홈 경로에 공백(예 `C:\Users\First Last`)이면 매 턴 깨지고**, node가 훅 spawn PATH에 없으면 전부 안 돎. 흔한 케이스라 다수 사용자 영향. → 경로 따옴표 감싸기 + node 절대경로(설치기가 pin)·또는 가용성 진단.
- [ ] **자체폴더 home 일치 보장**: `CODEX_BRIDGE_HOME` override + **doctor가 확장 home vs 훅 home '양쪽 비교'해 불일치 경고**(WSL/Remote/Container/포터블에서 확장호스트≠훅 home). ⚠ 도입 시 `readActive()`(codex-bridge.js:65) 등 `os.homedir()/.codex-bridge`를 **직접 조립하는 곳들을 단일 `BRIDGE_DIR`로 통일**(override 누락 방지). 핵심은 '경로 찾기'보다 **확장·훅이 같은 브릿지 home을 보는지 확인**.
- [ ] **`.js` codex에서 확장 home 탐지 스킵 보완**: 확장 `syncCodexHome()`이 codex 경로가 `.js`면 home 탐지를 스킵(extension.ts) → 그 배포형태선 확장이 매 활성화 home 갱신을 못 하고 브릿지 detect-home에만 의존. 확장도 탐지하도록(또는 브릿지 detect-home을 활성화 때 호출).
