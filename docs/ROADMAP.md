...
**Tier 1.5 — 설치기 토대 (설치기보다 먼저 · 경로 유연성 미완분 중 설치기가 의존하는 것 · 2026-06-21 Codex 검증으로 분리)**
경로 유연성은 현재 '부분 완성'(표준 Windows=그린, WSL/Remote/Container·비표준 세션폴더·유니코드 경로=옐로우). 설치기가 의존하는 토대를 먼저:
- [ ] **자체폴더 home 일치 보장**: `CODEX_BRIDGE_HOME` override + **doctor가 확장 home vs 훅 home '양쪽 비교'해 불일치 경고**(WSL/Remote/Container/포터블에서 확장호스트≠훅 home). 핵심은 '경로 찾기'보다 **확장·훅이 같은 브릿지 home을 보는지 확인**. (설치기가 만드는 폴더와 런타임이 보는 폴더 일치의 토대)
- [ ] **`normWs` NFC 정규화 통일**: 3카피(codex-bridge·contract-lib·extension) 전부 비교 전 `.normalize("NFC")` 추가 → 한글/유니코드 경로가 환경별 NFC/NFD로 갈려 계약키(sha1)·링크키가 분리되는 것 방지. ⚠ **마이그레이션 주의**: 키 계산이 바뀌므로 기존 비ASCII 경로(예: `d:\프로그래밍\에이전트 활용`)의 계약·링크가 새 키로 안 잡힘 → 1회 마이그레이션(옛 키→새 키 이전) 또는 재연결 안내 필요. **설치 전에 해야 설치 후 키 변경 마이그레이션 부담 회피.**
- [ ] **detectCodexHome 거짓 성공 수정**: `detectCodexHome()`이 `atomicWrite(f,home)` 반환을 안 보고 `ok=true`(codex-bridge.js) → 원자적저장 '거짓 성공' 잔재가 doctor/detect-home 경로에 남음. 반환값 확인으로 수정(설치기/doctor 신뢰성).

**Tier 2 — 낯선 환경 적응 (설치기 이후/동시 가능)**
- [ ] **세션폴더 실측 탐지(V11 잔여) + codex 실행파일/home 수동 고정 UI**: (a) codex 실행 직후 '실제 rollout 떨어진 루트' 관찰→`codex-sessions-dir.txt`(비표준 세션경로 견고) (b) 자동탐지 깨질 때 폴더 선택창(showOpenDialog, 현재 없음)으로 home/실행파일 직접 지정 + 현재값/출처/마지막확인/상태 표시. **(a)와 (b)는 보완 관계** — 수동 고정만으론 자동탐지 잔여를 못 메움(Codex 검증 확인). (표준 Windows v1 설치기의 hard blocker는 아님 — doctor 경고로 둘 수 있음)
