...
| # | 취약점 | 코드 위치 | 핵심 |
|---|---|---|---|
| ~~V1~~ ✅해결 | 검증 "성공"이 아니라 **명령 문자열**만 확인 | `verify-guard.js` checkProof · `codex-bridge.js` writeProof | **해결(2026-06-21)**: 브릿지가 실제 성공(exit0·비어있지 않은 응답) 시에만 `proofs/<claudeSession>.json` 기록 → verify-guard가 명령 문자열이 아니라 '이번 사용자 발화+마지막 변경 이후 + status=success + answerChars>0' proof만 인정. `echo`·실패·미연결·이전턴·검증후수정 전부 거름. 테스트 19케이스. |
