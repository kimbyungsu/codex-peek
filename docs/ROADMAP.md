...
## 9. 진행 로그
- **2026-06-22 — V9(withContract cwd 의존) 해결**: `withContract(prompt, ws)`로 워크스페이스 명시 인자화. `cmdAsk`가 ask 시작 시 `const ws = workspace()` 한 스냅샷을 만들어 modelPref·엉뚱폴더 가드(here)·wsKey·두 runCodex의 withContract 모두 같은 ws로 통일. 효과: Codex 계약이 링크/proof/가드와 동일한 '검증된 워크스페이스'로 로드 → cwd가 어긋날 때 codex 계약이 전역/타 프로젝트로 새는 잠재 위험 제거. (현재 `currentWs()===workspace()`라 동작 무변화지만, contract-lib 기본값에 대한 암묵 의존을 끊고 미래 분기 위험을 없앰.) writeProof의 workspace는 비게이트 정보용이라 그대로(동일 값). 테스트 `withcontract.test.js` 6케이스(ws별 계약·폴백·비유출). 잔여: 결정2.
