...
- [ ] **테스트(V8)**(기여자·신뢰성), **README/설치·제거 문서**, dev 하드코딩(낮음). (~~V9~~ 완료 ✅, ~~결정2 1단계~~ 완료 ✅ — 아래)
- [~] **결정2(검증 무결성)**: **1단계 완료(2026-06-22, 5a3232e)** — '검증이 필요했는데 미완으로 끝난 턴'(V4 MAX 소진)을 침묵하지 않고 무결성 신호 채널(integrity.json)→상태바 빨강+펄스+대시보드 배너+확인(ack)으로 가시화. cry-wolf 위험 큰 fuzzy 근거대조 대신 고신뢰 사건부터. **2단계(후속)**: Codex/Claude 답의 인용 근거(file:line)↔실제 연 파일(코덱스 rollout function_call·클로드 transcript tool_use) 대조를 **노랑(로그)→고신뢰만 빨강**으로 단계 추가. 양측 인용 파싱·세션 전체 미등장 파일만 고신뢰.
