...
- **Track selector (2-track default / 3-track advisory)** — 3-track adds a **scope ledger** card: for the files you're changing right now, it mines *this project's own git history* for "what historically changed together" candidates. Purely local git queries (nothing sent anywhere), blocks/forces nothing, saved per project, and when the sample is too small it honestly says "no data" instead of guessing.
