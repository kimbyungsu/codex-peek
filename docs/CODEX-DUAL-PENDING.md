...
> 출처: 사용자가 다른 PC(개발자 홈의 `Documents\codex-peek`)에서 개발한 "Codex↔Codex 이원화"
...
- 이 PC 핫픽스(적용·검증 완료): ~/.codex/hooks.json 4개 훅의 command·commandWindows를
  `node "<홈>/.codex-bridge/codex-hook.js"`로 교체(PS·cmd 양쪽 실측 통과, PATH 3중 확인).
...
> ★**v2.6 캐논 압축(2026-07-19 — 사용자 승인: '핵심 프롬프트가 과한 장문 나열형' 지적·'핵심' 명칭은 유지)**:
> 기능 등가 축소 — ko 3,611→2,820자(22%↓)·en 7,414→5,790(22%↓)·footer pass-notes 528→약 310. 골격='한 논리
> 정본, 수신자별 두 렌더링'(분류 정의 정본=검증자 baseline 유지·구현자 rejudge=압축 분류표+행동·footer=행동
> 요약+재판단 규약 위임). 삭제=수신자 간 중복 재서술·수사적 부연(이유 설명절)·보류 부연 축약 / 유지=blocker
> 5기준 전문·3분류 정의·5항 기계 블록 문법 전문·자동 등록 3줄 규칙(성공 영수증/거부·실패 수동/승격 수동)·
> 처리표·기각권·확인 단계 종결·반박 대등·보류 3분류 고유 첨부(반박 근거·왕복 이력/시도 내역·잔여 위험 —
> 구현검증 1차 blocker 복원)·footer 자립 최소 규약(rejudge 미주입 경로[검증 모드 off의 직접 ask]에서도
> [주의] 승격 기록·[백로그] 수정 금지·수동 fallback이 서게 — 1차 blocker 복원). 재비대 방지 상한 테스트
> (ko≤3,000·en≤6,200 — p12-profile [2b]·80단언/0 실패). 판단 검증 2왕복(실패→통과(보완))+구현 검증 2왕복(실패→통과).
> 무결성 캐논 1글자 불변(해시 스냅샷 유지). 부속 판단 기록: '핵심'의 실체=범위 축+운영 축 합성 — 운영 축
> 일부는 기계화(2b는 프로필 공통·2c는 core 전용), 명칭·구조는 사용자 결정으로 현행 유지. 기계층(2c 판독·
> 자동 등록)의 integrity 확장은 별도 계약 변경 — 사용자 결정 대기.
...
- 착수 순서는 **HANDOFF §1-0 끝의 '다음 순서'가 정본(2026-07-17 갱신)**: ~~P-8 1단~~(완료 07-15) →
  ...
  ~~P-6b 서술 정정~~(완료 07-19 — P-6b '서술 정정 완결' 참조) → ~~fallback↔훅 경합~~(완료 07-19 — P-6b '잔여 경합 완결'·tests/p6b-race 29단언) → **다음 착수=3트랙(정찰) 잔여**(§5~§11 보존分). (이 단락의 옛 참조 'HANDOFF §깃헙 동기화 절 07-14 14:02'와 'P-6·P-5 같은 묶음 권장'은
