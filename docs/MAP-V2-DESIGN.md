...
- **P10** ✅ 통계·비용·건강도(scout-usage 확장·지표 분리). **[상세 설계 v1 동결·증분 1~3 구현·독립 검증·0.1.88 설치 완료 2026-07-26]**
  정본=`docs/MAP-P10-DESIGN.md` — 외부 호출/실행 묶음/지도 항목을 서로 다른 분모로 고정하고,
  실제 토큰과 글자 수를 분리하며, 현재 Project MAP 건강도는 28일 이력이 아닌 판독 시점 스냅샷으로 둔다.
  본문 변경 없는 독립 설계 검증을 통과한 뒤 세 증분의 구현·전체 검사·독립 구현 검증·로컬 설치를 마쳤다.
  P10이 이 로드맵의 마지막 계획 Phase이며 P11은 정의하지 않았다. 이후 새 Phase는 사용자 지정 범위가 있을 때만 설계한다.
...
**P9-5 UI**(표현은 VERIFY-GOVERNANCE §8 전역 계약 준수 — 기술용어 금지·상황예시·달라지는 것/유지되는 것 명시): 대시보드 3트랙 구역 'MAP 대기 선택' — intent 카드 목록(버튼 4종·scope 확정은 2단: 대상 목록 표시→확정)+needs-investigation 정보 행+recovery 행(진입 상태·잔존 잠금 시만·2단 버튼)+정책 요지(유효 leaf 수·최근 supersession). ko/en·postMessage type="intentAct"(ledgerAct 패턴)·단일-flight·결과는 사람 문장.
...
### P9 v12 소규모 개정 (2026-07-24 — ⓒ 확정 반영·구현 증분 1과 동반)

ⓒ("검증 담당이 판정") 확정에 따른 v12 개정 3건 — v12 본문의 저장·전이·선기록·재개 계약은 정책 op 경로에
그대로 유효하며, 아래만 바뀐다:
① **기본 분류 개정**: change_steward·change_authority의 기본 분류를 intent-choice→**verifier-resolved**로
   변경(비정책 의미 제안=검증 담당이 근거 파일을 읽고 판정·자동 반영 — P8 해소 경로 재사용). 정책 op
   3종(create/supersede/revoke_intent_policy)만 intent-choice 유지(사용자 철학의 명시 산물 — ab-3 불가침).
② **기존 pending 전환 규칙**(전사 검증 blocker④ 이행): 이미 저장된 '비정책 op+classification=intent-choice'
   pending은 P8 실행기의 item 재개 경로에서 **재분류(classifyPatch 재실행)**로 전환 — classified 상태는
   재분류 가능(실측: claimed·종결만 거부). 재분류 후 새 분류(verifier-resolved)로 기존 해소 경로 진행.
   방치=영구 잔존이던 경로가 자연 소진된다.
③ **재사용 한정 명문**: 비정책 재라우팅의 저장·재개 정본=P8 verifier-resolution 계약. P9 신설분(카드·정책
   저장·frontier 자동 적용·recovery)은 정책 op와 운영 복구에 한정.

**구현 메모 — P9 증분 2A(2026-07-24)**: P9-0b의 typed chosenMeaning·거부 expireCode·policyDelegation
P2 경계만 먼저 구현했다. policyDelegation은 유효 frontier leaf+policyFp뿐 아니라 typed disposition=apply,
opClass, scopeTarget, exclusions를 적용 시점에 다시 확인한다. 관여 대상은 명시 대상·생성물·merge 외부 목적지·
split edgeReroute뿐 아니라 현재 topology에 순수 적용을 미리 계산한 실제 changedIds까지 포함하며, 그 전부가
비-project 범위 안이어야 한다. typed 뜻은 predicate v1 op-class와 정본 단계부터 한 쌍이고, 정책 op 자체에는 사용할 수 없다. 카드/선택 선기록,
전이 장부, sweepIntentAuto, recovery-action, UI는 다음 증분이며 아직 발동하지 않는다. 이 메모는 구현 경계 기록이고
v12의 저장·재개 순서나 사용자 선택 불가침을 바꾸는 새 설계 결정이 아니다.

**구현 메모 — P9 증분 2B(2026-07-24)**: `bridge/map-intent.js`에 정책 충돌 카드 파생 뷰와 충돌 선택
선기록까지만 구현했다. classified·intent-choice·비정책 pending마다 유효 frontier leaf를 scope 특이도
(entity>subgraph>project)로 좁히고, 같은 최고 특이도에서 typed apply/decline이 갈릴 때 정렬 head 집합 지문으로
한 장에 합친다. 조회는 파일을 만들지 않는다. 선택 시에는 현재 frontierHash·dch·head 집합을 map lock 안에서
재검사하고, 승계 head의 scope·scopeTarget·predicateExpr·exclusions를 복사하되 chosenMeaning만 사용자 선택으로
바꾸며 선택 시점 VerificationBasis를 새로 캡처한다. 새 policyId·createdFromDecision·supersedePatchId와 완성된
canonical supersede patch 전문·opHash를 `<conflictKey>-<cardId>.json`에 phase=chosen으로 먼저 원자 기록한다.
같은 cardId 재호출은 동일 선택만 멱등이고, 다른 뜻 덮어쓰기·동일 충돌의 미완 다중 선택·낡은 pfh/dch는
거부한다. parked 뒤 명시 재시도는 WAL 선행 복구 상태표와 함께 다음 증분에서만 열며 2B 표면은 새 세대 생성을
거부한다. 이 단계에서는 propose/classify/apply·정책 파일 생성·자동 스윕·UI가 모두 0이다. typed 뜻과 결합한
predicate v1은 `{version,kind,opClass}` 정확 키만 자동 해석하도록 2A 경계도 보강했다. 다음 증분은 이 선기록의
phase 재개와 정책 적용, 위임 전이 장부 및 `sweepIntentAuto` 연결이다.

**구현 메모 — P9 증분 2C(2026-07-24, 인계 잔여 구현 완료)**: 2B가 선기록한 충돌 선택을 저장된
canonical patch 그대로 propose→classify→apply하고, decision/WAL/pending 각 중단 지점에서 재개하는 실행부를
연결했다. 비정책 대기 변경은 유효 frontier leaf를 entity>subgraph>project 순으로 매칭해 단일 apply 정책이면
새 read-set의 policyId·policyFp·frontierHash에 재결속한 replacement를 **위임 장부 선기록 뒤** 적용하고, decline
정책이면 policy-declined로 종결한다. 동급 뜻 갈림은 자동 적용하지 않고 충돌 카드로 남긴다. `parked`는 자동
스윕에서 재복구하지 않으며, 대시보드의 명시 재시도만 충돌 선택의 저장 canonical 전문을 unpark하거나 위임의
WAL을 먼저 정리한 뒤 새 attempt를 append한다. 발동은 3트랙 대시보드 조회와 P8 enrich 종료 직후 1회이다.
선택 기록·재개·위임·복구의 core API도 호출 시점과 잠금 안에서 3트랙을 재확인하므로, 카드/모달을 띄운 뒤
2트랙으로 바뀐 경우까지 서랍·로그·적용·복구 쓰기 0으로 닫는다.

복구 표면은 invalid/unreadable topology에만 열리고 decisions·로컬 snapshot·최근 git 20커밋에서 mapId 후보를
모은다. 1단계는 P2 `recoverCorruption`으로 `topology.recovered.json`을 만들고, 동시에 브릿지 홈의 내구 계획에
planId·nonce·손상 원본 지문·선택 후보 지문·복구본 지문을 결속한다. 2단계는 이 계획과 현재 파일을 모두
재대조한 뒤 손상 원본을 `topology.corrupt-<ts>.json`으로 보존하고 교체한다. 첫 rename 전 `replacing` 전이를
선기록하므로 두 rename 사이 또는 설치 직후 종료돼도 다음 상태 판독이 같은 계획으로 전진/원복해 수렴한다.
dead nsLock만 P2 GC 정본으로 회수한다. 대시보드는 반대 정책 선택·조사 정보 행·parked 명시 재시도·손상 주의·
복구 2단을 ko/en 사람 문장과 호스트/웹뷰 단일-flight로 표시하며, busy·2트랙·저장소 불일치의 호스트 조기
종료도 `intentDone`을 돌려준다. v12 ⓒ에 따라 폐기된 비정책 4버튼 카드는 되살리지 않는다.
첫 독립 검증의 blocker 5건(모달 뒤 2트랙 쓰기, parked 재시도 표면 부재, 복구 plan/nonce 미결속, rename 중단 창,
호스트 조기 종료 잠금)을 전부 수용해 위 계약과 실행 반례로 봉합했다. 테스트는 foundation 50·conflict 43·
auto/delegation 26·recovery 30·UI 계약 13단언과 기존 P2/P8 회귀에 분산돼
있다. 정책 파일의 create/supersede/revoke 불변 스키마와 적용기는 유지되며, 이 증분의 사용자 카드 신설은
서로 반대인 유효 정책을 supersede하는 경우로 한정한다. P10 통계·비용·건강도는 별도 Phase다.

### P9 착수 전 사용자 지시 부기 (2026-07-24 — 전사 완료·ⓒ 확정[쉬운 언어 재질문으로 재확인])
