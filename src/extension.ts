...
<body><main class="shell">
  ...
  <div class="card">
    ...
    <div class="hint">${t("무결성: 관련 호출부·경합·회귀·문서까지 넓게 탐색 — 모든 결함 부재를 보증하지는 않음 · 핵심: 선언된 요구·직접 영향 중심. 비차단 지적은 세 갈래(v2.4) — [주의](보안·데이터 인접·구체 경로 필수 — 구현모델이 재판단: 지금 고침 또는 사용자 판단으로 승격해 보관함 기록), [보완](자명·국소 — 그 루프에서 일괄 반영+확인 1회), [백로그](범위 밖 제안 — 보관함 기록·갚을 의무 없음·채택 시만 작업) — 범위 밖 잔여 위험이 남을 수 있고, 모델에 주입되는 처리 규약이라 자동수정 차단은 없음 — 기계적 왕복 상한은 아래 '검증 왕복 상한'(기본 무제한·유한 상한에서 기록 실패 시 미집계로 표시)으로 별도 설정. 프로필·언어 선택은 이후 시작되는 검증(ask)부터 즉시 적용됩니다 — 이미 진행 중인 턴의 주입 지침은 바뀌지 않아 한 턴 안에서 규약이 섞일 수 있으니, 턴 전체 일관성이 필요하면 다음 프롬프트를 보내기 전에 전환하세요. 핵심으로 작업했다면 push·배포 전에 무결성 승격 검증 1회를 권장합니다.", "Integrity: searches widely across call sites, races, regressions, and docs — does not guarantee absence of all defects. Core: focuses on the declared requirements and their direct impact; non-blocking findings are marked three ways (v2.4) — [caution] (security/data-adjacent, concrete path required — the implementer re-judges: fix now or escalate to your judgment and park it), [notes] (obvious & local — applied in that loop as a batch with one confirmation), [backlog] (out-of-scope proposal — parked, no repayment duty, work only when adopted) — residual out-of-scope risk may remain, and this is a protocol injected into the models with no auto-fix blocking — a mechanical round-trip cap is available separately via 'Verification round cap' below (unlimited by default; under a finite cap, bookkeeping failures show as untracked). Profile/language choices apply immediately to verifications (asks) started afterwards — directives already injected into the current turn do not change, so protocols can mix within one turn; switch before your next prompt if you need whole-turn consistency. If you worked in Core, one Integrity escalation verification before push/deploy is recommended.")}</div>
    ...
    <div class="hint">${t("빈값=무제한(기본 — 지금까지와 동일). 숫자를 넣으면(예: 3) 한 번의 지시 안에서 검증 왕복을 그 횟수까지만 돕니다 — 마지막 회차엔 '이번이 마지막 왕복'이라는 예고가 붙고, 그 다음 요청은 기계적으로 거부됩니다. 그 시점에 심각한 결함(blocker)이 남아 있으면 '완료'로 포장하지 않고, 선택지를 붙인 보고(예: 더 검증하도록 승인 / 이 상태로 두기 / 상한 변경)로 사용자의 지시를 기다립니다. 저장 후 다음 지시부터 적용 — 진행 중인 지시의 상한은 그대로예요. 상한을 설정한 지시에서 내부 기록이 실패한 왕복은 세지 못하며 '미집계'로 표시됩니다(절대 상한 아님 — 무제한일 땐 세지 않고 표시도 없음).", "Empty = unlimited (default — same as before). With a number (e.g., 3), verification runs at most that many round-trips per instruction — the last one carries a 'this is the final round-trip' notice, and the next request is mechanically refused. If serious defects (blockers) remain at that point, the report is not dressed up as 'done'; instead it presents options (approve more verification / leave as is / change the cap) and waits for your instruction. Applies from the next instruction after saving — the running one keeps its cap. Under a finite cap, round-trips whose internal bookkeeping failed cannot be counted and are shown as 'untracked' (not an absolute cap — with unlimited, nothing is counted or shown).")}</div>
