...
interface BridgeState {
  ...
  // [HARNESS-STRUCTURE §B3 · D4] 수칙 흐름 4칸(개요 패널 카드) — bridge/rules-flow.js rulesFlowFor(ws) 그대로(판독 실패=null → 카드가 '자료 없음'으로)
  rulesFlow: { core: { st: string; count: { supportedEnv: number; alwaysBlocker: number; outOfScope: number; total: number }; injectedChars: number }; archive: { state: string; count: number; max: number; selectCap: { items: number; bytes: number }; lastSelection: { ts: string; selectedCount: number; askId: string } | null }; proposals: { pending: number; max: number; nextCheck: { tickK: number; tickCount: number; unusedDays: number; lastTickTs: string } }; curator: { lastTs: string; lastOutcome: string; lastReason: string; lastTrigger: string; running: boolean; metrics: { adoptRate: number; proposedTotal: number; adopted: number; declined: number; archiveSize: number; selOverCount: number; unusedCount: number }; lastItems: Array<{ id: string; title: string; op: string; status: string }> } } | null;
  ...
  scoutNames: ScoutNames; // 정찰 담당 표시명 묶음(배포 브릿지 scoutArmNames 정본 · 한/영 동시) — 화면은 이름을 짓지 않고 여기서 고른다(문구 감사 2026-09-21)
  scoutArm: { raw: string | null; eff: "self" | "deepseek" | "codex"; hasKey: boolean; slot?: string; defaultArm?: "self" | "codex"; ready?: boolean; reason?: string | null; noKeyArm?: "self" | "codex"; noKeyReady?: boolean } | null; // noKeyArm/noKeyReady=키 없을 때 실행될 담당과 그 담당의 준비 상태(5판 blocker — 키 있는 DeepSeek 의 ready 로 키 없는 담당을 보장하지 않게) // defaultArm/ready/reason=묶음 (다) 모드 기본·정찰 준비 // 탐색 담당(2026-07-20·P6 codex 추가) — raw=명시 선택(반대 언어 슬롯 상속·null=미지정), eff=실효(deepseek는 키 없으면 self 강등·codex는 강등 없음), slot=계산 언어. null=2트랙/ws 없음
  ...
  envelope: { label: string; btn: string | null; btn2: string | null; btn3?: string; repo: string; tone: string; lang: Lang; proposal?: string; gen?: string; axes?: Record<string, string[]>; wsKey?: string; arc?: { state: string; count: number; max: number }; cands?: Array<{ id: string; kind: string; n: number; title: string; status: string; gen: string; wsKey: string }>; rules?: Array<{ target: string; targetHash: string; axis: string; index: number; itemFp: string; text: string; tag: string }>; signals?: Array<{ kind: string; n: number; title: string }>; ride?: string } | null; // 거버넌스 증분 1(2차 배치 정정) — 검증 경계(수칙서). 최상위=2트랙 기본 프로젝트 포함·lang=렌더 슬롯 결속. null=부재/ws 없음. arc=[4a] 서고(2층) 현황
}
...
// P-12 2b: 왕복 예산 — bridge/contract-lib.js normVerifyBudget/normCodexVerifyBudget와 동형(스키마 정합).
// 1 이상 정수=상한, 그 외=0(무제한·무회귀). C-C는 '부재'만 상속 — 명시 0='이 슬롯만 무제한'(B3).
function normVerifyBudget(o: any): number {
  const v = o && o.verifyBudget;
  return (typeof v === "number" && Number.isInteger(v) && v >= 1) ? v : 0;
}
function normCodexVerifyBudget(o: any): number {
  const v = o && o.codexVerifyBudget;
  if (typeof v === "number" && Number.isInteger(v) && v >= 1) return v;
  if (v === 0) return 0; // 명시 무제한(상속 차단)
  return normVerifyBudget(o);
}
...
interface Contract {
  ...
  verifyBudget: number; // P-12 2b: CL-C 왕복 예산(0=무제한)
  codexVerifyBudget: number; // P-12 2b: C-C 슬롯(부재=CL-C 상속)
  codexVerifyBudgetRawVal: number | null; // 상속 '표시'용 raw-presence — null=부재(상속), 0=명시 무제한, n=명시 상한(저장 경로 미포함 · 1차 B3)
...
function loadContract(ws?: string | null, lang?: Lang): Contract {
  ...
  return {
    ...
    verifyBudget: normVerifyBudget(o),
    codexVerifyBudget: normCodexVerifyBudget(o),
    codexVerifyBudgetRawVal: (typeof o.codexVerifyBudget === "number" && Number.isInteger(o.codexVerifyBudget) && o.codexVerifyBudget >= 0) ? o.codexVerifyBudget : null,
...
// 정찰 담당 표시명(문구 감사 2026-09-21 · D-2026-09-21-scout-wording-mode-default): 이름은 배포 브릿지 정본
...
function scoutNameExt(ws: string | null, key: keyof ScoutNameSet, lang?: Lang): string {
  const n = scoutNamesExt(ws);
  if (!n) return /Text$/.test(key) ? ((lang || loadLangExt()) === "en" ? "the mode's default scout" : "모드 기본 정찰 담당") : String(key); // 대상 폴더 없음(드묾): 중립 구절
  return ((lang || loadLangExt()) === "en" ? n.en : n.ko)[key];
}
function scoutArmViewExt(ws: string, slotIn?: Lang): { raw: string | null; eff: "self" | "deepseek" | "codex"; hasKey: boolean; slot: Lang; defaultArm: "self" | "codex"; ready: boolean; reason: string | null; noKeyArm: "self" | "codex"; noKeyReady: boolean } {
  ...
  // 키 없을 때 실행될 담당과 '그 담당'의 준비 상태(5판 blocker): 키가 없으면 실효와 같으니 위 ready 를 그대로, 키가 있으면(실효가 DeepSeek 일 수 있음) 가정 담당을 따로 점검
  // 규칙=scoutArmView 의 강등 규칙과 동일: 명시 선택이 self/codex 면 그 선택, DeepSeek 선택·미지정이면 모드 기본(키 없는 상태에선 eff 와 같다). 이 함수는 시험이 산출물에서 통째로 추출해 실행하므로 바깥 헬퍼를 부르지 않는다.
  const noKeyArm: "self" | "codex" = raw === "self" || raw === "codex" ? raw : defaultArm;
  let noKeyReady = ready;
  if (hasKey || eff !== noKeyArm) { try { const lib: any = bridgeLib(); noKeyReady = lib && typeof lib.scoutArmReadinessOf === "function" ? lib.scoutArmReadinessOf(noKeyArm, hasKey).ready !== false : true; } catch { noKeyReady = true; } }
  return { raw, eff, hasKey, slot, defaultArm, ready, reason, noKeyArm, noKeyReady };
}
...
// 재점검일 뿐 — 담당별 지문(fp)당 1회, 모달 없이 자동 실행(결과는 알림으로 고지=과금 투명성).
...
function maybeAutoReprobe(ws: string | null, rv: any): void {
  try {
    ...
    const need0 = [...probeTargetsFor(ws)].filter((k) => (rv as any)[k] && !(rv as any)[k].ok && AUTO.has(String((rv as any)[k].reason || "")));
    if (!need0.length) return;
...
// 자동 재점검 실행부(3차 blocker② ab-6): probe는 spawnSync 기반이라 확장 호스트에서 직접 돌리면
...
function runAutoProbeDetached(ws: string | null, need: string[], keys: string[]): void {
  ...
  try {
    ...
    const payload = {
      ...
      inv: need.includes("precision") ? codexInvForProbe() : null,
      prompt: tE("아무 도구도 쓰지 말고 'ready'라고만 답하라.", "Reply with only 'ready'; use no tools."),
    };
    const child = spawn(process.execPath, [runner], { windowsHide: true, stdio: ["pipe", "pipe", "ignore"], env: { ...process.env, ELECTRON_RUN_AS_NODE: "1" } });
    let out = "";
    ...
    child.on("close", (code: number | null) => {
      ...
      try {
        ...
        const notes = need.map((k) => {
          const e = r ? r[k] : null;
          if (!e) return k + ": ?";
          if (e.skipped) return k + ": " + tE("설정 없음(건너뜀)", "not configured (skipped)");
          return k + ": " + (e.ok && e.write && e.write.ok ? "OK" : String(e.detail || e.err || (e.write && e.write.reason) || tE("실패", "failed")).slice(0, 120));
        });
        vscode.window.showInformationMessage(tE("설정 변경을 감지해 준비 점검을 자동으로 다시 했어요 — ", "Config change detected — readiness was re-checked automatically: ") + notes.join(" · "));
        maybeSpawnEnrichExt(ws, "probe"); // 수동 경로와 동일한 후속(P8 발동 ⓑ — 게이트·동의는 함수 내부)
        vscode.commands.executeCommand("codexBridge.refresh");
...
async function setScoutArmFromUi(ws: string | null, arm: "self" | "deepseek" | "codex", slotLang?: Lang): Promise<void> {
  ...
  if (arm === "deepseek" && !readDeepseekView().hasKey) {
    ...
    const koM = "DeepSeek 키가 아직 없어요. 지금 저장하면 선호만 기록되고, 키를 등록(⚙️ 고급설정)하기 전까지는 " + scoutNameExt(ws, "defaultText", "ko") + "이 맡아요."; // 모드 기본 담당(문구 감사 2026-09-21 — 'Claude 로 동작' 단정 제거)
    const enM = "No DeepSeek key yet. Saving now records the preference only — " + scoutNameExt(ws, "defaultText", "en") + " runs until you register a key (⚙️ Advanced).";
    const goOn = await vscode.window.showWarningMessage(
...
// P-12 2b: 웹뷰 예산 입력 파싱 — 0 이상 정수 문자열=그 수(0=명시 무제한·C-C 상속 차단), 그 외(빈값 포함)=
// undefined(JSON 직렬화에서 키 탈락=비물질화 — C-C는 상속 복원, CL-C는 무제한).
function parseBudgetInput(v: unknown): number | undefined {
...
function readEnvelopeView(ws: string | null): { label: string; btn: string | null; btn2: string | null; btn3?: string; act?: string; driftTarget?: string; repo: string; tone: string; lang: Lang; proposal?: string; candsView?: string; gen?: string; axes?: Record<string, string[]>; wsKey?: string; cands?: Array<{ id: string; kind: string; n: number; title: string; status: string; gen: string; wsKey: string; ts: string; why?: string; op?: string; explain?: { happened: string; ifAdopted: string; ifNot: string; recommend: string } | null }> } | null {
  ...
  try {
    ...
    if (typeof CL9.readVerifyEnvelope !== "function") return null; // 구 런타임 — 표시 없음
    ...
    let envSignals9: Array<{ kind: string; n: number; title: string }> = []; // [재편 B §3-1b] 참고 신호(합성) — 후보와 분리 전달
    ...
    try {
      ...
      if (typeof CL9.readFindingsLedger === "function" && typeof CL9.campaignFileFor === "function") {
        ...
        if (rows9.length) {
          ...
          adm9 = ""; void dem9; void esc9; // [2026-08-29 실보고] "인정·강등·승격" 집계는 기계 사정(지적 입장 심사 결과)이라 수칙 카드에 비노출 — 사용자가 수칙 자체가 자동 폐기되는 것으로 오해
        }
      }
    } catch { adm9 = ""; }
    const cands9 = candsFor9(); // 헬퍼 단일 경로(초안 대기 분기와 같은 계산 — 2026-08-21)
    // [재편 B §3-1] 수칙 목록(통합 표시) — 행마다 {target,targetHash,axis,index,itemFp} 결속(§3-2 빼기 TOCTOU
    // 재료·ab-2). itemFp=정규화 문안 sha1(빌더 대조와 동형). 서고 행은 도장 일치(active)일 때만.
    const rules9: Array<{ target: string; targetHash: string; axis: string; index: number; itemFp: string; text: string; tag: string }> = [];
    let ride9 = "";
    try {
      const sha1R = (s: string) => crypto.createHash("sha1").update(String(typeof CL9.normBacklogTitle === "function" ? CL9.normBacklogTitle(s) : s), "utf8").digest("hex");
      for (const ax of ["supportedEnv", "alwaysBlocker", "outOfScope"]) (evv.data[ax] || []).forEach((x: string, i: number) => rules9.push({ target: "core", targetHash: String(hash9 || ""), axis: ax, index: i, itemFp: sha1R(x), text: String(x), tag: "always" }));
      const ah9R = (() => { try { return (CL9.loadContract(ws, slot) || {}).archiveHash || null; } catch { return null; } })(); // arc9 IIFE는 아래에 선언 — 도장 일치 판정만 자체 수행(동일 규칙)
      if (ah9R && typeof CL9.readVerifyEnvelopeArchive === "function") {
        const arF9 = CL9.readVerifyEnvelopeArchive(repo9);
        if (arF9 && arF9.st === "ok" && arF9.sha1 === ah9R) arF9.data.alwaysBlocker.forEach((x: string, i: number) => rules9.push({ target: "archive", targetHash: String(arF9.sha1), axis: "alwaysBlocker", index: i, itemFp: sha1R(x), text: String(x), tag: "rel" }));
      }
    } catch { rules9.length = 0; /* 목록 실패=표시 생략(카드 본체 유지) */ }
    try { // "이번 검증에 실린 수칙 N개" — 마지막 검증 판 선별 영수증(미리보기 제외)에서 실측(없으면 생략)
      // [재편 B 1차 blocker⑥ ab-1] 영수증 서랍은 전 프로젝트 합본 — 이 프로젝트(wsKey) 행만 채택.
      // wsKey 미상 행·판독 불가=표시 생략(타 프로젝트 실적 오귀속 금지가 표시보다 우선).
      if (typeof CL9.readSelectorUsage === "function" && typeof CL9.wsKeyFor === "function") {
        const myKey9 = String(CL9.wsKeyFor(ws));
        const us9 = CL9.readSelectorUsage() || [];
        for (let i = us9.length - 1; i >= 0; i--) { const r = us9[i]; if (r && r.purpose !== "preview" && String(r.wsKey || "") === myKey9 && Array.isArray(r.selectedIds)) { ride9 = tE(`지난 검증에서 관련 수칙 ${r.selectedIds.length}개가 함께 실렸어요(영수증 실측)`, `last verification carried ${r.selectedIds.length} relevant rule(s) (from the receipt)`); break; } }
      }
    } catch { ride9 = ""; }
...
function draftSummaryDetail(CLA: any, repo: string, pr: any, en: boolean): string {
  ...
  const lines: string[] = [];
  if (d) {
    const dest = d.target === "archive" ? (en ? "rules applied when relevant" : "관련될 때 적용되는 수칙") : (en ? "rules always applied" : "항상 적용되는 수칙"); // [재편 B] 어휘 — 서고/코어는 내부어
...
<div class="arrow">→</div>
${card("#9a6cdc", "⚡", "영향지도", "Impact map", [[tE("무엇", "what"), tE("이 변경이 어디까지 번질지 미리보기(확인 목록)", "a preview/checklist of how far the change reaches")], [tE("누가", "who"), tE("정찰 LLM — 직접 또는 자동 지시로 실행", "scout LLM — run directly or via auto-directive")], [tE("비용", "cost"), tE("Claude 정찰=별도 과금 없음(쓰시던 Claude 사용량 범위) · DeepSeek 정찰은 키 등록 시(=동의) · Codex 정찰은 쓰시는 Codex 계정 사용량 범위 — 기본 담당은 모드 따라(클로드-코덱스=Claude · 코덱스-코덱스=Codex)", "Claude scout = no separate billing (within the Claude usage you already have) · DeepSeek scout only with a key (=consent) · Codex scout within your existing Codex account usage — the default follows the mode (Claude↔Codex = Claude · Codex↔Codex = Codex)")], [tE("저장", "store"), tE("보관함(최근 30장) → 영향지도 카드", "archive (last 30) → impact-map card")]])}
<div class="arrow">→</div>
...
<div class="vrel">${tE("<b>검증(2트랙)과의 관계</b> — 구현 담당(모드에 따라 Claude 또는 구현 Codex)이 코드를 바꾸면 검증 Codex가 검증합니다(기존 2트랙). 정찰은 그 앞뒤에 붙습니다: 바꾸기 전 ②가 '어디를 확인해야 하나'를 주고, 검증이 끝나면 그 결과가 ③에 자동으로 쌓여 다음 ②가 더 똑똑해집니다. 검증이 지켜야 할 경계(⑤ 수칙서)는 당신의 승인 도장으로만 바뀌고, 코어는 항상·서고는 관련분만 다음 검증에 실립니다.", "<b>Relation to verification (2-track)</b> — the implementer (Claude, or an implementer Codex, depending on the mode) changes code and a verifier Codex verifies it (the existing 2 tracks). Recon wraps around it: before a change, ② tells you what to check; after verification, results accrue into ③ so the next ② gets smarter. The bounds verification must respect (⑤ rulebook) change only with your approval stamp; the core rides along always, the archive only when relevant.")}</div>
...
<div class="sub">${tE("실선=자동 흐름 · 점선=피드백(다음 정찰·검증이 더 똑똑해지는 경로) · ⚙ 자동(AI 없음) · ⚡ AI 호출 · 👤 사람 선택", "solid = automatic flow · dashed = feedback (how the next recon/verification gets smarter) · ⚙ auto (no AI) · ⚡ AI call · 👤 human optional")}</div>
<svg viewBox="0 0 960 500" style="width:100%;max-width:960px;border:1px solid var(--vscode-panel-border);border-radius:8px;background:var(--vscode-editorWidget-background);margin:8px 0" xmlns="http://www.w3.org/2000/svg">
  ...
  <rect class="bx" x="542" y="22" width="160" height="68" stroke="#9a6cdc"/><text class="t1" x="554" y="42">${tE("⚡ ② 정찰 AI", "⚡ ② Scout AI")}</text><text class="t2" x="554" y="58">${tE("기본=모드별(Claude/Codex)", "default: per mode")}</text><text class="t2" x="554" y="72">${tE("셋 중 선택 · DeepSeek=키", "any of 3 · DeepSeek: key")}</text>
  ...
  <rect class="bx" x="868" y="30" width="72" height="52" stroke="#7f8c9b"/><text class="t1" x="878" y="50">${tE("👁 게시판", "👁 Board")}</text><text class="t2" x="878" y="66">${tE("열람 전용", "read-only")}</text>
  <line class="ln" x1="134" y1="56" x2="166" y2="56" marker-end="url(#ah)"/><line class="ln" x1="310" y1="56" x2="342" y2="56" marker-end="url(#ah)"/><line class="ln" x1="506" y1="56" x2="538" y2="56" marker-end="url(#ah)"/><line class="ln" x1="702" y1="56" x2="724" y2="56" marker-end="url(#ah)"/><line class="ln" x1="848" y1="56" x2="864" y2="56" marker-end="url(#ah)"/>
  <rect class="bx" x="60" y="150" width="250" height="56" stroke="#d97a7a"/><text class="t1" x="72" y="170">${tE("🚧 플랜 게이트", "🚧 Plan gate")}</text><text class="t2" x="72" y="186">${tE("플랜 확정 전 지도 신선도 확인(3트랙 기본 켜짐)", "map freshness before plan exit (3-track default on)")}</text><text class="t2" x="72" y="199">${tE("낡으면 '지도부터' 안내(2회 상한) · 미준비면 통과", "stale: 'map first' (max 2) · passes if scout not ready")}</text>
  <path class="ln" d="M 74 82 C 74 110 80 130 100 150" marker-end="url(#ah)"/>
  <path class="ln fb" d="M 150 150 C 150 122 132 100 120 86" marker-end="url(#ah)"/><text class="lb" x="156" y="120">${tE("낡음 → 정찰 재요청", "stale → re-run recon")}</text>
  <rect class="bx" x="366" y="150" width="230" height="56" stroke="#2f6fb3"/><text class="t1" x="378" y="170">${tE("🔍 Codex 검증 ⚡ (기존 2트랙)", "🔍 Codex verify ⚡ (2-track)")}</text><text class="t2" x="378" y="186">${tE("관련 항목 우선(없으면 기본 동봉+고지)", "relevant first (else default attach + notice)")}</text><text class="t2" x="378" y="199">${tE("통과+실제 파일 인용 → '확인' 신호", "pass + real file citations → 'confirm' signal")}</text>
  <path class="ln" d="M 728 76 L 714 76 L 714 100 L 560 100 L 560 148" marker-end="url(#ah)"/><text class="lb" x="566" y="116">${tE("동봉", "attach")}</text>
  <rect class="bx" x="700" y="116" width="240" height="80" stroke="#c98a2b"/><text class="t1" x="712" y="136">${tE("📜 ⑤ 수칙서(검증 경계) 👤", "📜 ⑤ Rulebook (verify bounds) 👤")}</text><text class="t2" x="712" y="152">${tE("후보: 정정 원문·검증 답·직접 입력·정리 담당", "from corrections·verdicts·manual·curator")}</text><text class="t2" x="712" y="166">${tE("→ 병합 초안 → 👤 승인(코어·서고 두 도장)", "→ merged draft → 👤 approve (2 stamps)")}</text><text class="t2" x="712" y="180">${tE("코어=항상 · 서고=관련분만 → 검증에 동봉", "core: always · archive: relevant → verify")}</text>
  <line class="ln" x1="696" y1="160" x2="600" y2="160" marker-end="url(#ah)"/><text class="lb" x="612" y="156">${tE("수칙 동봉", "rules attach")}</text>
  <line class="ln" x1="600" y1="184" x2="696" y2="184" marker-end="url(#ah)"/><text class="lb" x="606" y="197">${tE("답에서 후보 회수", "harvest candidates")}</text>
  <rect class="bx" x="400" y="226" width="160" height="52" stroke="#7f8c9b"/><text class="t1" x="412" y="246">${tE("🗣 당신의 정정", "🗣 Your correction")}</text><text class="t2" x="412" y="262">${tE("버튼·대행 기록 → 일지", "button · CLI note → journal")}</text><text class="t2" x="412" y="274">${tE("정정 원문 → 수칙서 후보", "quote → rulebook candidate")}</text>
  <line class="ln" x1="480" y1="278" x2="480" y2="296" marker-end="url(#ah)"/>
  <line class="ln" x1="580" y1="206" x2="580" y2="296" marker-end="url(#ah)"/><text class="lb" x="588" y="240">${tE("확인/반박", "confirm/refute")}</text>
  <rect class="bx" x="360" y="300" width="400" height="88" stroke="#3ca89a"/><text class="t1" x="372" y="320">${tE("📔 ③ 관찰 일지 — 자동 기억(이 PC)", "📔 ③ Field journal — auto memory (this PC)")}</text>
  <text class="t2" x="372" y="338">${tE("✚ 제안(지도가 발견) ▶ 동봉(자료에 실림) ✔ 확인(검증이 인정) ✖ 반박(틀림 판명)", "✚ proposed (map) ▶ attached (pack) ✔ confirmed (verify) ✖ disputed")}</text>
  <text class="t2" x="372" y="354">${tE("신분: 미검증(참고) → 검증됨(다음 정찰에 동봉) / 틀림 판명(제외 — 재확인 시 복권)", "states: unverified → verified (feeds scout) / disputed (excluded, may return)")}</text>
  <text class="t2" x="372" y="370">${tE("👤 개입(선택): 직접 확인·정정 / 고정·차단 — 안 눌러도 자동 운용", "👤 optional: confirm/correct / pin/ban — automatic when untouched")}</text>
  <path class="ln" d="M 840 82 L 840 100 L 952 100 L 952 344 L 764 344" marker-end="url(#ah)"/><text class="lb" text-anchor="end" x="946" y="330">${tE("발견을 제안으로", "findings → proposals")}</text>
  <rect class="bx" x="728" y="420" width="220" height="60" stroke="#d9a441"/><text class="t1" x="740" y="440">${tE("📕 ④ 확정 교범 👤", "📕 ④ Field manual 👤")}</text><text class="t2" x="740" y="456">${tE("도장 찍은 항목만 저장소 문서로", "only stamped items → repo doc")}</text><text class="t2" x="740" y="470">${tE("1회 도장·저절로 안 쌓임·수칙서와 별개", "one stamp · never piles up · ≠ rulebook")}</text>
  <path class="ln" d="M 740 388 C 745 400 750 410 760 418" marker-end="url(#ah)"/><text class="lb" x="766" y="404">${tE("👤 도장(선택)", "👤 stamp (optional)")}</text>
  <path class="ln fb" d="M 728 450 L 336 450 L 336 104 L 380 104 L 380 96" marker-end="url(#ah)"/><path class="ln fb" d="M 360 344 L 336 344"/><text class="lb" text-anchor="end" x="330" y="236">${tE("검증된 일지·확정 교범 → 다음 꾸러미", "verified journal · manual → next pack")}</text>
</svg>
<div class="sub">${tE("한눈 요약: 지도(⚡ 1회)가 발견을 내고 → 검증이 그 발견을 채점하고 → 일지가 스스로 기억하고(틀림도 반박 뒤 재확인이 쌓이면 복권) → 확실해진 것만 당신이 도장 찍어 문서로 남깁니다. 수칙서(⑤)는 여러 경로의 후보를 모아 당신의 승인 도장으로만 검증 경계가 됩니다. 사람 개입은 직접 확인·정정, 고정·차단·도장, 수칙 승인, 게이트 스위치와 원할 때의 정찰 태도 편집뿐이며 전부 선택입니다.", "In one line: the map (one ⚡ call) makes findings → verification grades them → the journal remembers by itself (even 'wrong' entries rehabilitate on later re-confirms) → you stamp only what's proven into a doc. The rulebook (⑤) gathers candidates from several routes and becomes verification bounds only with your approval stamp. Human touchpoints—confirm/correct, pin/ban/stamp, rule approval, the gate switch, and optional scout-attitude editing—are all optional.")}</div>
...
${tE("⚡ 표시가 붙은 단계들입니다 — ①영향지도 생성: 담당에 따라 — Claude 정찰은 별도 과금 없이 쓰시던 Claude로 실행되고(Claude 사용량 범위), DeepSeek 정찰은 키를 등록했을 때만, Codex 정찰은 쓰시는 Codex 계정 사용량 범위(기본 담당은 모드 따라 Claude 또는 Codex) ②Codex 검증(기존 2트랙의 본체): 검증을 보낼 때마다 쓰시는 Codex 계정 사용량 범위로 호출됩니다. ⚙ 단계들은 LLM 없이 돌고, 상태바 호버에 '지금 실행 중인 LLM 호출' 여부가 항상 표시됩니다.", "The steps marked ⚡ — ① map generation, by scout: the Claude scout adds no separate billing and runs on the Claude you already use (within your Claude usage); the DeepSeek scout only with a registered key; the Codex scout within your existing Codex account usage (the default scout follows the mode: Claude or Codex) ② Codex verification (the core of 2-track): every verification you send is a call within your existing Codex account usage. ⚙ steps run without LLM, and the status-bar hover always shows whether an LLM call is running.")}
...
function scoutActualText(ws: string | null): string {
  ...
  try {
    ...
    const armLabel = scoutNameExt(ws, best.arm === "deepseek" ? "deepseek" : best.arm === "codex" ? "codex" : "self"); // 담당 이름 정본(모드 기본 반영)
...
function computeState(turnsN: number): BridgeState {
  ...
  return {
    ...
    rulesFlow: (() => {
      // [HARNESS-STRUCTURE §B3 · D4] 수칙 흐름 4칸 — 순수 계산 모듈(bridge/rules-flow.js)이 장부·상수를 모아 준다. 개요 '지금 정할 것'엔 합산하지 않는다.
      try { if (!ws) return null; const RF9: any = require(path.join(BRIDGE_DIR, "rules-flow.js")); return typeof RF9.rulesFlowFor === "function" ? RF9.rulesFlowFor(ws, { lang: loadLangExt() }) : null; } catch { return null; }
    })(),
    ruleCheck: (() => {
    ...
    scoutNames: scoutNamesExt(ws), // 담당 표시명(정찰 on/off 무관 — 고급설정 키 안내 등 상시 표면이 쓴다)
    ...
    envelope: readEnvelopeView(ws), // 거버넌스 증분 1 — 검증 경계(정찰·MAP과 무관한 '검증' 계층: 2트랙 포함·매 푸시 신선 계산=승인 후 캐시 지연 없음)
  };
}
...
class Dashboard {
    ...
    if (!this.panel) {
      ...
      this.panel.webview.onDidReceiveMessage((m) => {
        ...
        if (m?.type === "envelopeApprove" && typeof m.repo === "string" && m.repo) { // 거버넌스 증분 1 — 수칙서 승인(지문 도장). 2차 정정: 렌더 언어 슬롯 결속(1차 blocker②)+항목 전문 모달(1차 blocker③)
          ...
          const en9 = apLang === "en";
          const wsE = dashboardWorkspace();
          const tgtE = wsE ? scoutTargetFor(wsE).repo : null;
          if (!tgtE) return;
          if (normWs(tgtE) !== normWs(m.repo)) { vscode.window.showWarningMessage(en9 ? "The scout target changed — the card refreshes. Please confirm again." : "정찰 대상이 바뀌었어요 — 카드가 갱신됩니다. 다시 확인해 주세요."); this.post(); return; }
          let evv: any = null;
          try { const CL9: any = require(path.join(BRIDGE_DIR, "contract-lib.js")); evv = typeof CL9.readVerifyEnvelope === "function" ? CL9.readVerifyEnvelope(tgtE) : null; } catch { evv = null; }
          if (!evv || evv.st !== "ok") { vscode.window.showWarningMessage(en9 ? "Cannot read the rulebook file — approval aborted." : "수칙서 파일을 읽을 수 없어 승인할 수 없어요."); this.post(); return; }
          if (evv.truncated === true) { // 2차 blocker①: 절삭 상태 승인 금지 — 모달에 '전부' 보이지 않는 내용의 원본 지문을 도장 찍는 경로 차단(안전 방향)
            vscode.window.showWarningMessage(en9 ? "Some rulebook items exceed the caps (12 per axis · 200 chars) and are cut off — approval is blocked because the modal cannot show the omitted content. Trim verify-envelope.json, then approve." : "수칙서 항목이 상한(축 12·항목 200자)을 넘어 일부가 잘립니다 — 잘려서 보이지 않는 내용까지 승인할 수는 없어요. verify-envelope.json을 상한 안으로 줄인 뒤 다시 승인해 주세요.");
            this.post(); return;
          }
          ...
          vscode.window.showInformationMessage(msg9, { modal: true, detail: detail9 }, okBtn).then((sel) => {
            ...
            if (!tgtNow || normWs(tgtNow) !== normWs(tgtE)) { vscode.window.showWarningMessage(en9 ? "The target changed during approval — not approved." : "승인하는 사이 정찰 대상이 바뀌어 승인하지 않았습니다."); this.post(); return; }
            let ev2: any = null;
        ...
        // [2026-08-29 사용자 결정] 대시보드 직접 입력(넣기 입력칸·ruleAdd)은 제거 — 수칙은 대화에서 한 약속의 포착과 구현 담당의 마감 상신 두 경로로만 제안되고,
        // 사용자는 승인/안 올림/빼기만 한다(자동 적용 체계에 수동 입력칸이 섞이면 "왜 있는지" 설명이 없어 혼란 — 실보고). 브릿지 directRuleCandidate·user-direct kind는 과거 레코드 호환용으로만 남는다.
        ...
        if (m?.type === "envelopeRevise" && Array.isArray(m.adds) && Array.isArray(m.removes) && typeof m.gen === "string" && m.gen && typeof m.wsKey === "string" && m.wsKey) {
          ...
          try {
            ...
            const rem9 = m.removes.filter((r: any) => r && typeof r.axis === "string" && ["supportedEnv", "alwaysBlocker", "outOfScope"].includes(r.axis) && Number.isInteger(r.index) && r.index >= 0).map((r: any) => ({ axis: r.axis, index: r.index, ...(typeof r.itemFp === "string" && /^[0-9a-f]{40}$/.test(r.itemFp) ? { itemFp: r.itemFp } : {}) })); // [재편 B §3-2] 행 지문 동봉 — 빌더가 잠금 안에서 대조(TOCTOU)
        ...
        if (m?.type === "proposalDiscard" && typeof m.repo === "string" && m.repo) { // 초안 폐기(2026-08-21 실보고) — 수칙서 무변·채택 후보 복원
          ...
          // [1차 blocker②] 클릭 시점 초안 지문 포착 — 모달이 떠 있는 사이 초안이 바뀌면 무접촉(오폐기 차단)
          let hashD = "";
          try { const CLD0: any = require(path.join(BRIDGE_DIR, "contract-lib.js")); const prD0 = typeof CLD0.readEnvelopeProposal === "function" ? CLD0.readEnvelopeProposal(wsD, tgtD) : null; if (prD0 && prD0.st === "ok") hashD = String(prD0.draftId || prD0.newHash || ""); } catch { hashD = ""; } // [2차 blocker①] 신원 토큰
          if (!hashD) { vscode.window.showWarningMessage(enD ? "Cannot read the draft — the card refreshes." : "초안을 읽을 수 없어요 — 카드가 갱신됩니다."); this.post(); return; }
          ...
          vscode.window.showInformationMessage(
            ...
            if (sel !== okD) return;
            let rD: any = null;
            try { const CLD: any = require(path.join(BRIDGE_DIR, "contract-lib.js")); rD = typeof CLD.discardEnvelopeProposalRestoring === "function" ? CLD.discardEnvelopeProposalRestoring(wsD, hashD) : { ok: false, restored: false, reason: "old-runtime" }; } catch { rD = { ok: false, restored: false }; }
            ...
            else if (rD && rD.reason === "draft-changed") vscode.window.showWarningMessage(enD ? "The draft changed while this dialog was open — nothing was discarded. The card refreshes." : "확인하는 사이 초안이 바뀌어 아무것도 폐기하지 않았어요 — 카드가 갱신됩니다.");
        ...
        if (m?.type === "saveContract") {
          ...
          if (ok && wantScoutOn && !prevScoutOn) {
            ...
            // 모드 기본 담당·준비 상태(문구 감사 2026-09-21): 'Claude 가 전부 동작' 단정 대신 이 프로젝트의 기본 담당 이름과 지금 준비 여부를 말한다.
            const wsK = dashboardWorkspace();
            const armK = wsK ? scoutArmViewExt(wsK) : null;
            const armNoKey: "self" | "codex" = armK ? armK.noKeyArm : "self"; // 키 없을 때 실제 실행될 담당(명시 self/codex 우선 · 4판 blocker — 실효 뷰가 계산)
            const defKo = scoutArmTextExt(wsK, armNoKey, "ko"), defEn = scoutArmTextExt(wsK, armNoKey, "en");
            const readyKo = armK && !armK.hasKey && armK.ready === false ? "(지금은 준비 안 됨 — 정찰 카드의 사유 참조)" : "", readyEn = armK && !armK.hasKey && armK.ready === false ? " (not ready right now — see the reason on the recon card)" : ""; // 준비 상태는 실효 담당 기준 — 키 없음(실효=armNoKey)일 때만 붙임
            if (!readDeepseekView().hasKey) {
              const go = tE("등록하러 가기", "Register key");
              vscode.window.showWarningMessage(tE(
                "3트랙(정찰)이 켜졌지만 등록된 DeepSeek API 키가 없어요.\n\n키 없이도 정찰은 " + defKo + "이 맡아요" + readyKo + " — '비교용 두 번째 정찰(DeepSeek)'만 잠겨 있어요. 그리고 어느 쪽이든 정찰이 한 번도 실행되지 않으면 3트랙의 효과가 미비할 수 있어요.\n\n상세는 정찰 카드의 '정찰 구조 자세히 보기 (새탭)'에서 확인하세요.",
                "3-track (recon) is on, but no DeepSeek API key is registered.\n\nWithout a key, recon is handled by " + defEn + readyEn + " — only the second comparison scout (DeepSeek) stays locked. And with either scout, if recon never actually runs, 3-track delivers little.\n\nSee 'Recon structure in detail (new tab)' on the recon card."),
              ...
              vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: tE("DeepSeek 연결 점검 중…", "Checking DeepSeek connection…") }, () => new Promise<void>((resolve) => {
                try {
                  ...
                  if (!tok) { vscode.window.showWarningMessage(tE("연결 점검을 실행할 node를 찾지 못했어요 — 키는 등록돼 있고, " + defKo + "은 영향받지 않아요.", "Couldn't find node to run the check — the key is registered and " + defEn + " is unaffected.")); resolve(); return; }
                  ...
                  p.on("close", (code) => {
                    ...
                    else vscode.window.showWarningMessage(tE("키는 등록돼 있지만 연결 점검에 실패했어요(" + (err || out || "응답 없음").trim().slice(0, 120) + "). " + defKo + "은 영향받지 않아요 — 키/네트워크를 확인하세요(⚙️ 고급설정).", "The key is registered but the connection check failed (" + (err || out || "no response").trim().slice(0, 120) + "). " + defEn + " is unaffected — check the key/network (⚙️ Advanced)."));
                    resolve();
                  });
                  p.on("error", () => { vscode.window.showWarningMessage(tE("연결 점검 실행 실패 — 키는 등록돼 있고, " + defKo + "은 영향받지 않아요.", "Failed to run the check — the key is registered and " + defEn + " is unaffected.")); resolve(); });
                } catch { resolve(); }
              }));
            }
          }
  ...
  runProposalApprove(repoMsg: string, langMsg: unknown, autoDiscardOnCancel: boolean): void {
    const m = { repo: repoMsg, lang: langMsg } as { repo: string; lang: unknown };
          ...
          const draftTokA = String(prA.draftId || prA.newHash); // [2차 blocker①] 폐기 결속=wrapper 신원(legacy=newHash)
          const okA = enA ? "Reviewed — approve & apply" : "확인했고 승인해 적용합니다"; // [재편 B] 어휘 4종(도장→승인)
          const msgA = enA
            ? "Approve this rules change — existing rules stay as they are; only the additions/removals in the summary below apply (until you approve, nothing changes). Disagree? Cancel — nothing changes, and you can tell the implementer what to reword."
            : "수칙 변경 승인 — 기존 수칙은 그대로 있고, 아래 요약의 추가·빼기만 반영됩니다(승인 전까지는 아무것도 바뀌지 않아요). 동의하지 않으면 취소하세요 — 취소해도 아무것도 바뀌지 않고, 바꿀 점은 구현 담당에게 말씀하시면 됩니다."; // [재편 B] 어휘 4종(수칙·승인·빼기 — 도장·초안·서고는 내부어)
          vscode.window.showInformationMessage(msgA, { modal: true, detail: draftSummaryDetail(CLA, tgtA, prA, enA) }, okA).then((sel) => { // [기억 권위 A-4] note 경고 노출·절단 금지(전문은 요약 아래 감사용으로 유지 — 재검증 blocker②) · [4c UX] 요약 선행
            if (sel !== okA) {
              // [재편 B §3-4] 1클릭 체인의 취소=복원형 폐기(초안 잔존 금지 — 복원 실패 시엔 초안 보존이 계약이라 재시도 안내)
              if (autoDiscardOnCancel) {
                try { const dR = typeof CLA.discardEnvelopeProposalRestoring === "function" ? CLA.discardEnvelopeProposalRestoring(wsA, draftTokA) : null; // [1차 blocker②→2차] 내가 연 초안 신원(draftId) 결속 — 다른 초안이면 무접촉
                  if (dR && dR.ok) vscode.window.showInformationMessage(enA ? "Cancelled — nothing changed (the proposal went back to the list)." : "취소했어요 — 아무것도 바뀌지 않았고, 제안은 목록으로 돌아갔습니다.");
                  else vscode.window.showWarningMessage(enA ? "Cancel could not fully clean up (" + ((dR && dR.reason) || "unknown") + ") — the draft is kept; use the draft card to retry." : "취소 정리가 완료되지 않았어요(" + ((dR && dR.reason) || "unknown") + ") — 초안은 보존됐고, 초안 카드에서 이어서 처리할 수 있어요."); } catch { /* 보존 우선 — 초안 카드가 복구 표면 */ }
                this.post();
              }
              return;
            }
            ...
            if (tr && tr.ok) vscode.window.showInformationMessage(enA ? "Approved — the change applies from the next verification." : "승인 완료 — 다음 검증부터 적용됩니다."); // [재편 B] 어휘 4종
...
<style>
  ...
  .rf-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
  .rf-cell{padding:8px;border-radius:6px;background:var(--vscode-editorWidget-background);min-width:0}
  .rf-title{font-weight:600;font-size:12px;margin-bottom:3px}
  .rf-body{font-size:12px;white-space:normal;word-break:break-word}
  .rf-body .rf-num{font-weight:700}
  ...
  .hfgrid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:6px 0 4px}
  .hfcol{border:1px solid var(--vscode-panel-border);border-left:4px solid var(--hfa,var(--vscode-charts-green));border-radius:8px;padding:8px 10px;background:color-mix(in srgb,var(--hfa,var(--vscode-charts-green)) 6%,transparent);font-size:11.5px;line-height:1.5}
  .hfcol.core{--hfa:var(--vscode-charts-orange)}
  .hfcol .hfhead{font-weight:700;color:var(--hfa);margin-bottom:2px}
  .ftag{display:inline-block;padding:0 6px;border-radius:4px;font-size:10px;font-weight:700;border:1px solid currentColor;margin:0 3px 0 6px}
  .ftag.warn{color:var(--vscode-charts-orange)} .ftag.note{color:var(--vscode-charts-yellow,#d7ba7d)} .ftag.bl{color:var(--vscode-descriptionForeground)}
  details.hintfold{margin:3px 0 6px} details.hintfold summary{cursor:pointer;font-size:11px;color:var(--vscode-textLink-foreground);list-style:none;user-select:none}
  details.hintfold summary::before{content:"▸ "} details.hintfold[open] summary::before{content:"▾ "}
...
<main class="shell">
  ...
  <h2 class="sec base">${t("수칙 흐름", "Rules flow")} <span class="sub2">${t("항상 4칸 — 항상 적용 · 관련될 때만 · 제안함 · 정리 담당. 승인·편집은 '검증 설정'", "always 4 cells — always-on · when relevant · proposals · curator. Approve/edit in Verify Settings")}</span></h2>
  <div class="card" id="ovRulesFlow">
    <div class="rf-grid">
      <div class="rf-cell" id="rfCore"><div class="rf-title">${t("항상 적용되는 수칙", "Rules always applied")}</div><div class="rf-body muted">–</div></div>
      <div class="rf-cell" id="rfArchive"><div class="rf-title">${t("관련될 때만 적용되는 수칙", "Rules applied when relevant")}</div><div class="rf-body muted">–</div></div>
      <div class="rf-cell" id="rfProposals"><div class="rf-title">${t("제안함", "Proposals")}</div><div class="rf-body muted">–</div></div>
      <div class="rf-cell" id="rfCurator"><div class="rf-title">${t("정리 담당", "Curator")}</div><div class="rf-body muted">–</div></div>
    </div>
    <div class="muted" style="font-size:11px;margin-top:6px"><button type="button" class="secondary" id="rfGoSetup" style="font-size:12px">${t("승인·편집으로 이동", "Go to approve / edit")}</button></div>
  </div>
  </div><!-- /tab-overview -->
  ...
  <div class="card">
    ...
    <div class="hfgrid">
      <div class="hfcol"><div class="hfhead">${t("무결성 — 기본", "Integrity — default")}</div>${t("관련 호출부·경합·회귀·문서까지 넓게 탐색", "Searches widely across call sites, races, regressions, and docs")}<div class="muted">${t("모든 결함 부재를 보증하지는 않음", "does not guarantee absence of all defects")}</div></div>
      <div class="hfcol core"><div class="hfhead">${t("핵심 — 직접 영향 중심", "Core — direct impact")}</div>${t("선언된 요구·직접 영향 중심으로 좁게·빠르게", "Focuses narrowly and quickly on the declared requirements and their direct impact")}<div class="muted">${t("범위 밖 잔여 위험이 남을 수 있고, 자동수정 차단은 없음 — 기계적 왕복 상한은 아래 '검증 왕복 상한'(기본 무제한)에서 별도 설정", "residual out-of-scope risk may remain, with no auto-fix blocking — a mechanical round-trip cap is available separately via 'Verification round cap' below (unlimited by default)")}</div></div>
    </div>
    <div class="hint">${t("핵심의 비차단 지적 세 갈래(v2.4):", "Core's three non-blocking finding lanes (v2.4):")} <span class="ftag warn">${t("[주의]", "[caution]")}</span>${t("보안·데이터 인접 — 재판단(지금 고침/승격해 보관함)", "security/data-adjacent — re-judged (fix now / escalate & park)")} <span class="ftag note">${t("[보완]", "[notes]")}</span>${t("자명·국소 — 그 루프에서 일괄+확인 1회", "obvious & local — batched in that loop + one confirmation")} <span class="ftag bl">${t("[백로그]", "[backlog]")}</span>${t("범위 밖 제안 — 보관함·갚을 의무 없음", "out-of-scope — parked, no repayment duty")}</div>
    <details class="hintfold"><summary>${t("ⓘ 전환하면 언제부터 적용되나", "ⓘ When does switching take effect")}</summary><div class="hint">${t("프로필·언어 선택은 이후 시작되는 검증(ask)부터 즉시 적용됩니다 — 이미 진행 중인 턴의 주입 지침은 바뀌지 않아 한 턴 안에서 규약이 섞일 수 있으니, 턴 전체 일관성이 필요하면 다음 프롬프트를 보내기 전에 전환하세요. 핵심으로 작업했다면 push·배포 전에 무결성 승격 검증 1회를 권장합니다.", "Profile/language choices apply immediately to verifications (asks) started afterwards — directives already injected into the current turn do not change, so protocols can mix within one turn; switch before your next prompt if you need whole-turn consistency. If you worked in Core, one Integrity escalation verification before push/deploy is recommended.")}</div></details>
    ...
    <label class="ck verify">${t("검증 왕복 상한 — 한 번의 지시(사용자 턴)당 검증을 몇 번까지 돌지", "Verification round cap — how many verify round-trips per instruction (user turn)")}
      <input id="vBudget" type="number" min="0" step="1" style="width:76px" placeholder="${t("무제한", "unlimited")}">
      <span class="muted" id="vBudgetNote" style="font-size:11.5px"></span>
    </label>
  ...
  <section id="tab-adv" class="tab-panel">
    ...
    <div class="card">
      <div class="chead">${t("DeepSeek API 키", "DeepSeek API key")} <span class="muted" style="font-weight:400">${t("· 3트랙의 'DeepSeek 비교 정찰'(두 번째 정찰자)에만 필요 — 키 없이도 변경 감지와 모드 기본 담당(클로드-코덱스=Claude 정찰·별도 과금 없음 / 코덱스-코덱스=Codex 정찰)의 영향지도는 동작해요", "· only needed for 3-track's DeepSeek comparison scout (the second scout) — change sensing and impact maps by the mode's default scout (Claude↔Codex = Claude scout, no separate billing / Codex↔Codex = Codex scout) work without it")}</span></div>
      <div class="hint">${t("키는 이 컴퓨터의 브릿지 홈(<code>~/.codex-bridge/deepseek.json</code>)에만 저장되고 저장소(GitHub)에는 절대 들어가지 않아요. 자료 꾸러미 전송은 <b>지도 생성이 실행될 때만</b> 일어나요 — 당신이 직접 실행하거나, <b>키 등록=동의 모델</b>에 따라 3트랙 자동 지시를 받은 구현 담당(운용 모드에 따라 Claude 또는 구현 Codex)이 실행(조건: 지도가 없거나 낡았을 때 그 상태에 1회 지시 — 확장·훅 자체는 전송하지 않음). 그 외 외부 요청은 3트랙을 켤 때의 <b>연결 점검 1회</b>(꾸러미 아님 — 접속 확인용)뿐이에요. 대시보드를 보는 것만으로는 전송 없음. 발동 조건과 전송·제외 내용은 PRIVACY에 명시돼 있어요.", "The key is stored only in this machine's bridge home (<code>~/.codex-bridge/deepseek.json</code>) and never enters the repo (GitHub). The evidence package is sent <b>only when map generation runs</b> — you run it directly, or under the <b>key-registration=consent model</b> the implementer of the current mode (Claude, or an implementer Codex) runs it on the 3-track auto-directive (issued once per missing/stale-map state — the extension/hooks themselves never transmit). The only other external request is a single <b>connection check</b> when 3-track is switched on (not a package — reachability only). Viewing the dashboard sends nothing. Trigger conditions and what is sent/excluded are documented in PRIVACY.")}</div>
  ...
  <details class="card baseline" id="baseDetails" style="margin-top:10px">
    ...
    <div id="bScoutWrap" style="display:none">
      <div class="chead" style="margin-top:12px">${t("④ 정찰 기본 원칙", "④ Scout baseline")} <span class="muted" style="font-weight:400">${t("→ 정찰 AI에게 · 지도를 그리기 직전 · 3트랙일 때만 (Claude 정찰·DeepSeek 정찰·Codex 정찰 공통)", "→ to the scout AI · right before drawing a map · 3-track only (all scouts)")}</span> <span id="bScoutOv" class="muted" style="font-weight:400"></span></div>
...
<script nonce="${nonce}">
  ...
  var scoutNames9 = null; // 정찰 담당 표시명 묶음(호스트가 브릿지 정본 scoutArmNames 로 계산 — 문구 감사 2026-09-21). 화면은 이름을 짓지 않고 표시 언어로 고른다.
  ...
  try { var rfg=$("rfGoSetup"); if(rfg) rfg.addEventListener("click", function(){ var t0=$("envCard"); if(t0 && t0.style.display!=="none"){ gotoEl(t0); } else { var tb=document.querySelector('.tabbtn[data-tab="setup"]'); if(tb) tb.click(); } }); } catch(e){ /* 무해 */ }
  ...
  function cardInputLock(on){
    ...
    var vb=$("vBudget"); if(vb) vb.disabled = on; // P-12 2b(구현검증 1차 B5): 저장 중 예산 입력 잠금 — DOM 값과 저장값 분기 차단
  }
  // 카드 미저장 초안 종합(P-10: 규칙 textarea+세그 초안 — 체크박스는 즉시 저장이라 초안 없음)
  function cardDirtyNow(){
    var seg = (appVM!==null && curVM!==appVM) || (appIM!==null && curIM!==appIM) || (appSM!==null && curSM!==appSM) || (appVP!==null && curVP!==appVP) || (appVB!==null && curVB!==appVB);
  ...
  // [HARNESS-STRUCTURE §B3 · D4] 수칙 흐름 4칸 — 항상 그린다(빈 상태는 '왜 비었는지' 문장). 자료=d.rulesFlow(브릿지 순수 계산). 교차 패널 이동은 gotoEl 단일 경로.
  function rfWhen(ts){ var dt=new Date(ts||""); if(isNaN(dt.getTime())) return ""; return (dt.getMonth()+1)+"/"+dt.getDate()+" "+String(dt.getHours()).padStart(2,"0")+":"+String(dt.getMinutes()).padStart(2,"0"); }
  function rfSet(id, lines, fold){ var c=$(id); if(!c) return; var b=c.querySelector(".rf-body"); if(!b) return; b.replaceChildren(); b.classList.remove("muted");
    (lines||[]).forEach(function(ln){ var d9=el("div","ovline"); if(typeof ln==="string") d9.textContent=ln; else { var n9=el("span","rf-num",String(ln.num)); d9.appendChild(n9); d9.appendChild(document.createTextNode(" "+ln.text)); } b.appendChild(d9); });
    if(fold&&fold.items&&fold.items.length){ var dt=document.createElement("details"); var sm=document.createElement("summary"); sm.style.cssText="cursor:pointer;font-size:11px"; sm.textContent=fold.summary; dt.appendChild(sm);
      fold.items.forEach(function(it){ var li=el("div","ovline muted"); li.style.fontSize="11px"; li.textContent="· "+(it.op?"["+it.op+"] ":"")+(it.title||it.id)+(it.status?" — "+it.status:""); dt.appendChild(li); }); b.appendChild(dt); } }
  function renderRulesFlow(d){
    var rf=d.rulesFlow; var card=$("ovRulesFlow"); if(!card) return;
    if(!rf){ ["rfCore","rfArchive","rfProposals","rfCurator"].forEach(function(id){ rfSet(id,[T("자료 없음 — 브릿지 장부를 읽지 못했어요(설치본 갱신·폴더 확인)","no data — bridge ledgers unreadable (check install / folder)")]); }); return; }
    var c=rf.core, a=rf.archive, p=rf.proposals, u=rf.curator;
    var coreL=[];
    if(c.st==="absent") coreL.push(T("아직 없음 — 검증 설정에서 첫 수칙을 승인하면 생겨요","none yet — approve the first rules in Verify Settings"));
    else { coreL.push({num:c.count.total, text:T("항 (전제 "+c.count.supportedEnv+" · 금지 "+c.count.alwaysBlocker+" · 제외 "+c.count.outOfScope+")","items (env "+c.count.supportedEnv+" · blockers "+c.count.alwaysBlocker+" · out-of-scope "+c.count.outOfScope+")")});
      if(c.st==="ok") coreL.push({num:c.injectedChars, text:T("글자가 검증 요청마다 실려요(비용 근거)","chars ride on every verify request (cost basis)")});
      else coreL.push(T(c.st==="unapproved"?"승인 전이라 아직 실리지 않아요":c.st==="mismatch"?"승인 없이 바뀌어 실리지 않아요 — 검증 설정에서 승인":"파일 손상 — 검증 설정에서 다시 저장", c.st==="unapproved"?"not applied yet — approval pending":c.st==="mismatch"?"changed without approval — not applied; approve in Verify Settings":"file corrupt — re-save in Verify Settings")); }
    rfSet("rfCore", coreL);
    var arcL=[];
    if(a.state==="active"){ if(a.count===0) arcL.push(T("관련될 때만 적용되는 수칙 0항 · 정리 담당이 제안하면 채워집니다","0 items — filled when the curator proposes")); else arcL.push({num:a.count+"/"+a.max, text:T("항 보관 · 검증마다 관련분만 골라 실어요(최대 "+a.selectCap.items+"항 · "+a.selectCap.bytes+"바이트)","stored · only relevant ones ride each verification (max "+a.selectCap.items+" items · "+a.selectCap.bytes+" bytes)")}); }
    else if(a.state==="none") arcL.push(T("관련될 때만 적용되는 수칙 0항 · 정리 담당이 제안하면 채워집니다","0 items — filled when the curator proposes"));
    else if(a.state==="broken") arcL.push(T("승인 없이 보관 수칙 파일이 바뀌어 검증이 시작되지 않아요 — 검증 설정에서 확인","stored rules changed without approval — verification blocked; check Verify Settings"));
    else arcL.push(T("도장 없는 보관 파일 — 검증 설정에서 승인하거나 정리","stored file without approval stamp — approve or clean up in Verify Settings"));
    if(a.lastSelection) arcL.push(T("마지막 선별 "+rfWhen(a.lastSelection.ts)+" · "+a.lastSelection.selectedCount+"항 실림","last selection "+rfWhen(a.lastSelection.ts)+" · "+a.lastSelection.selectedCount+" items rode"));
    else if(a.state==="active"&&a.count>0) arcL.push(T("이 저장소의 선별 기록 아직 없음 — 다음 검증부터","no selection record for this repository yet — from the next verification"));
    rfSet("rfArchive", arcL);
    var prL=[]; var nc=p.nextCheck;
    if(p.pending>0) prL.push({num:p.pending+"/"+p.max, text:T("건 승인 대기 — 승인·안 올림은 검증 설정의 제안함","pending — approve / decline in the proposal box (Verify Settings)")});
    else prL.push(T("지금 제안 없음","no proposals right now"));
    prL.push(T("다음 검사: 훅 "+nc.tickK+"회마다(지금 "+nc.tickCount+"회) · "+nc.unusedDays+"일 미사용 관측"+(nc.lastTickTs?" · 마지막 검사 "+rfWhen(nc.lastTickTs):" · 아직 검사 없음"),"next check: every "+nc.tickK+" hook ticks (now "+nc.tickCount+") · "+nc.unusedDays+"-day unused watch"+(nc.lastTickTs?" · last check "+rfWhen(nc.lastTickTs):" · no check yet")));
    rfSet("rfProposals", prL);
    var cuL=[];
    if(u.running) cuL.push(T("정리 제안 실행 중","curation running"));
    if(u.lastTs){ var trg=u.lastTrigger==="auto"?T("자동","auto"):u.lastTrigger==="button"?T("버튼","button"):T("수동","manual"); cuL.push(T("마지막 실행 "+rfWhen(u.lastTs)+" · "+trg+" · "+u.lastOutcome+(u.lastReason?" ("+u.lastReason+")":""),"last run "+rfWhen(u.lastTs)+" · "+trg+" · "+u.lastOutcome+(u.lastReason?" ("+u.lastReason+")":""))); }
    else cuL.push(T("아직 실행 없음 — 장부 재료가 쌓이면 스스로 돌아요(비용 없음)","no run yet — runs by itself once ledger material accumulates (no cost)"));
    var m=u.metrics||{}; cuL.push(T("측정: 채택률 "+m.adoptRate+"% (제안 "+m.proposedTotal+" · 채택 "+m.adopted+" · 안 올림 "+m.declined+") · 서고 "+m.archiveSize+"항 · 선별 초과 "+m.selOverCount+"회 · 미사용 "+m.unusedCount+"항","measures: adoption "+m.adoptRate+"% (proposed "+m.proposedTotal+" · adopted "+m.adopted+" · declined "+m.declined+") · stored "+m.archiveSize+" · selection over-range "+m.selOverCount+" · unused "+m.unusedCount));
    rfSet("rfCurator", cuL, u.lastItems&&u.lastItems.length?{summary:T("마지막 제안 "+u.lastItems.length+"건 보기","show last "+u.lastItems.length+" proposal(s)"), items:u.lastItems}:(u.lastTs?{summary:T("마지막 실행의 제안 없음","last run produced no proposals"), items:[]}:null));
  }
  ...
  function renderStats(vs,mapOn){
    ...
    var rows = Object.keys(bp).map(function(k){
      ...
      return '<div class="vrow"><span class="vlbl">'+(profLbl[k]||k)+'</span><b class="vnum">'+b.count+T("건"," runs")+'</b><span>'
        ...
        + chip(T("범위 밖이라 메모로 낮춤","out-of-scope→note"), b.demoted, "note") + chip(T("기계 정정","corrected"), b.corrected, "note")
  ...
  window.addEventListener("message", (ev) => {
    ...
    safeTop(function(){
      ...
      scoutNames9 = d.scoutNames || null; // 담당 이름은 매 상태와 함께 갱신(모드 전환 반영)
      ...
      if (d.contract && (!renderedLangC || !d.lang || d.lang === renderedLangC)) {
        ...
        // P-12 2b 표시 규칙(1차 B3): C-C=원시 슬롯 값 그대로(null=""·0="0"=명시 무제한), CL-C=유한값만(무제한="").
        var preVB = ccR ? (d.contract.codexVerifyBudgetRawVal===null ? "" : String(d.contract.codexVerifyBudgetRawVal))
                        : (d.contract.verifyBudget>=1 ? String(d.contract.verifyBudget) : "");
        ...
        if (appVB!==null && curVB!==appVB && curVB===preVB) appVB = curVB;
      }
    ...
    const segDirtyC = (appVM!==null && curVM!==appVM) || (appIM!==null && curIM!==appIM) || (appSM!==null && curSM!==appSM) || (appVP!==null && curVP!==appVP) || (appVB!==null && curVB!==appVB);
    ...
    if (d.contract && !holdC){
      ...
      const pVM=appVM, pIM=appIM, pSM=appSM, pVP=appVP, pVB=appVB; // pVP/pVB: P-12 — 초안이 상태 푸시에 안 되돌아가게
      ...
      appVB = ccMode ? (d.contract.codexVerifyBudgetRawVal===null ? "" : String(d.contract.codexVerifyBudgetRawVal))
      ...
      safe(function(){ var ec=$("envCard"); if(!ec) return; var e9=d.envelope; ec.innerHTML=""; if(!e9){ ec.style.display="none"; return; } ec.style.display="block";
        ...
        // [§B3 · D4] 정리 담당 줄·'마지막 정리 결과 보기' 버튼은 개요 패널 '수칙 흐름' 카드(정리 담당 칸 인라인 접기)로 옮겼다 — 설정 탭은 승인·편집만.
        ...
        if(e9.cands && e9.cands.length){ // [재편 B §3-1] 제안함 — 장부 상신(구현 담당 마감 판단·사용자 약속)만. 어휘=제안·승인·안 올림.
          var ch9=document.createElement("div"); ch9.style.cssText="margin-top:8px;font-weight:600"; ch9.setAttribute("data-cands-box","1"); ch9.textContent=T("제안 — "+e9.cands.length+"건 (승인해야만 효력이 생겨요)","Proposals — "+e9.cands.length+" (effective only after you approve)"); ec.appendChild(ch9);
          ...
          chSub9.textContent=viewOnly9
            ...
            :T("대화에서 하신 약속을 시스템이 포착한 것과, 구현 담당이 '왜 지킬 규칙인지'와 함께 올린 제안만 여기 옵니다. [승인]을 누르면 바뀌는 내용을 확인한 뒤 승인 1번으로 적용돼요.","Only promises captured from the conversation and proposals from the implementer (with a reason) appear here. Press [Approve] to review the change, then one approval applies it."); ec.appendChild(chSub9);
          ...
          e9.cands.forEach(function(cd, ix9){
            ...
            if(undecided9 && !viewOnly9){
              ...
              row9.appendChild(apb9);
              var deb9=document.createElement("button"); deb9.className="secondary"; deb9.style.cssText="margin-top:5px;margin-right:5px;font-size:12px"; deb9.textContent=T("안 올림","Not this one");
              deb9.onclick=function(){ vscode.postMessage({type:"candMark", id: cd.id, kind: cd.kind, status: "declined", gen: cd.gen, wsKey: cd.wsKey, lang: e9.lang}); };
              row9.appendChild(deb9);
            } // 초안 대기 중=버튼 없음(열람만)
            if(ix9>=8 && !candsMoreOpenWeb){ row9.style.display="none"; row9.setAttribute("data-candmore","1"); more9=more9||0; more9++; }
            ec.appendChild(row9);
          });
        ...
        if(e9.signals && e9.signals.length){ // [재편 B §3-1b] 반복 신호=참고 표시 전용(버튼·처분·판단 의무 없음)
          ...
          e9.signals.forEach(function(sg){ var ln9=document.createElement("div"); ln9.className="muted"; ln9.style.cssText="font-size:11px;margin-top:3px";
            ...
            ln9.textContent="· "+sk9+" ×"+(sg.n||1)+(sg.title?" — "+String(sg.title).slice(0,60):""); ln9.title=String(sg.title||""); sg9.appendChild(ln9); });
        ...
        if(e9.rules && e9.proposal===undefined){ // rules 빈 배열도 렌더 — 수칙 0개여도 목록 틀(요약 줄)을 보여 상태를 알린다 // [재편 B §3-1] 수칙 목록 — 승인된 것 전부 한 곳(줄마다 빼기·직접 추가 안내). 어휘=수칙·빼기.
      ...
      safe(function(){ var n=$("vBudgetNote"); if(!n) return;
        ...
        n.textContent = (inh?T("CL-C 설정 상속(실효 "+d.contract.codexVerifyBudget+") · ","inherited from CL-C (effective "+d.contract.codexVerifyBudget+") · "):"")
          + (ccMode?T("빈값=상속 · 0=이 슬롯만 무제한 · ","empty=inherit · 0=unlimited for this slot · "):"")
          + T("저장 후 다음 지시(턴)부터","applies from the next instruction (turn) after saving"); });
    ...
    safe(function(){
      ...
      (function(){
        ...
        { // 상태 배지+메타 칩으로 분해(문장 나열 → 구조 — 2026-07-20 UI 개선)
          ...
          if(d.deepseek && d.deepseek.hasKey){
            ...
            { const nr=(d.scoutArm&&d.scoutArm.ready===false)?T("(지금 준비 안 됨)"," (not ready now)"):""; // 실효 담당·준비 상태(4판 blocker: 키 없음=eff 가 실제 담당)
            const ea=d.scoutArm?armText(d.scoutArm.eff):scoutName("defaultText");
            hint2.textContent=T("정찰은 "+ea+"이 맡음"+nr+" — 비교 정찰만 잠김(⚙️ 고급설정에서 등록)","recon by "+ea+nr+" — only the comparison scout is locked (register in ⚙️ Advanced)"); }
        ...
        safe(function(){ // 탐색 담당 선택(2026-07-20 사용자 요청) — .seg 세그먼트(기존 디자인 언어)로 시각화(UX 교훈 3: 말 대신 구조)
          ...
          mk("self", scoutName("selfShort"), T("Claude · 무과금","Claude · free"), slotMismatch); // 이름은 정본(selfShort — 모드 기본이 codex 면 'Claude 정찰')
          ...
          else if(av.raw==="deepseek"&&!av.hasKey){ note.textContent=T("선호=DeepSeek이나 키 미등록 — 키 등록 전까지 "+armText(av.eff)+"이 맡아요","Preference=DeepSeek but no key — "+armText(av.eff)+" runs until you register one"); note.appendChild(advBtn()); } // 강등 실효(=모드 기본)
      ...
      safe(function(){
        ...
        apiBox("#3ca89a",
          !noKeyReady9(d.scoutArm) ? T("지금 이대로(키 없음) = 정찰 담당 준비 뒤 기본 흐름 동작","As-is (no key) = the default flow works once the scout is ready") : T("지금 이대로(키 없음) = 기본 흐름 전부 동작","As-is (no key) = the entire default flow works"), // 키 없을 때 실행될 담당의 준비 상태 기준(5판 blocker)
          [T("정찰 AI = "+armText(noKeyArm9(d.scoutArm))+(noKeyReady9(d.scoutArm)?"":"(지금 준비 안 됨)"),"scout AI = "+armText(noKeyArm9(d.scoutArm))+(noKeyReady9(d.scoutArm)?"":" (not ready now)")), // 키 없을 때 실제 실행될 담당+그 담당의 준비 상태(4·5판 blocker)
        ...
        apiBox("#9a6cdc",
          ...
          [T("정찰을 다른 AI에게 맡겨 관점 비교·"+armName(noKeyArm9(d.scoutArm))+" 사용량 절약","a second AI scouts — perspective compare · saves "+armName(noKeyArm9(d.scoutArm))+" usage"),
           d.deepseek&&d.deepseek.hasKey?T("현재: 등록됨 — 비교 정찰 사용 가능","now: registered — comparison scout ready"):T("실측(2026-07)에선 "+armName("self")+"이 더 정확했어요 — 필수 아님","in our 2026-07 test the "+armName("self")+" was more accurate — not required")]);
        box.appendChild(api);
      });
      ...
      safe(function(){
        ...
        add(T("⚡ 실질 효과는 '정찰 실행'에서 나와요 — 한 번도 안 돌리면 지도·일지·교범은 비어 있습니다.","⚡ Real value comes from recon actually running — if it never runs, maps/journal/manual stay empty."),"muted");
      ...
      (function(){
        ...
        if(d.scoutLive) line=T("지금: 지도 생성 중… ("+armName(d.scoutLive.arm)+" — 끝나면 아래 게시판에 도착)","Now: generating a map… ("+armName(d.scoutLive.arm)+" — lands on the board below when done)");
      ...
      safe(function(){
        ...
        if(d.deepseek && !d.deepseek.hasKey){
          addD(T("ⓘ 키 없이도 변경 감지 + 지도 생성까지 가능해요(지도는 "+armText(noKeyArm9(d.scoutArm))+"이 맡음"+(noKeyReady9(d.scoutArm)?"":" · 지금 준비 안 됨")+") — DeepSeek API 키는 '비교용 두 번째 정찰'만 엽니다(⚙️ 고급설정 탭).","ⓘ Without any key you get change sensing plus map generation (maps by "+armText(noKeyArm9(d.scoutArm))+(noKeyReady9(d.scoutArm)?"":" · not ready now")+") — a DeepSeek API key only unlocks the second, comparison scout (⚙️ Advanced tab)."),"muted"); // 키 없을 때 실행될 담당+준비 상태(4·5판 blocker)
        }
        const sc=d.scope;
    ...
    safe(function(){
      ...
      if(!sm || !sm.count){
        ...
        add(nonGit
          ...
          : T("AI 정찰 보고서(영향지도)가 아직 없어요 — 생성은 터미널에서: " + BC9("scope-scout-self.js") + " <프로젝트경로> ("+armName("self")+" · 별도 과금 없음) 또는 같은 폴더의 scope-scout-deepseek.js ("+armName("deepseek")+") · scope-scout-codex.js ("+armName("codex")+"). 마켓 설치본에도 이 실행 파일이 함께 들어 있어요. 생성되면 몇 초 뒤 여기 자동으로 떠요.","No AI recon report (impact map) yet — generate from a terminal: " + BC9("scope-scout-self.js") + " <repo> ("+armName("self")+" · no separate billing), or scope-scout-deepseek.js ("+armName("deepseek")+") / scope-scout-codex.js ("+armName("codex")+") in the same folder. The marketplace build ships these runners too. New maps appear here a few seconds after generation."),"muted");
        return;
      }
      ...
      sm.items.forEach(it=>{
        ...
        add("• ["+when+"] "+armName(it.arm)+(it.model?" ("+it.model+")":"")+usage,"muted"); // 담당 이름 정본
      });
      if(sm.latest){
        // 펼침 유지 키는 지도 시각 기반 — 새 지도가 오면 기본 접힘(옛 지도에서 연 상태가 새 지도로 새지 않게 — Codex 보완)
        const det=keyedDetails("map:"+(sm.latest.ts||"?"), T("최신 지도 펼쳐보기 ("+armName(sm.latest.arm)+")","Open latest map ("+armName(sm.latest.arm)+")"));
      ...
      { const impl9 = (d.contract&&d.contract.harnessMode==="codex-codex") ? T("구현 Codex","the implementer Codex") : "Claude"; // 자동 지시 수신자=현재 모드의 구현 담당(문구 감사 2026-09-21 — Claude 단정 제거)
      add(T("ⓘ 이 게시판은 열람 전용(보는 것만으로는 아무것도 전송 안 됨) — 지도 생성·전송은 명령이 실행될 때만: 당신이 직접, 또는 3트랙 자동 지시를 받은 "+impl9+"가(같은 상태엔 1회 지시). 대형·장기 프로젝트를 위해 프로젝트별 최근 30장을 보관.","ⓘ Read-only board (viewing sends nothing) — maps are generated/sent only when the command runs: by you directly, or by "+impl9+" on the 3-track auto-directive (issued once per state). The latest 30 are kept per project for large, long-running work."),"muted"); }
    });
    ...
    safe(function(){ renderRulesFlow(d); }); // [§B3 · D4] 수칙 흐름 4칸(개요)
  ...
  const render = () => {
    ...
    const scoutSb = (() => {
      ...
      try {
        ...
        if (live) { const armN = scoutNameExt(ws, live.arm === "deepseek" ? "deepseek" : live.arm === "codex" ? "codex" : "self"); return tE(`정찰(3트랙): 지도 생성중… (${armN} · ${new Date(live.startedAt).toLocaleTimeString()} 시작)`, `recon (3-track): generating map… (${armN} · started ${new Date(live.startedAt).toLocaleTimeString()})`); }
    ...
    const scoutLiveArmName = scoutLiveNow ? scoutNameExt(ws, scoutLiveNow.arm === "deepseek" ? "deepseek" : scoutLiveNow.arm === "codex" ? "codex" : "self") : ""; // 담당 이름 정본(모드 기본 반영)
    const llmLine = (mode === "linked" || mode === "unlinked")
      ? (scoutLiveNow
        ? tE(`⚡ LLM 호출 중: 정찰 지도 생성(${scoutLiveArmName})`, `⚡ LLM call in flight: recon map (${scoutLiveArmName})`)
        : scoutOn
    ...
    if (flowActive && live) {
      ...
      const flowScout = scoutLiveNow
        ? tE(`\n\n⚡ 정찰 지도 생성 중(${scoutLiveArmName}) — 이 턴 안에서 실행`,`\n\n⚡ recon map generating (${scoutLiveArmName}) — running inside this turn`)
        : "";
