...
const attEnv = buildScoutAttach(ws, { scoutMode: "on" }, "ko");
const att = attEnv && attEnv.text;
ok(attEnv && Array.isArray(attEnv.mapItems) && attEnv.mapItems.length > 0, "envelope — 이번 ask에 실린 항목 스냅샷(mapItems) 동봉(L1-A echo 판정 입력)");
...
const attStale = (buildScoutAttach(ws, { scoutMode: "on" }, "ko") || {}).text;
ok(attStale && attStale.includes("낡음"), "낡은 지도 → 버리지 않고 낡음 라벨로 고지");
const attEn = (buildScoutAttach(ws, { scoutMode: "on" }, "en") || {}).text;
...
const attStored = (buildScoutAttach(ws, { scoutMode: "on" }, "ko") || {}).text;
...
const attFallback = (buildScoutAttach(ws, { scoutMode: "on" }, "ko") || {}).text;
...
const attBroken = (buildScoutAttach(ws, { scoutMode: "on" }, "ko") || {}).text;
...
console.log("[끝-끝(§6-7-1)] 문서 9번째 high가 지금 바뀐 파일이면 동봉 8개 안으로(조기 컷 무효화 반례 잠금)");
{
  ...
  const att9 = (buildScoutAttach(repo9, { scoutMode: "on" }) || {}).text;
