...
ok(/키를 등록하는 행위는 자동 탐색에 대한 동의/.test(privacy) && /지도가 없거나 낡았고/.test(privacy) && /키를 삭제하면 자동 호출도 함께 꺼집니다/.test(privacy), "PRIVACY: 키 등록=자동 호출 동의 + 발동 조건 정직 명시(사용자 결정)");
ok(/키 등록은 자동 탐색 동의로 간주돼요/.test(ext) && /counts as consent to automatic scouting/.test(ext), "고급설정 hint에도 동의 모델 양언어 명시");
