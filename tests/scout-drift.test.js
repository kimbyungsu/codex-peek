...
ok(/head: \(pkg\.meta && pkg\.meta\.head\) \|\| ""/.test(fs.readFileSync(path.join(__dirname, "..", "bridge", "scout-providers.js"), "utf8")), "공통 파이프라인(P5)이 메타에 head 기록(커밋 신호 재료)");
ok(/runScout\(repo, "self"/.test(fs.readFileSync(path.join(__dirname, "..", "bridge", "scope-scout-self.js"), "utf8")) && /runScout\(repo, "deepseek"/.test(fs.readFileSync(path.join(__dirname, "..", "bridge", "scope-scout-deepseek.js"), "utf8")), "러너 2종 runScout 위임(패리티는 구조 보장)");
...
const pkgSrc = fs.readFileSync(path.join(__dirname, "..", "bridge", "scope-package.js"), "utf8");
...
for (const rp of ["bridge/scout-providers.js", "bridge/scope-scout-self.js", "bridge/scope-scout-deepseek.js"]) {
