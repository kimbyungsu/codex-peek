...
// 논리 프로젝트(A)와 계약에 명시한 실제 작업 저장소(A/repo)가 다른 경우는 둘 중 최신 관측을 사용한다.
// 형제 프로젝트(B)는 후보에 없으므로 여전히 섞이지 않는다.
function lastCtxForProject(entries, project, mapped) {
  const candidates = [project, mapped].filter(Boolean).map((ws) => {
    const hit = lastCtxForWs(entries, ws);
    let ts = "";
    for (const e of entries) if (normPath(e.cwd) === normPath(ws) && (!ts || Date.parse(e.ts || "") >= Date.parse(ts || ""))) ts = e.ts || "";
    return {...hit, ts};
  });
  return candidates.sort((a,b)=>(Date.parse(b.ts||"")||0)-(Date.parse(a.ts||"")||0))[0];
}
const mappedActual = [
  { cwd:"D:\\A", model:"gpt-5.5", effort:"high", ts:"2026-07-12T10:00:00Z" },
  { cwd:"D:\\B", model:"gpt-other", effort:"low", ts:"2026-07-12T12:00:00Z" },
  { cwd:"D:\\A\\repo", model:"gpt-5.6", effort:"xhigh", ts:"2026-07-12T11:00:00Z" },
];
ok(lastCtxForProject(mappedActual,"D:\\A","D:\\A\\repo").model === "gpt-5.6", "프로젝트 매핑: 실제 작업 저장소의 최신 응답을 논리 프로젝트 실제값으로 사용");
ok(lastCtxForProject(mappedActual,"D:\\A","").model === "gpt-5.5", "프로젝트 격리: 명시 매핑이 없으면 하위/형제 폴더 값을 임의 사용하지 않음");
...
ok(/function sessionModelMetaForProject[\s\S]*projectWorkspaceCandidates/.test(ext), "확장 배선: 프로젝트↔실제 작업 폴더 메타 선택기 존재");
ok(/codexImplementerActiveMeta\(rawLink\.implementerSession, ws\)/.test(ext), "확장 배선: 구현자 훅 현재값을 drift에 사용");
ok(/sessionSelectedMetaForProject\(f, ws\)/.test(ext) && /thread_settings_applied/.test(fs.readFileSync(path.join(__dirname,"..","src","rollout-scan.ts"),"utf8")), "확장 배선: Codex 피커 선택 이벤트를 프롬프트 전 구현자 drift에 사용");
ok(/if \(ok\) this\.onChange\?\.\(\)/.test(ext), "확장 배선: 검증 두뇌 저장 직후 상태바 즉시 재계산");
ok(/const settingsPoll = setInterval[\s\S]*maybeAttributeSettingsChange\(\)[\s\S]*750/.test(ext), "확장 배선: Claude 설정 watch 누락을 포커스 폴링으로 보완");
ok(/implementerActualAge/.test(ext) && /d\.implementerActualAge\?/.test(ext), "구현 Codex 실제 답에도 경과시간 데이터·UI 표시 배선");
