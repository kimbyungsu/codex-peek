...
fs.writeFileSync(path.join(scoutsDir, "2026-07-07T00-00-00-000Z-00-self.md"), "① 직접 변경 후보\n- src/seed-fresh.md — high"); // 형식 계약(구획 표기) 충족 — 잡문 md는 v2에서 invalid(별도 케이스 [3c])
...
fs.writeFileSync(path.join(scoutsDir, "2026-07-07T00-00-01-000Z-00-self.md"), "① 구버전 지도 — 형식은 유효");
...
console.log("[3c] 형식 불명(invalid) 지도 — 빈/잡문 md는 fresh를 사칭 못 함(L1-C 품질: high 0개 지도 통과 결함 봉합)");
fs.writeFileSync(path.join(scoutsDir, "2026-07-07T00-00-02-000Z-00-self.md"), "구획 표기도 항목도 없는 자유 잡문 — 지도가 아님");
fs.writeFileSync(path.join(scoutsDir, "2026-07-07T00-00-02-000Z-00-self.json"), JSON.stringify({ ts: new Date(Date.now() + 60_000).toISOString(), arm: "self", seedFiles: ["seed-fresh.md"] }));
r = runHook(undefined, "sess-I");
ok(r.status === 2 && /형식을 알아볼 수 없다/.test(r.stderr), "invalid → 차단+재생성 필요 문구(신선도가 아니라 품질 사유)");
fs.unlinkSync(path.join(scoutsDir, "2026-07-07T00-00-02-000Z-00-self.md"));
fs.unlinkSync(path.join(scoutsDir, "2026-07-07T00-00-02-000Z-00-self.json"));
