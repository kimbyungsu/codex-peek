...
console.log("[3-7] 판독 형태 드리프트 봉합(2026-09-06 실측 — D-2026-09-06-evidence-read-form-drift) — 실행 호출=계약 형태·PowerShell 리터럴 배열 foreach");
{
  ...
  // (tp22) ★11판 확인검증(2차) blocker: 선언 원소에 메서드 호출(__defineGetter__ 등)로 명령 변이=미인정
  ck("(tp22a) ★cmds[0].__defineGetter__(…) 변이=미인정([])", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\ncmds[0].__defineGetter__("cmd",()=> "x");\nconst r=await tools.exec_command(cmds[0]);\ntext(r.output);').length === 0);
  ck("(tp22b) ★임의 메서드 호출(cmds[0].valueOf()) 마무리=미인정", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\ntext(cmds[0].valueOf());').length === 0);
  ck("(tp22c) 선언 원소 값 읽기(text(JSON.stringify({c:cmds[0]})))는 인정(무회귀)", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\ntext(JSON.stringify({c:cmds[0],o:r.output}));').length === 1);
  ck("(tp22d) 결과 변수 메서드(results.forEach)=인정(무회귀)", B7.templateScriptCommands('const results=await Promise.all([tools.exec_command({cmd:"cat a.ts",workdir:"D:/x"})]);\nresults.forEach((r)=>text(r.output));').length === 1);
  // (tp23) ★11판 확인검증(2차) blocker: 파일명 소유 표식은 콜론(rg/Select-String)·@@ 만 — 줄 시작이라도 콜론/@@ 없으면 소유 아님
  ck("(tp23a) ★줄 시작 파일명+공백(foo.ts shared text)=소유 아님(제외 유지)", !B7.outputContainsFileLine("foo.ts shared text", ["foo.ts shared text"], { exclude: new Set(["foo.ts shared text"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp23b) 콜론/@@ 없는 줄 시작 파일명은 제외 집합을 우회하지 못함", !B7.outputContainsFileLine("foo.ts other-shared-line", ["foo.ts other-shared-line"], { exclude: new Set(["foo.ts other-shared-line"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp23c) rg 콜론 접두=소유 인정 · @@ 구역=소유 인정(무회귀)", B7.outputContainsFileLine("foo.ts:2:const shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }) && B7.outputContainsFileLine("@@ foo.ts\nconst shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }));
  // (tp24) ★11판 확인검증(3차) blocker: 선언 원소의 사슬 메서드 호출·C 스타일 for는 정형 밖(미인정)
  ck("(tp24a) ★cmds[0].cmd.toString() 사슬 메서드=미인정([])", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\ntext(cmds[0].cmd.toString());').length === 0);
  ck("(tp24b) ★for(;;) 무한 반복 마무리=미인정([])", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\nfor(;;) text("tick");').length === 0);
  ck("(tp24c) for-of 마무리=인정(무회귀) · 선언 원소 값 읽기(text(JSON.stringify(cmds[0])))=인정", B7.templateScriptCommands('const cmds=[{cmd:"cat a.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\ntext(JSON.stringify(cmds[0]));').length === 1 && B7.templateScriptCommands('const results=await Promise.all([tools.exec_command({cmd:"cat a.ts",workdir:"D:/x"})]);\nfor (const r of results) text(r.output);').length === 1);
  // (tp25) ★11판 확인검증(3차) blocker: Windows 절대 경로 접두도 소유 표식으로 인정(정직한 절대 경로 판독이 경보로 남지 않게)
  ck("(tp25a) ★D:\\…\\foo.ts:2: 절대 경로(백슬래시) 접두=소유 인정", B7.outputContainsFileLine("D:\\codex-peek\\foo.ts:2:const shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp25b) ★D:/…/foo.ts:2: 절대 경로(슬래시) 접두=소유 인정", B7.outputContainsFileLine("D:/codex-peek/foo.ts:2:const shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp25c) @@ D:\\…\\foo.ts 절대 경로 구역 표시=소유 인정", B7.outputContainsFileLine("@@ D:\\codex-peek\\foo.ts\nconst shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp25d) 절대 경로여도 콜론/@@ 없으면 소유 아님(본문 언급 무회귀)", !B7.outputContainsFileLine("see D:\\codex-peek\\foo.ts here", ["see D:\\codex-peek\\foo.ts here"], { exclude: new Set(["see D:\\codex-peek\\foo.ts here"]), name: "foo.ts", others: ["bar.ts"] }));
  // (tp26) ★11판 확인검증(4차) blocker: 선택적 호출(?.())·괄호 감싼 호출((expr)())도 선언 데이터 호출로 미인정
  ck("(tp26a) ★cmds[0].cmd.toString?.() 선택적 호출=미인정([])", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\ntext(cmds[0].cmd.toString?.());').length === 0);
  ck("(tp26b) ★(cmds[0].valueOf)() 괄호 감싼 호출=미인정([])", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\ntext((cmds[0].valueOf)());').length === 0);
  ck("(tp26c) 정상 마무리(text(r.output))=인정(무회귀)", B7.templateScriptCommands('const r=await tools.exec_command({cmd:"cat foo.ts",workdir:"D:/x"});\ntext(r.output);').length === 1);
  // (tp27) ★11판 확인검증(4차) blocker: 공백 포함 경로의 소유 표식·제외 집합
  ck("(tp27a) ★공백 포함 절대 경로 접두(D:\\My Project\\foo.ts:2:)=소유 인정", B7.outputContainsFileLine("D:\\My Project\\foo.ts:2:const shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp27b) ★공백 포함 상대 경로 접두(My Project/foo.ts:2:)=소유 인정", B7.outputContainsFileLine("My Project/foo.ts:2:const shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp27c) ★@@ 공백 포함 경로 구역 표시=소유 인정", B7.outputContainsFileLine("@@ D:\\My Project\\foo.ts\nconst shared = true;", ["const shared = true;"], { exclude: new Set(["const shared = true;"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp27d) 공백 있어도 콜론/@@ 없으면 소유 아님(무회귀)", !B7.outputContainsFileLine("foo.ts shared text", ["foo.ts shared text"], { exclude: new Set(["foo.ts shared text"]), name: "foo.ts", others: ["bar.ts"] }));
  ck("(tp27e) ★callMentionedFiles: 따옴표로 묶인 공백 경로를 통째 토큰으로 인식(this repo README 사용)", (() => { const repo = path.join(__dirname, "..").replace(/\\/g, "/"); const rd = repo + "/README.md"; const scr = 'const r=await tools.exec_command({cmd:"Get-Content -LiteralPath \\"' + rd + '\\"",workdir:"' + repo + '"});\ntext(r.output);'; const men = B7.callMentionedFiles({ type: "custom_tool_call", name: "exec", input: scr }, repo); return [...men.keys()].some((k) => k.toLowerCase().endsWith("readme.md")); })());
  // (tp28) ★사용자 결정 8f2908ced14fc9a2: 결과 목록 첨자 순회 for 마무리(검증자 실측 9/7 정직 6건)=인정 · 무한 루프 for(;;)·다른 C 스타일 for는 계속 미인정
  ck("(tp28a) ★for (let i = 0; i < results.length; i++) text(`RESULT_${i+1}…`)=인정(실측 마무리)", B7.templateScriptCommands('const results = await Promise.all([\n  tools.exec_command({\n    cmd: "git -c safe.directory=D:/x rev-parse HEAD",\n    workdir: "D:\\\\x",\n    yield_time_ms: 10000,\n    max_output_tokens: 8000\n  }),\n  tools.exec_command({\n    cmd: "node tests/evidence-unseen.test.js",\n    workdir: "D:\\\\x",\n    yield_time_ms: 30000,\n    max_output_tokens: 20000\n  })\n]);\nfor (let i = 0; i < results.length; i++) {\n  text(`RESULT_${i + 1}\\n${results[i].output}\\nEXIT ${results[i].exit_code}`);\n}\n').length === 2);
  ck("(tp28b) 한 줄 형태 for (let i = 0; i < results.length; i++) text(…)=인정", B7.templateScriptCommands('const results=await Promise.all([tools.exec_command({cmd:"cat a.ts",workdir:"D:/x"})]);\nfor (let i = 0; i < results.length; i++) text(`RESULT_${i + 1}\\n${results[i].output}\\nEXIT ${results[i].exit_code}`);').length === 1);
  ck("(tp28c) 첨자 순회 머리가 선언 배열(cmds)을 돌거나 결과 변수가 아니면 미인정 · for(;;)·for(let i=0;i<9;i++)는 계속 미인정", B7.templateScriptCommands('const cmds=[{cmd:"cat foo.ts",workdir:"D:/x"}];\nconst r=await tools.exec_command(cmds[0]);\nfor (let i = 0; i < cmds.length; i++) text(cmds[i].cmd);').length === 0 && B7.templateScriptCommands('const r=await tools.exec_command({cmd:"cat foo.ts",workdir:"D:/x"});\nfor(;;) text("t");').length === 0 && B7.templateScriptCommands('const r=await tools.exec_command({cmd:"cat foo.ts",workdir:"D:/x"});\nfor(let i=0;i<9;i++) text("t");').length === 0);
