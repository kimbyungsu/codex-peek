...
console.log("[5] 배포 파일 — 3카피 패리티+실물+deepseek enrich 계약");
{
  ...
  ok(a.length === 51 && c.length === 51 && JSON.stringify([...a].sort()) === JSON.stringify([...c].sort()), "51파일(+정찰 층 12·코어 사본 3[2026-09-16 이관]+cap-handoff·router·enrich·intent·providers·retrieval·evidence-challenge·probe-batch·provenance·selector-runner·preview-gate[4b]·session-start[§4-B ①]·curation[CURATION v3]·citation-check[HARNESS-STRUCTURE §B1]·map-rotation[§B2]) 집합 일치");
