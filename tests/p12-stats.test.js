...
console.log("[8] 경보 supersede ws 격리(3축 감사 blocker) — 다른 프로젝트 경보 미삭제");
{
  ...
  CL2.supersedeIntegrity("S-other", "verdict-nonclean", "D:/proj-B"); // B의 새 검증(폴백 세션이 달라진 상황까지 재현)
  const left = CL2.readIntegrityEvents();
  ok(left.some((e) => e.workspace === "D:/proj-A") && !left.some((e) => e.workspace === "D:/proj-B"), "ws 전달 supersede — B만 갱신·A의 미확인 경보 보존(교차 삭제 봉합)+세션이 달라도 같은 프로젝트 경보는 갱신(잔존 경보 고아화 방지 — 확인 검증 blocker)");
