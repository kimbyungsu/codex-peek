...
console.log("[6] 배선 소스 단언 — codex-bridge·캐논·정본");
{
  ...
  ok(cb.includes("writeEnvelopeFreeze(wsIn, evi.st === \"ok\" ? evi.sha1 : null, jid9, fx9)") && cb.includes("경계 동결 기록 실패"), "ask 조립=주입에 쓴 지문을 재판독 없이 동결+잡 id 결속+합본 manifest 동결(원자 결속·v7 §2 — extra 인자 확장)+실패 가시화(1차 blocker②·5차 ①)");
  ...
  ok(cb.includes("v2DynamicData(wsIn, lang)") && cb.includes("[열린 지적 — 재지적·미완 수정 보고 시 이 id를 인용하라(미인용=신규 취급)]"), "경계 활성=v2 서식 요구+열린 지적 하네스 직접 주입(구현모델 선별 금지)");
...
console.log("[8] 증분 3 — 범위 확장 통로(비차단·캠페인당 승격 1회)");
{
  ...
  ok((cb3.match(/breakdownNoticeFor\(ws, langSnap, budgetGate\.res, repoKeySnap9\)/g) || []).length === 1 && (cb3.match(/integrityReviewLine\(ws, langSnap, profileSnap, repoKeySnap9\)/g) || []).length === 1 && (cb3.match(/finishVerifyRun\(/g) || []).length === 3, "분해·재심 병기=공유 꼬리 1곳(세 출력 경로 호출 경유)");
