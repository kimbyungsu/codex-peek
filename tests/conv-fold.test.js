...
{
  ok(ext.includes("const foldScroll = new Map()") && ext.includes("function keepInnerScroll(pre, key)"), "내부 스크롤 저장소+공용 헬퍼 존재");
  ok(ext.includes('pre.addEventListener("scroll"') && ext.includes("foldScroll.set(key, pre.scrollTop)"), "스크롤 시 좌표 저장 배선");
  ok(ext.includes("foldScroll.get(key)") && /requestAnimationFrame\(function\(\)\{ pre\.scrollTop=sv; \}\)/.test(ext), "재렌더 직후 한 프레임 뒤 복원(레이아웃 완료 시점)");
  ok(ext.includes("keepInnerScroll(pre, fkey)") && ext.includes('keepInnerScroll(pre, "mapPre:"+(sm.latest.ts||"?"))') && ext.includes('keepInnerScroll(pre, "ledgerPre:"+(ml.mapRel||"?"))'), "동류 상자 3곳 전부 적용(주입 지침·최신 지도·확정 장부 — 보관함 1cc80d65856b7c41 채택)");
