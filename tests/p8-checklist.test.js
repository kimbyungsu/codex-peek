...
ok(/m\?\.type === "saveChecklist"/.test(ext) && /patchContractRetryExt\(wsCk, slotLang, \{ \[field\]: !!m\.value \}\)/.test(ext), "호스트 saveChecklist 핸들러 — 관문 재읽기-단일필드 병합(P-8 2단: 비동기 재시도 경유·의미 동일)");
...
ok(/if \(ckR\.act === "commit" && ckModeOk\) \{/.test(ext) && /if \(ckEl\) \{ if \(!ckModeLock\) ckEl\.disabled = false;/.test(ext) && !/appCkX !== null \? appCkX : ckEl\.checked/.test(ext), "성공(commit·모드 결속 일치)만 재활성+기준선 — 실패·모드 불일치는 hold(2단 v10 ⑸: mode 직접 결속 추가·상태기 우회 없음)");
...
ok(/fail-closed — P-1 교훈/.test(cl) && /단일 관문\(v10\)/.test(cl) && /function updateContractPatch\(/.test(cl), "도우미 — 2단 승격 완료: 단일 관문 updateContractPatch 실재+fail-closed 근거(승격 예정 서술은 실체로 대체)");
