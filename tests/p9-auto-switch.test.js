...
ck("A9 성공 Stop=done 종결(소스 계약)", /if\(gate\.ok&&residualOk\)\{try\{writePhase\("done",\{session:sid,workspace:ws\}\);\}catch\{\} return;\}/.test(hookSrc)); // [잔여 재검증 강제 2026-08-29] 성공 Stop=done은 잔여 마커가 없을 때(residualOk)에만
setContract("codex-codex"); clearState();
