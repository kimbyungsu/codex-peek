...
// 임시 브릿지 홈 격리(무이력 물때표 계단이 scout-store를 lazy require — 실사용 지도 보관함 오염/의존 방지)
process.env.CODEX_BRIDGE_HOME = require("fs").mkdtempSync(path.join(require("os").tmpdir(), "sp-test-home-"));
...
ok(hp.historyless === true && hp.blindSpots.some((b) => /간주 기준'은 근사/.test(b)) && hp.blindSpots.some((b) => /전후 비교\(diff\)가 없다/.test(b)), "무이력 맹점 각주 3종(간주 기준=근사·diff 없음·통계 불가 — basisNote와 모순 없음)");
const hmd = renderPackageMarkdown(hp);
ok(/\[무이력 모드 — 비-git\]/.test(hmd) && /변경으로 간주한 파일\(이력 없음\)/.test(hmd) && /최근 수정 파일 발췌 \(전후 비교 불가/.test(hmd), "렌더 라벨이 '변경'을 사칭하지 않음(간주·발췌 명시)");
...
// 물때표 계단 재현: '마지막 지도'가 어제 있었고, 노트.md만 그 뒤에 수정됨 → seed=노트.md만(옛 참조.md는 역참조 대상)
const store2 = require(path.join(__dirname, "..", "scripts", "scout-store.js"));
const mapPath = store2.saveMap(tmpNg, "self", "이전 지도", {});
const metaPath = mapPath.replace(/\.md$/, ".json");
const meta2 = JSON.parse(require("fs").readFileSync(metaPath, "utf8"));
meta2.ts = new Date(Date.now() - 24 * 3600 * 1000).toISOString(); // 지도 생성 시각을 어제로(물때표)
require("fs").writeFileSync(metaPath, JSON.stringify(meta2));
