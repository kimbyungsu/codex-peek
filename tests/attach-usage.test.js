...
console.log("[4] 배선 소스 계약 — withContract 동봉 try 안·best-effort·게이트 생략 표기");
{
  ...
  ck("기록은 조립 검사(fail-closed) 통과 뒤 — 차단된 ask는 행을 안 남김", /contractTooLong[\s\S]{0,7000}appendAttachUsage\(\{/.test(fn) && !/appendAttachUsage[\s\S]{0,7000}contractTooLong/.test(fn));
  ...
  ck("자체 try — 기록 실패가 ask를 못 막음", /if \(attSnap9 \|\| \(carrier && carrier\.envelope\)\) \{\s*\n\s*try \{[\s\S]{0,900}appendAttachUsage\(\{/.test(fn));
