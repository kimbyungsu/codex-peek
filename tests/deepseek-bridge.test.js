...
const selfSrc = fs.readFileSync(path.join(__dirname, "..", "bridge", "scout-providers.js"), "utf8"); // P5: self 팔 로직은 공통층(scout-providers.js)으로 이동
...
const runnerSrc = fs.readFileSync(path.join(__dirname, "..", "bridge", "scout-providers.js"), "utf8"); // P5: deepseek 팔 로직도 공통층으로 이동
ok(/collectPackage/.test(runnerSrc) && /deepseek-bridge\.js/.test(runnerSrc), "러너는 같은 꾸러미 수집기 + 브릿지 map 경유(별도 수집 경로 없음 — 공통층 한 곳)");
ok(/runScout\(repo, "deepseek"/.test(fs.readFileSync(path.join(__dirname, "..", "bridge", "scope-scout-deepseek.js"), "utf8")), "deepseek 러너는 runScout 위임(P5)");
...
ok(!/\*\*외부 전송 없음\*\*/.test(readmeKo) && /외부로 나가는 경로는 네 갈래/.test(readmeKo) && !/외부로 나가는 경로는 세 갈래/.test(readmeKo) && !/두 갈래/.test(readmeKo) && !/유일한 예외/.test(readmeKo) && !/예외는 둘뿐/.test(readmeKo), "README(ko): 절대 표현 제거 + 네 갈래 모델(Codex 검증/DeepSeek 키 2종/기본 정찰 Claude CLI/Codex 정찰 codex CLI — 검색 4조각 2026-08-03 개정), 구 모델 잔재 0");
...
ok((readmeKo.match(/외부로 나가는 경로는 네 갈래|외부로 나가는 것은 Codex 검증 전송/g) || []).length >= 2 && !/외부로 나가는 것은 정찰 관련뿐/.test(readmeKo), "README 안전 절·원칙 절이 같은 네 갈래 모델(정찰-관련뿐 구문 잔재 0)");
