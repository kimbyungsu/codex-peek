...
ok(/function baseInputLock\(on\)\{/.test(ext) && /\["saveB","resetB"\]\.forEach/.test(ext) && /\["bVerify","bTransmit","bRejudge"\]\.forEach/.test(ext) && /sc\.readOnly = on;/.test(ext) && /rv\.disabled = baseM\.saving\(\);/.test(ext), "4차1 — 저장·복원 버튼+4필드 입력을 정본 확인까지 잠금(P-12: 3칸은 core 읽기 전용 겸용·④정찰 분리, 되돌리기는 응답 대기만 — 복구 수단)");
