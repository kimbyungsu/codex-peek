...
console.log("[2] 순수 함수 실행 — 컴파일 산출물에서 computeChallengeView 추출");
{
  ...
  ok(v.counts.resolved === 2 && v.counts.failed === 1 && v.counts.dispatched === 1 && v.counts.pending === 1, "상태별 집계 정확");
  ok(v.open === 2, "진행 중=pending+dispatched");
  ok(v.cleared === 1 && v.kept === 2, "cleared=전 파일 일치만 · kept=종결-해소(부분 일치 포함)");
  ...
  ok(v.items.length === 5 && v.items[0].id === "ch-4", "오염 줄 무시·최신(경과 짧은) 순 정렬");
...
console.log("[3] 렌더 블록 실행 반례 — 숨김/빈 상태/요약·목록·상태 라벨");
{
  ...
  const items = [
    { id: "ch-a", state: "resolved", files: 2, resolvedFiles: 2, ageMin: 3, cleared: true },
    { id: "ch-b", state: "resolved", files: 2, resolvedFiles: 1, ageMin: 5, cleared: false },
  ...
  n = runRender({ open: 0, cleared: 1, kept: 3, counts: { resolved: 2, failed: 1, "no-dispatch": 1 }, items }, "ko");
  ok(/총 4건/.test(n.chSummary.textContent) && /해소 1/.test(n.chSummary.textContent) && /경고 유지 3/.test(n.chSummary.textContent), "CH-4 요약 — 해소=cleared만·경고 유지=kept(부분 일치 포함)");
  ok(n.chList.children.length === 4, "CH-5 목록 렌더 4건");
