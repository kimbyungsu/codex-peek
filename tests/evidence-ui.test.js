...
console.log("[2] 순수 함수 실행 — 컴파일 산출물에서 computeChallengeView 추출");
{
  ...
  const unacked = new Set(["ev-open-1"]); // ch-6의 경고가 아직 미ack(종결 직후 중단 재현)
  const v = view(recs, now, unacked);
  ok(v.counts.resolved === 3 && v.counts.failed === 1 && v.counts.dispatched === 1 && v.counts.pending === 1, "상태별 집계 정확");
  ok(v.open === 2, "진행 중=pending+dispatched");
  ok(v.cleared === 1 && v.kept === 3, "cleared=전 파일 일치+실제 ack만 · kept=종결-해소(부분 일치·반영 대기 포함)");
  ...
  ok(v.items.length === 6 && v.items[0].id === "ch-4", "오염 줄 무시·최신(경과 짧은) 순 정렬");
...
console.log("[3] 렌더 블록 실행 반례 — 숨김/빈 상태/요약·목록·상태 라벨");
{
  ...
  const items = [
    { id: "ch-a", state: "resolved", files: 2, resolvedFiles: 2, ageMin: 3, cleared: true, matchedAll: true },
    { id: "ch-a2", state: "resolved", files: 1, resolvedFiles: 1, ageMin: 4, cleared: false, matchedAll: true },
    { id: "ch-b", state: "resolved", files: 2, resolvedFiles: 1, ageMin: 5, cleared: false, matchedAll: false },
    { id: "ch-c", state: "failed", files: 1, resolvedFiles: 0, ageMin: 9, cleared: false },
    { id: "ch-d", state: "no-dispatch", files: 1, resolvedFiles: 0, ageMin: 12, cleared: false },
  ];
  n = runRender({ open: 0, cleared: 1, kept: 4, counts: { resolved: 3, failed: 1, "no-dispatch": 1 }, items }, "ko");
  ok(/총 5건/.test(n.chSummary.textContent) && /해소 1/.test(n.chSummary.textContent) && /경고 유지 4/.test(n.chSummary.textContent), "CH-4 요약 — 해소=cleared만·경고 유지=kept(부분 일치·반영 대기 포함)");
  ok(n.chList.children.length === 5, "CH-5 목록 렌더 5건");
  ...
  ok(row1.children.some((c) => /부분 일치\(경고 유지\)/.test(c.textContent)), "CH-7 resolved+부분 일치=경고 유지 라벨(과대 표시 금지)");
  const row3 = n.chList.children[4];
