...
console.log("[3] 확장 배선 — 상태에 이름 묶음이 실리고 화면·상태바·모달이 그 이름만 쓴다");
{
  ...
  ok(/function noKeyArm9\(av\)\{ if\(av && \(av\.noKeyArm==="self"\|\|av\.noKeyArm==="codex"\)\) return av\.noKeyArm; if\(scoutNames9 && \(scoutNames9\.noKeyArm==="self"\|\|scoutNames9\.noKeyArm==="codex"\)\) return scoutNames9\.noKeyArm; var d9 = \(scoutNames9 && scoutNames9\.defaultArm\) \|\| \(av && av\.defaultArm\) \|\| "self"; return av && \(av\.raw==="self"\|\|av\.raw==="codex"\) \? av\.raw : d9; \}/.test(src), "웹뷰 noKeyArm9: 정찰 카드 상태(scoutArm.noKeyArm) → 상시 상태(scoutNames.noKeyArm) → 같은 규칙 폴백(명시 self/codex 우선 · 모드 기본)");
  ...
  ok(/function noKeyReady9\(av\)\{ if\(av && typeof av\.noKeyReady==="boolean"\) return av\.noKeyReady; if\(scoutNames9 && typeof scoutNames9\.noKeyReady==="boolean"\) return scoutNames9\.noKeyReady; return true; \}/.test(src) && (src.match(/noKeyReady9\(d\.scoutArm\)/g) || []).length >= 7 && !/\(d\.scoutArm&&d\.scoutArm\.ready===false\) \? T\("지금 이대로/.test(src), "웹뷰: API 비교 박스 머리·정찰 AI 줄·정찰 카드 안내·dsState 가 noKeyReady9(키 없을 때 실행될 담당의 준비 — 정찰 카드 상태 → 상시 상태 순) 기준 — eff 의 ready 차용 0");
  // 도장 검증 1판 blocker: 2트랙(정찰 off · scoutArm=null)에서도 고급설정 키 안내가 같은 준비 판정을 쓰고 '3트랙을 켜면' 조건부로 적는다
  ok(/type ScoutNames = \{ defaultArm: "self" \| "codex"; noKeyArm: "self" \| "codex"; noKeyReady: boolean; ko: ScoutNameSet; en: ScoutNameSet \} \| null;/.test(src) && /try \{ const v = scoutArmViewExt\(ws\); noKeyArm = v\.noKeyArm; noKeyReady = v\.noKeyReady; \}/.test(src) && (src.match(/return \{ defaultArm, noKeyArm, noKeyReady, ko: /g) || []).length === 2, "상시 상태 scoutNames 에 noKeyArm/noKeyReady(정찰 on/off 무관 · 같은 계산 scoutArmViewExt)");
  ok(/if\(scoutNames9 && \(scoutNames9\.noKeyArm==="self"\|\|scoutNames9\.noKeyArm==="codex"\)\) return scoutNames9\.noKeyArm;/.test(src) && /\(\(d\.contract&&d\.contract\.scoutMode==="on"\)\?"":"3트랙을 켜면 "\)/.test(src) && /\(\(d\.contract&&d\.contract\.scoutMode==="on"\)\?"":"With 3-track on, "\)/.test(src) && /"명령줄 설치 필요"/.test(src), "dsState: 정찰 off 면 '3트랙을 켜면 …' 조건부 + 준비 안 됨(명령줄 설치 필요) 표기 — 미점검 단정 0");
