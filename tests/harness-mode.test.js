...
const hookSrc=fs.readFileSync(hook,"utf8");assert.match(hookSrc,/HOOK_STARTED_AT = require\("perf_hooks"\)\.performance\.timeOrigin/,"process time origin records hook event order before module I/O");// 창 900→2400: P-9 자동 전환 블록(모드 게이트~dispatch 사이)이 들어옴 — 순서 계약(revision 캡처→cleanup→dispatch)은 불변
assert.match(hookSrc,/const sid=[^\n]+if\(!sid\)return;[\s\S]{0,300}roleRevision=codexRoleRevision\(\);[\s\S]{0,160}maybeCleanupState\(\)[\s\S]{0,2400}onSessionStart\(j,ws,sid,c,roleRevision\)/,"hook captures the global role revision before cleanup and workspace discovery");assert.match(hookSrc,/expectedSession = codexImplementerSnapshot\(ws, roleRevision, HOOK_STARTED_AT\)[\s\S]{0,500}isVscodeUserSession\(j, sid\)[\s\S]{0,300}pinImplementer\(j, ws, sid, expectedSession\)/,"hook CAS snapshot includes process-start order before rollout identity I/O");
