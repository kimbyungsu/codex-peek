...
const { appendLedgerEvent, readLedgerEventsText, ledgerSig } = require(path.join(__dirname, "..", "bridge", "contract-lib.js"));
...
if (!repoArg || !cmd || (cmd !== "list" && !TYPE_MAP[cmd])) {
  console.error('사용: node scripts/scope-ledger-note.js <repo> <list|dispute|confirm|pin|ban|unpin|unban> "<항목 텍스트 조각>" [--why "<발화 요지>"]');
  process.exit(2);
}
const repo = path.resolve(repoArg);
const entries = deriveLedger(parseEventsJsonl(readLedgerEventsText(repo)).events);
if (cmd === "list") {
  if (!entries.length) { console.log("장부 비어 있음 — 정찰 지도가 ⑥ 제안을 내면 쌓입니다."); process.exit(0); }
  for (const e of entries) console.log(`[${e.status}${e.pinned ? "·고정" : ""}] ${e.text || e.sig}`);
  process.exit(0);
}
if (!frag || !String(frag).trim()) { console.error("항목 텍스트 조각을 지정하라 — 먼저 list로 확인"); process.exit(2); }
const needle = String(frag).toLowerCase();
const hits = entries.filter((e) => (e.text || e.sig).toLowerCase().includes(needle));
if (hits.length === 0) {
  console.error(`일치 항목 없음: "${frag}" — 현재 장부(${entries.length}건):`);
  for (const e of entries.slice(0, 15)) console.error(`  [${e.status}] ${(e.text || e.sig).slice(0, 100)}`);
  process.exit(1);
}
if (hits.length > 1) {
  console.error(`조각이 ${hits.length}개 항목과 일치 — 더 좁혀라(조용한 오기록 방지):`);
  for (const e of hits.slice(0, 10)) console.error(`  [${e.status}] ${(e.text || e.sig).slice(0, 100)}`);
  process.exit(1);
}
const target = hits[0];
const ev = { ts: new Date().toISOString(), type: TYPE_MAP[cmd], sig: target.sig, text: target.text, from: why ? `사용자 발화: ${why.slice(0, 160)}` : "사용자 발화 기록(요지 미기재)" };
if (!appendLedgerEvent(repo, ev)) { console.error("기록 실패(권한/디스크?) — 아무것도 반영되지 않음"); process.exit(1); }
const after = deriveLedger(parseEventsJsonl(readLedgerEventsText(repo)).events).find((e) => e.sig === target.sig);
console.log(`기록됨: ${ev.type} → "${(target.text || target.sig).slice(0, 80)}"`);
console.log(`현재 신분: ${after ? after.status + (after.pinned ? "·고정" : "") + " · 차선 " + after.lane : "?"}`);
