...
if (!srcEvents.length) { console.log(tB(`원본 서랍이 비어 있음: ${ledgerEventsFileFor(from)}`,`Source drawer is empty: ${ledgerEventsFileFor(from)}`)); process.exit(0); }
...
console.log(tB(`복사 예정 ${copy}건 · 중복 스킵 ${dup}건 · 원본은 보존(삭제 없음)`,`To copy: ${copy} · duplicate-skipped: ${dup} · source preserved (nothing deleted)`));
