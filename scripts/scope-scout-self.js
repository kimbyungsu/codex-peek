...
try { console.error(tB("지도 보관(게시판): ","Map archived (board): ") + saveMap(repo, "self", map, { ...scoutPromptSignature(lang), highlights: extractMapHighlights(map), mapPatches: extractMapPatches(map), basis: pkg.basisNote || (pkg.historyless ? "" : "git-status"), seedFiles: pkg.seeds, ...baseline, head: (pkg.meta && pkg.meta.head) || "" })); } catch (e) { console.error(tB("지도 보관 실패(게시판에만 영향): ","Map archive failed (affects the board only): ") + (e && e.message)); } // head=지도 생성 시점 커밋 — 신선도의 '새 커밋' 신호 재료(2026-07-10)
