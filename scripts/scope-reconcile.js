...
const { atomicWrite, loadLang } = require(path.join(__dirname, "..", "bridge", "contract-lib.js")); // 확정층·상태 쓰기는 원자적으로(반쪽 파일 방지)
...
if ((cmd === "approve" || cmd === "reject") && nums.some((n) => !Number.isInteger(n) || n <= 0)) {
  console.error(tB(`번호가 아닌 인자 있음: ${rawNums.join(" ")} — 양의 정수 번호만(먼저 list로 확인)`,`Non-numeric argument: ${rawNums.join(" ")} — positive integers only (check with list first)`)); process.exit(2);
}
...
if (cmd !== "approve" && cmd !== "reject") { console.error(tB(`알 수 없는 명령: ${cmd} (list|approve|reject)`,`Unknown command: ${cmd} (list|approve|reject)`)); process.exit(2); }
