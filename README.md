...
> **Claude(구현) → 브릿지(훅 3종) → Codex(검증) → 근거 회신** (사람이 중계 안 함). 검증이 '흉내'가 아님을 보장하는 층(증명 V1 · Bash 변경 감지 V2 · 재검증 V4 · 근거 점검 결정2)과, 지금 검증이 어느 단계인지 **상태바·대시보드에 라이브 표시**합니다.
...
2. 검증 훅이 등록돼 있지 않으면 알림이 뜹니다 — **[설치 내용 보기]** → *바꾸는 파일·백업 위치·등록될 훅 3줄*을 확인하고 **[설치]** 1클릭. Claude Code 설정(`settings.json`)은 **이 동의 후에만** 병합됩니다(수정 전 `settings.json.bak.<시각>` 백업, 기존 다른 훅 보존 — 레포 설치기와 같은 규칙).
...
- **외부 전송 없음 / 읽기 전용 원칙**: 모든 데이터는 로컬에 머물고, 코덱스 원본 기록은 *직접 "영구삭제"를 누른 한 파일* 외엔 건드리지 않습니다([PRIVACY.md](PRIVACY.md)).
...
- **두뇌 drift 신선도 창(약 하루)**: 오래된 기록이 거짓 경고를 내지 않게 **신선도 창 밖의 옛 답/세션은 비교에서 제외**합니다. 그 대가로, *하루 넘게 쉬었다가 같은 세션을 재개*하면 **첫 새 응답이 나오기 전까지는 drift 경고가 조용히 억제**됩니다(의도된 trade-off: 거짓 경고보다 침묵). 새 답이 한 번 나오면 정상 비교로 돌아옵니다.
