...
- **상태바(무결성 경보)**: 검증이 필요했는데 **끝내 안 됐으면 빨강**(검증 미완, 확인 전까지 지속·신규 시 잠깐 점멸). **노랑은 두 종류** — Codex 결론이 통과·통과(보완)이 아니면(실패·보류·불가·정보부족) **‘Codex 결론 주의’**, 또는 검증 답의 **인용 근거가 실제 `파일:라인`과 불일치(없는 줄)하거나 이 검증 기록에서 다룬 흔적이 확인 안 되면 ‘근거 의심’**(해석 불가·모호한 인용은 거짓경보 방지로 건너뜀). 우선순위 = 빨강 > 노랑 > 진행. *(상태바 배경 채움색은 VS Code가 빨강/노랑만 허용 — 그래서 진행 단계는 글자색으로, 빨강/노랑은 경보 전용.)*
- **호버**: 세션 id·주제·연결시각·마지막 활동.
- **대시보드**(클릭): 연결 상태(Claude⇄Codex 모노그램·연결 라인), **검증 진행 스트립**([Claude]⟷[Codex] 방향·활성 박스), **무결성 배너**(빨강=검증 미완 / 노랑=Codex 결론 주의[통과 아님: 실패·보류·불가·정보부족]·근거 의심[불일치·미확인], ‘확인함’으로 해제), **검증 대화**(사용자 말풍선 + Codex 카드·판정 4색 칩[통과 / 통과(보완) / 보류 / 실패]·긴 글 "펼치기"), 후보 세션 목록(첫 발화로 식별)+`[연결]`·숨김/복원, 고정 계약 편집칸+체크박스, **검증 모드 세그먼트 토글**, **두뇌 설정**(모델·생각 강도, 계정 캐시 기반), **검증 대기시간**(기본 8분·1~60분, 전역), 접힌 **단계별 기본 원칙** 보기/수정/기본값 복원. 섹션마다 색 막대·구분선으로 경계 구분(장식 이모지 없음). `links.json`·`~/.codex/sessions`·진행/무결성 변경 자동 감지(여러 VS Code 창은 워크스페이스로 격리).
