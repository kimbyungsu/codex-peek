...
> 📐 **[전체 구조·검증 흐름 다이어그램 보기 → `docs/architecture.svg`](docs/architecture.svg)** — Claude(구현) ⇄ 브릿지(훅) ⇄ Codex(검증) · 검증 파이프라인 7단계 · 신뢰층(증명·변경 감지·재검증·근거 점검).

> **Claude(구현) ⇄ 브릿지(훅) ⇄ Codex(검증)** — 사람이 답을 옮기지 않는 구현→검증 루프. 검증이 '흉내'가 아님을 보장하는 층과, 지금 검증이 어느 단계인지 **상태바·대시보드에 라이브 표시**합니다.
