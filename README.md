...
사람이 두 에이전트 사이에서 답을 복사·전달하지 않아도, **세션을 고정**하고 **고정 계약(규약)을 매 턴 주입**하며, 원하면 **구현→검증 2트랙**을 하니스가 강제합니다. 기본 운용은 Claude Code(구현)↔Codex(검증)이고, 대시보드에서 Codex(구현)↔별도 Codex(검증)로 프로젝트별 전환할 수 있습니다.
...
- 대시보드 상단의 **Claude Code ↔ Codex**(기본) / **Codex ↔ Codex** 선택은 현재 프로젝트·현재 한/영 슬롯에 저장됩니다. 모드를 바꾸면 현황·검증 통계·고급설정의 구현자 명칭, 규칙 수신자, 전달 흐름, 작업 토큰 카드가 함께 바뀝니다.
- Codex ↔ Codex에서는 **현재 보이는 Codex 대화를 시작·재개할 때 구현 세션을 자동 고정**합니다. 사용 중인 Codex UI가 목록 클릭만으로 실제 `resume` 이벤트를 만들지 않는 경우 첫 프롬프트가 안전망으로 고정합니다. 대시보드에서 구현 세션을 다른 방으로 수동 우회 연결하지 않습니다. 검증자는 기본적으로 Claude Code ↔ Codex의 검증 세션을 실시간 공유하며(보라색 선·`동일 세션` 표지), 원할 때만 기존의 다른 세션을 C↔C 전용 검증자로 지정할 수 있습니다. 같은 세션의 양역할과 기존 구현 역할의 임의 탈취는 거부합니다.
- 규칙과 체크리스트는 운용 모드별·프로젝트별·한/영별로 분리됩니다. **단계별 기본 원칙과 3트랙 지도·일지·문서는 같은 프로젝트의 공용 자산**으로 유지됩니다.
- Codex 구현 훅은 사용자 실행층 `~/.codex/hooks.json`의 `SessionStart`/`UserPromptSubmit`/`PostToolUse`/`Stop`에서 대화 진입 자동고정, 구현 규칙, 전달 원칙, 검증 증명 게이트, 3트랙 플랜 게이트를 적용합니다. 구현 Codex는 자동 고정 당시 모델·추론강도와 최근 실제 응답을 모두 비교하고, 검증 Codex는 해당 운용 모드에 저장한 모델·추론강도와 비교해 무결성 경고를 냅니다.
- 확장 최초 활성화(또는 C↔C 모드 최초 선택) 때 Codex Peek 패키지나 사용자 실행 훅이 없으면 설치 내용을 검토하는 동의창이 자동으로 뜹니다. 명령 팔레트의 **Codex Bridge: Codex 구현 훅 설치**로도 다시 열 수 있습니다. 동의하면 기존 사용자 훅을 보존·백업한 뒤 네 명령을 병합합니다. 플러그인 번들 훅이 설정 목록에는 보이지만 실행되지 않는 Codex 버전이 있어 번들 훅은 중복 실행 방지를 위해 싣지 않고, `~/.codex/hooks.json`만 실행 권위로 사용합니다. 공식 `hooks/list`에서도 이 파일의 네 훅이 모두 신뢰됐는지 별도로 확인하며, 미신뢰 상태면 Codex 설정 → Hook에서 검토·신뢰하도록 자동 경고합니다. 신뢰 후 사용하려는 Codex 대화를 시작·재개하면 기존 구현 연결을 먼저 해제하지 않아도 그 대화로 구현 연결과 초록 표시가 자동 이동합니다. 목록 클릭이 실제 재개를 만들지 않으면 첫 프롬프트 rollout이 보조 고정합니다. 실제 lifecycle heartbeat가 최신 턴과 맞지 않으면 준비 완료로 표시하지 않고 닫을 수 없는 빨강 경보를 냅니다.
...
- 한 워크스페이스에는 검증을 **정확히 하나만** 실행합니다. 진행 중에는 문구가 다른 `ask`와 `--force-resend`도 차단하며, 새 세션은 긴 `session_meta`도 끝까지 판독해 생성 직후 연결합니다. 호출 창이 먼저 끊겨도 재전송하지 말고 대시보드/`ask-active status`를 확인하세요.
- **검증 대기시간은 고정 프리셋이 아닙니다.** 대시보드에 저장한 임의의 1~60분 값이 실제 검증 작업의 절대 deadline과 직접 호출 timeout에 반영됩니다. 긴 검증은 `ask-start`로 한 번만 시작하고 짧은 `ask-wait`를 반복하므로 외부 도구의 10분 실행창이 먼저 닫혀도 설정한 deadline까지 작업이 살아 있습니다.
...
- 이 검증 흐름의 핵심 지시문(아래 세 가지)은 **기본 지침(base directive)** — 사용자 고정 계약과 **별개로 하네스 최소 동작을 보장하는 고정 규약**입니다. 대시보드의 접힌 **단계별 기본 원칙** 섹션에서 **보기/수정/기본값 복원**할 수 있습니다(코드에 캐논 기본값 상존, 수정분은 `~/.codex-bridge/base-directive.json`·`base-directive.en.json` 오버라이드[언어별]; 비우면 기본값). **3트랙이면 같은 패널에 ④ 정찰 기본 원칙(정찰 AI 태도 — 수정 가능)과 ④-형식 계약(지도 ①~⑥ 양식 — 기계가 읽는 배선이라 잠금·내용만 공개) 칸이 늘어납니다.**
...
- **재검증 루프**: 검증 작업은 동시에 하나만 두고, 실패 뒤 수정하면 다음 회차를 순서대로 진행합니다. 사용자가 정한 왕복 상한에 닿으면 새 검증은 만들지 않고 **마지막 회차 지적만** `수용·처리 / 근거 있는 반박·종결 / 영수증 있는 보관함 이관 / 사용자 판단 필요`로 한 번씩 분류합니다. 각 항목은 키로 시작하는 한 줄이며, 수용의 변경·확인·근거와 반박의 관측·이유·근거는 **각 칸 자체에** 식별 가능한 파일·설정·시험·측정값이 있어야 합니다. 보관함은 **그 지적과 제목이 같은 열린 영수증**이 있어야 합니다. 앞의 세 갈래는 구현 담당이 정리하고, 마지막 갈래가 있을 때만 질문 하나로 묶습니다. 이 **검증 상한 마감문**을 끝내 쓰지 못한 경우에만 별도 빨강 **‘검증 상한 마감 누락’**을 남기며, 질문이 없어 자동 정리돼도 검증 통과로 표시하지 않습니다. **검증 후 또 고치면 다시 검증**을 강제 → ‘검증받은 상태 = 최종 상태’.
...
- **상태바(무결성 경보)**: **빨강 세 종류** — 검증이 필요했는데 **끝내 안 됐으면 ‘검증 미완’**(확인 전까지 지속), Codex 결론이 **‘검증 실패’면 ‘검증 실패’**(고쳐 재검증해 통과하면 자동 사라짐), 왕복 상한 뒤 마지막 지적의 네 갈래 행선지를 정리하지 못하면 **‘검증 상한 마감 누락’**(마감문을 완성하면 자동 사라짐). 마지막 빨강은 정상적인 상한 도달 자체가 아니라 **AI가 맡아야 할 분류까지 빠진 상태**이므로 그냥 지나치면 안 됩니다. **노랑 세 가지** — Codex 결론이 **보류·불가·정보부족이면 ‘Codex 보류·불가’**, 검증 답의 **인용 근거가 실제 `파일:라인`과 불일치(없는 줄)거나 이 기록에서 다룬 흔적이 확인 안 되면 ‘근거 의심’**, 설정한 **모델/생각강도가 최근 실제 응답과 다르면 ‘두뇌 설정 어긋남’**. 빨강이 있어도 함께 뜬 노랑 건수를 같이 표기. 우선순위 = 빨강 > 노랑 > 진행.
- **상태바·대시보드(두뇌 drift 경고)**: **의도한 모델·생각강도가 최근 실제 응답과 어긋나면** 노랑 ‘두뇌 설정 어긋남’으로 알립니다. 검증 Codex의 모델·추론강도 설정은 프로젝트뿐 아니라 **운용 모드별**로 분리되며, C↔C 전용값이 없으면 Claude 모드 설정을 실시간 상속합니다. C↔C 구현 Codex도 자동 고정 당시 모델·추론강도와 최근 rollout을 모두 비교합니다. **Claude Code만 모델 계열만 비교**합니다 — 실제 추론강도 레벨이 대화기록에 없어 정직하게 판단할 수 없기 때문입니다. Claude의 모델 의도는 해당 프로젝트 대화의 마지막 `/model` 또는 포커스 창의 모델 피커 선택(`cc-intent.json`)에서 읽으며, 다른 창의 전역 설정 변화는 거짓 경고를 피하려고 의도로 간주하지 않습니다. 비교 대상은 대시보드 기준 폴더의 같은 대화와 7일 이내 실제 응답으로 제한합니다.
- **정찰(3트랙) 용어 한눈에** — 대시보드의 정찰 구역은 하나의 흐름입니다(각 단계에 LLM 사용 여부 배지):
  | 단계 | 무엇 | LLM | 비용 |
  |---|---|---|---|
  | ① 변경 감지 | 지금 바뀌는 파일 + 과거에 함께 바뀐 통계(옛 표기 '범위 장부') | ⚙ 없음 | 0 |
  | ② 영향지도 | 이 변경이 어디까지 번질지 정찰 AI가 미리보기(옛 표기 '영향지도 게시판') | ⚡ 호출 | Claude 정찰=별도 과금 없음(쓰던 사용량 범위 · 클로드-코덱스 모드의 기본) / DeepSeek 정찰은 키 / Codex 정찰은 쓰는 Codex 계정 사용량 범위(코덱스-코덱스 모드의 기본) |
  | ③ 관찰 일지 | 검증을 지나며 맞은 것·틀린 것이 자동 축적(옛 표기 '관측 장부'·'MAP 장부') | ⚙ 추가 호출 없음 | 0 (검증 대화에 편승) |
  | ④ 확정 교범 | 원할 때만 도장 찍어 저장소 문서(docs/MAP.md)로 — 안 써도 ①~③은 그대로 자동. **전환(P3b cutover — 수동 명령) 후에는**: 기존 문서는 동결(이관 소스 배너)되고 신규 승인은 Project MAP 경로로만 들어가요 — "문서를 건드리지 않는다"는 약속은 전환 전까지의 서술이며, 전환은 사용자가 직접 실행할 때만 일어납니다 | 👤 선택 | — |

  적재·승격·강등은 전부 자동입니다. 사람의 직접 확인/정정과 고정/차단/교범 기록은 실제 사정을 아는 경우에만 쓰는 선택적 예외이며, 누르지 않아도 자동 흐름은 멈추지 않습니다. 일지의 **검증됨**은 다음 정찰 자료에 우선 동봉된다는 뜻이지 코드나 Project MAP에 자동 적용됐다는 뜻이 아닙니다. 대시보드는 기계 확인 기록·승격 가능한 증거·파일 취급 확인 불가를 나눠 보여줍니다. 상태바 호버에는 "지금 실행 중인 LLM 호출" 여부가 상시 표시됩니다.
- **호버**: 세션 id·주제·연결시각·마지막 활동.
- **대시보드**(클릭): 연결 상태(Claude⇄Codex 모노그램·연결 라인), **검증 진행 스트립**([Claude]⟷[Codex] 방향·활성 박스), **무결성 배너**(빨강=검증 미완·Codex 검증 실패 / 노랑=Codex 보류·불가[정보부족 등]·근거 의심[불일치·미확인]·두뇌 설정 어긋남, 각 줄은 빨강/노랑 점으로 분리, ‘확인함’으로 해제), **검증 대화**(사용자 말풍선 + Codex 카드·판정 4색 칩[통과 / 통과(보완) / 보류 / 실패]·긴 글 "펼치기"), 후보 세션 목록(첫 발화로 식별)+`[연결]`·숨김/복원, 고정 계약 편집칸+체크박스, **검증 모드 세그먼트 토글**, **코덱스 두뇌 설정**(모델·생각강도 *선택*, 계정 캐시 기반 — Claude 쪽은 카드 없이 위 두뇌 drift 경고만), **검증 대기시간**(기본 8분·1~60분, 전역), **트랙 세그먼트**(2트랙=구현↔검증 기본 / 3트랙=+정찰 — 위 '정찰(3트랙) 용어 한눈에'의 4단계 흐름[①변경 감지 ②영향지도 ③관찰 일지 ④확정 교범]이 대시보드에 켜집니다. 프로젝트별 저장 · 관찰(advisory) 중심 — 단 하나의 예외는 **플랜 게이트**(3트랙에서 기본 켜짐, 2026-07-09 승격): 영향지도가 없거나 낡으면 플랜 확정 전에 먼저 지도를 요청하되 세션당 2회까지만 막고 이후 통과(fail-open — 오류·판정불가면 절대 안 막음), 안내에는 그 프로젝트의 관찰 신호가 함께 실리며, `node "<브릿지 홈>/scope-gate.js" <프로젝트> off`로 언제든 끌 수 있습니다(그 외에는 아무것도 막거나 강제하지 않음) · 외부로 나가는 경로는 네 갈래 — Codex 검증을 보낼 때(2트랙 기본 동작: 검증 요청문+자동 동봉[지도 조각·결합 확인 문안·하네스 기본 지침]이 쓰시던 codex CLI로 전달)와, DeepSeek 키 등록 시(②의 DeepSeek 정찰 꾸러미+3트랙 켤 때 연결 점검 1회, 키 등록=동의)와, Claude 정찰 실행 시 같은 꾸러미가 **쓰시던 Claude CLI를 통해 Claude 서비스로** 전달되는 것(별도 결제 없음 — 검증이 Codex로 가는 것과 같은 성격), 그리고 Codex 정찰 선택·실행 시 같은 꾸러미가 **쓰시던 codex CLI를 통해 Codex 서비스로** 전달되는 것(검증과 분리된 독립 실행 1회·읽기 전용 강제 — 계정 사용량 범위, [PRIVACY.md](PRIVACY.md)). **전제: ①의 함께-변경 통계는 로컬 git '커밋 이력'을 읽습니다 — git 저장소가 아니거나 커밋이 거의 없으면 통계만 "데이터 없음"이 정상이고, ② 지도는 '무이력 모드'(최근 수정 파일 기준)로 여전히 만들 수 있습니다. 정찰 대상 자기진단: 세션 폴더와 실제 개발 레포가 다르면(예: 문서 폴더를 열고 다른 레포를 개발) 검증 인용의 레포 귀속을 근거로 어긋남을 스스로 감지해 — 자동 지시가 에이전트에게 대상 지정을 안내하고, 대시보드에 원클릭 설정 카드가 뜨며, 3트랙을 켜는 순간에도 대상 확인 스텝이 나옵니다(모두 advisory·현재 언어 슬롯에 저장)**), **Project MAP 자동 생성(P1)**: 3트랙을 켤 때 모달로 동의하면 — 또는 `node "<브릿지 홈>/scope-map.js" <저장소> bootstrap`을 직접 실행하면(실행=동의) — 정찰 대상 저장소 안에 구조 지도 폴더(`project-map/`)가 백그라운드에서 자동 생성됩니다(대화 비차단·결정론 파일 스캔·LLM 호출 0·외부 전송 0 — 동의 전에는 어떤 자동 생성도 없음, 2트랙은 완전 무관, [PRIVACY.md](PRIVACY.md)), **자동 의미 보강(P8)**: '의미 보강 담당'(기본 Claude/경제형 DeepSeek/정밀형 Codex/자동형)을 정하고 **자동 실행에 별도 동의**하면(기본 담당=대시보드 1클릭·유료 담당=선택 시 모달 — 예전 선택·3트랙 동의만으로는 자동 실행되지 않음) 새 지도·소스 변경 관측 때 지도 초안+소스 발췌가 그 담당으로 전송돼 지도의 의미(근거·확신·연결)가 백그라운드에서 채워집니다. 결과는 지도 수정 파이프라인(제안→검증→적용)을 그대로 거치고, 의미 판단이 필요한 항목은 Codex 판정을 받거나 보류로 표시됩니다(라우팅·비용 감사는 통계 장부에 60일 — PRIVACY.md). 담당에게 보내는 소스 발췌는 '이번에 바뀐 부분 주변'(git 기준점 대비 변경 줄 앞뒤, git 없거나 새 파일이면 첫머리 폴백+제목 표기)이고 담당 답의 근거 인용은 보낸 발췌 안의 원문만 인정됩니다 · **발췌 범위**(담당 줄 아래): 보내는 파일 수와 파일당 글자를 프로젝트마다 고를 수 있어요 — 프리셋 가벼움 8/2,000 · 권장 10/3,000(기본값) · 대형 15/4,000 · 최대 20/4,000, 상한 밖 값은 저장되지 않으며 수치는 현황 카드의 인용 측정을 보고 직접 조정합니다(벤치 결과 아님) · **다시 묻기**: 답이 거부돼 다시 묻는 호출(자동 재시도·무인 재개·'다시 시도' 버튼)에는 지난 답에서 제외된 항목의 기계 분류(단계·코드·항목 색인·종류)만 붙고 파일 내용이나 답의 문장은 다시 보내지 않습니다 — 호출 횟수는 늘지 않습니다, **지도 수정 파이프라인(P2 — 비활성)**: 지도를 고치는 제안→분류→적용→복구·정리 절차 본체가 이 판에 들어 있지만(스키마·검증·적용·복구·GC·명령까지) 자동으로는 돌지 않습니다(수동 명령+명시 플래그 --pre-cutover 전용 — 전체 전환은 후속 단계에서 한 번에, 결정 기록은 저장소 `project-map/decisions/`에 · 하네스 홈의 작업 기록은 정리 명령이 오래된 순으로 비우며 보관 상한은 CODEX_BRIDGE_MAP_GC_KEEP 환경변수로 조정[기본 200건]), **권위 전환 준비(P3a — 준비만)**: 옛 확정 장부(docs/MAP.md)와 구조 지도를 잇는 대응표를 준비합니다 — 훑기(legacy-scan)는 저장소를 읽기만 하고, 대응 확정(binding-confirm — 사용자가 직접 실행할 때만)이 저장소 `project-map/bindings.json`을 만듭니다(전환 자체는 후속 단계에서 한 번에 — 지금은 기존 동작 무변경), 접힌 **단계별 기본 원칙** 보기/수정/기본값 복원. 섹션마다 색 막대·구분선으로 경계 구분(장식 이모지 없음). `links.json`·코덱스 세션·Claude 설정/대화기록·진행/무결성 변경 자동 감지(여러 VS Code 창은 워크스페이스로 격리).
- **Project MAP 정책·복구(P9, 3트랙 전용)**: 자동 의미 보강이 끝나면 유효한 사용자 정책과 맞는 대기 변경은 P2 파이프라인을 통해 자동으로 이어지고, 반대 정책끼리 충돌할 때만 대시보드의 **Project MAP 대기 선택·복구** 카드가 뜻을 묻습니다. 선택은 먼저 로컬 복구 장부에 보존되고 새 정책·decision은 저장소 `project-map/policies/`·`decisions/`에 공유 기록됩니다. 자동 마무리가 안전하게 보류되면 대시보드의 명시 재시도만 같은 선택/위임을 다시 진행합니다. 정책을 교체하거나 해제(revoke)해도 옛 파일을 삭제하지 않고 supersession/`.revoke.json` 이력으로 남깁니다. 손상 복구는 ‘복구본 만들기’와 ‘백업 후 교체’의 2단 확인이며, 확인 계획은 nonce·원본/후보/복구본 지문에 결속되고 교체 중 종료돼도 다음 판독에서 이어집니다. 2트랙에서는 모달 뒤 전환된 경우까지 core API가 다시 확인해 이 채널과 자동 sweep을 모두 멈춥니다. 위 긴 대시보드 설명의 옛 **“P2 — 비활성”** 문구는 P9 이전 이력으로, 현재 동작은 이 항목이 우선합니다.
- **Project MAP 검증 확인 대기**: 자동 의미 보강 중 검증 연결이 없거나 판정이 불충분한 고급 의미 변경은 별도 확인 대기 장부로 분리됩니다. 그 항목 때문에 기본 지도나 독립된 다음 항목이 멈추지 않으며, 대시보드에는 적용·확인 대기·거절·조사 필요 수가 따로 표시됩니다. 일반 관측 때 검증을 반복 호출하지 않고, 사용자의 명시 재시도 또는 새 검증 연결 때만 정해진 조건으로 다시 확인합니다.
- **Project MAP 운영 현황(P10, 3트랙 전용)**: 검증 통계 탭에서 현재 지도의 최신 근거/갱신 필요/확인 불가/임시 제외, 최근 28일 자동 의미 보강의 고유 실행·작업 상태와 분모가 드러난 자동 완료 비율, 현재 선택·재시도·복구 대기, 영향지도·전역 준비 점검·의미 보강·검증 담당 판정별 외부 호출을 따로 보여줍니다. 실측 토큰이 없는 호출은 0토큰으로 꾸미지 않고 입력/출력 글자 수로 분리합니다. 수치는 로컬에서 기록에 성공한 사건만 반영하며 청구서나 지도 정확도 점수가 아닙니다. 원장은 60일 보존하고 화면은 28일만 집계하며, 2트랙에서는 카드뿐 아니라 두 통계 파일과 지도 판독도 실행하지 않습니다.
- **대형·장기 프로젝트 여유**: 구조 초안은 의존성·빌드 산출물을 제외한 뒤 최대 10만 항목·20단계 폴더 깊이까지 훑고, 근거 최신성 기준선 1만 개·관찰 일지 약 1만 사건·영향지도 원본 최근 30장을 유지합니다. 관찰 일지 화면 목록은 최근 12개만 보여도 상태 수·관찰 신호·자동 선별은 전체 일지를 계산합니다. 한 번의 AI 자료 꾸러미는 관련도 높은 일부만 싣는 제한을 유지합니다(저장 손실이 아니라 긴 입력에 핵심 근거가 묻히는 것을 막는 선별 폭). 어느 스캔이든 상한에 닿으면 완전하다고 꾸미지 않고 판정 불가/부분 범위로 표시합니다.
- **관찰 일지의 자동 승격**: 과거 기록을 시간이 지났다는 이유나 사용자가 반박하지 않았다는 이유만으로 맞다고 바꾸지 않습니다. 대신 뒤의 검증이 일지 결합을 명시적으로 골라 그 결합의 정확한 두 파일을 실제로 읽고 인용하면 강한 확인 1회로 `검증됨`이 됩니다. 결합을 고르지 않은 채 답에 두 파일이 함께 인용된 약한 신호와 구형 확인 기록은 서로 다른 실행 2회가 필요합니다. 영향지도가 없거나 불량이어도 일지 결합 재확인은 지도 원문 없이 독립적으로 전달됩니다.
...
- **언어(한국어/English)**: 대시보드 탭바 우측 토글로 전환합니다. **전역 설정 하나**(`~/.codex-bridge/language.json`)라 모든 프로젝트·창에 함께 적용되고, 첫 실행 시 VS Code UI 언어를 따라 초기화됩니다. 전환 시 **UI 문자열 + 주입 지침(기본 지침·검증 directive·훅 차단문) + 판정 형식**이 함께 바뀌며, 판독기는 언어 설정과 무관하게 한/영 판정 줄(`검증: 통과` / `Verdict: pass`)을 항상 둘 다 인식합니다. **프로젝트 규칙·기본지침 오버라이드는 언어별 슬롯**에 따로 저장됩니다 — 한국어 슬롯은 기존 파일 그대로(무손실), 영어는 별도 파일(`<키>.en.json`·`base-directive.en.json`). **정찰 부속 설정(게이트·정찰 대상 폴더)과 정찰 기본 원칙(`scout-baseline.en.json`)도 언어 슬롯별**이고(단 정찰 대상 폴더는 '사실'이라 현재 슬롯이 비면 반대 슬롯 값을 상속 — 현재 슬롯에 지정하면 독립), **정찰 프롬프트·지도 원문 언어도 전역 언어를 따릅니다**(영어 모드에서 만든 지도는 영어). 언어를 바꿨을 때 그 언어 칸이 비어 있으면 "다른 슬롯에 규칙이 있다"는 안내가 떠서 사라진 것으로 오해하지 않게 합니다.
- **검증 통계 탭** (대시보드 상단 `[현황]` / `[검증 통계]` 토글): 이 폴더의 검증 활동을 한 화면에 — 최근 7일 **검증 횟수·완전통과율·보완이상 비율·전환(실패·보류 뒤 통과)**, 28일 **verdict 분포**(도넛 + 색별 가로막대, ‘판정표지 누락’=Codex가 통과/실패 결론 줄을 안 적은 답은 통과율 분모서 제외), 14일 **추이**, 4주 **활동 히트맵**, 검증·작업 **토큰**, **프로젝트별 비교**, **3트랙 기여(관찰 신호)**를 보여줍니다. 3트랙에서는 여기에 **Project MAP 운영 현황**이 더해져 현재 지도 상태, 최근 자동 의미 보강, 현재 선택·복구 대기, **목적별 외부 호출·사용량**, 자료 coverage와 한계를 순서대로 표시합니다. Project MAP 수치는 실제 정찰 대상 저장소 기준의 로컬 기록이며, 28일 화면/60일 원장이고 서비스 청구서나 정확도 점수가 아닙니다. 기존 검증 통계의 ‘이 폴더’ 귀속과 rollout 토큰 근사 표시는 그대로 유지됩니다.
...
### GitHub 설치 파일 (전체 기능 권장)

개발 도구를 따로 내려받거나 버전 숫자를 올리지 않아도 되는 설치 묶음입니다.

1. **[codex-peek-0.1.102-installer.zip 내려받기](https://github.com/kimbyungsu/codex-peek/releases/download/v0.1.102/codex-peek-0.1.102-installer.zip)** → 압축 풀기.
...
이 경로는 확장·브릿지·훅과 개발용 스크립트(회고·재조정·동기화)까지 함께 둡니다 — 정찰(3트랙) 실행층은 마켓/VSIX 설치본에도 똑같이 들어 있습니다(2026-09-16 이관). 이미 `0.1.102`이 설치된 PC에서도 설치기가 기존 확장 폴더를 강제 삭제하지 않고, 바뀐 런타임만 백업한 뒤 같은 버전에 안전하게 갱신합니다. 비표준 `extensions` 폴더를 쓰는 환경만 `CODEX_BRIDGE_EXTENSIONS_DIR`로 그 폴더를 지정하면 됩니다.

확장만 빠르게 설치하려면(검증·대시보드·정찰 3트랙 모두 동작) **[codex-bridge-0.1.102.vsix](https://github.com/kimbyungsu/codex-peek/releases/download/v0.1.102/codex-bridge-0.1.102.vsix)**를 받아 VS Code의 `확장: VSIX에서 설치...`로 여세요. 같은 `0.1.102`을 이미 쓰는 PC는 VSIX 강제 재설치 대신 위 전체 설치 묶음을 사용해야 안전하게 최신 코드가 반영됩니다. 파일 지문은 [v0.1.102 릴리스 페이지](https://github.com/kimbyungsu/codex-peek/releases/tag/v0.1.102)의 `SHA256SUMS.txt`에서 확인할 수 있습니다.
...
3. 검증이 동작합니다 — Claude Code는 설정 파일 변경을 감시하므로 **그 파일을 가지고 시작한 세션이면 재시작 없이** 반영됩니다. 다만 설정 파일이 아예 없던 상태에서 시작한 세션은 그 파일을 감시하지 않으니 **새 세션**이 필요합니다. (알림을 닫았어도 명령 팔레트 `Codex Bridge: Claude Code 검증 훅 설치`로 언제든 다시 실행)
...
5. **정찰(3트랙)도 마켓 설치본만으로 동작합니다**(2026-09-16): 정찰 실행층(실행기 3종·꾸러미 수집·보관·대상 지정·게이트·지도 명령·일지 도구)이 브릿지 엔진과 함께 브릿지 홈에 배치되고, 훅·대시보드의 실행 안내도 그 경로를 가리킵니다 — 예: `node "<브릿지 홈>/scope-scout-self.js" <프로젝트>`. 저장소에만 있는 것은 개발용 도구(회고·재조정·동기화)뿐입니다. 정찰 담당의 기본값은 모드를 따릅니다 — 클로드-코덱스는 Claude 정찰(무과금), 코덱스-코덱스는 Codex 정찰. 담당의 명령줄(또는 DeepSeek 키)이 없으면 대시보드가 '준비 안 됨'과 사유를 보여 주고, 플랜 게이트와 자동 지시는 따라 할 수 없는 실행을 요구하지 않고 통과합니다(2026-09-17).

> 이미 레포 한방 설치(`install.js`)를 쓴 경우: 설치기는 확장의 배치 표식을 지우고, 확장은 다음 활성화 때 브릿지 파일을 자기 번들과 대조합니다 — **같은 세대면 관리 모드로 승격해 다음 업데이트부터 자동 동기화**하고, 무수정 설치본이 구세대면 안전하게 전체 동기화하며, 수정 흔적이 있으면 덮지 않고 경보와 함께 빠진 파일만 채웁니다(2026-08-12 혼합 세대 사고 뒤 '무조건 존중' 규칙 폐기).

### Git clone 후 한방 설치 (개발·최신 소스용)
...
- **자체 서버 전송 없음**: 이 확장·브릿지가 따로 데이터를 보내는 서버는 없습니다. 외부로 나가는 경로는 네 갈래 — ⑴ **Codex 검증을 보낼 때(2트랙 기본 동작)**: 검증 요청문과 자동 동봉(지도 조각·결합 확인 문안·하네스 기본 지침)이 **쓰시던 codex CLI를 통해 Codex 서비스로** 전달 ⑵ **DeepSeek 키 등록 시**: ① 탐색(3트랙)의 DeepSeek 지도 생성이 **실행될 때** 자료 꾸러미(직접 실행하거나, 3트랙 자동 지시를 받은 구현 담당(운용 모드에 따라 Claude 또는 구현 Codex)이 실행 — 키 등록=동의 · 확장·훅 자체는 꾸러미를 전송하지 않음) ② 3트랙을 켤 때 **연결 점검 요청 1회**(꾸러미 아님) ⑶ **Claude 정찰(키 불필요 · 클로드-코덱스 모드의 기본 담당) 실행 시**: 같은 꾸러미가 **쓰시던 Claude CLI를 통해 Claude 서비스로** 전달됩니다(별도 결제 없음 — 검증이 사용자의 codex CLI로 Codex에 가는 것과 같은 성격) ⑷ **Codex 정찰(대시보드에서 탐색 담당을 Codex로 선택한 경우) 실행 시**: 같은 꾸러미가 **쓰시던 codex CLI를 통해 Codex 서비스로** 전달됩니다(검증과 분리된 독립 실행 1회·읽기 전용 강제 — 쓰시는 Codex 계정 사용량 범위). 무엇을 보내고 무엇을 자동 제외하는지는 [PRIVACY.md](PRIVACY.md)에 명시돼 있습니다.
...
- 검증 모드는 opt-in이며 Stop 훅의 같은 안내는 한 턴에 최대 3회까지만 반복합니다. 검증 왕복 상한에 닿으면 새 검증 대신 마지막 지적의 네 갈래 마감문을 요구하고, 사용자 판단 항목이 있을 때만 질문합니다. 마감문마저 끝내 작성되지 않을 때만 누락 빨강을 남긴 뒤 종료를 허용해 무한정지를 막습니다.

### 자세한 정책 문서
- [SECURITY.md](SECURITY.md) — 보안 경계인 것 / 아닌 것, 설치기가 바꾸는 것, 신고 방법
- [PRIVACY.md](PRIVACY.md) — 정확히 어떤 파일을 읽고 쓰는지, 외부 전송 경로 네 갈래(Codex 검증 시 요청문+자동 동봉 / DeepSeek 키 시 꾸러미+연결 점검 / Claude 정찰 시 Claude CLI 경유 꾸러미 / Codex 정찰 선택 시 codex CLI 경유 꾸러미)의 내용·제외·조건, 데이터 삭제
...
- **자체 서버 전송 없음 / 읽기 전용 원칙**: 확장·브릿지가 따로 보내는 곳은 없고, 외부로 나가는 것은 Codex 검증 전송(요청문+자동 동봉)과 정찰 관련뿐입니다 — 정찰 실행 시의 꾸러미(DeepSeek 키 등록 시 DeepSeek API로 / Claude 정찰은 쓰시던 Claude CLI 경유 / Codex 정찰 선택 시 쓰시던 codex CLI 경유)와 DeepSeek 키 등록 시 3트랙 켤 때의 연결 점검 1회([PRIVACY.md](PRIVACY.md)에 전송·제외·조건 명시). 코덱스 원본 기록은 *직접 "영구삭제"를 누른 한 파일* 외엔 건드리지 않습니다.
...
- **무한정지 방지 상한**: 검증 왕복과 Stop 안내 반복은 다른 숫자입니다. 왕복이 사용자 상한에 닿으면 검증 호출은 끝내되 마지막 지적의 네 갈래 마감문을 강제하고, 같은 안내를 세 번 받고도 마감문을 쓰지 못한 경우에만 **‘검증 상한 마감 누락’ 빨강**으로 남긴 뒤 종료를 허용합니다.
