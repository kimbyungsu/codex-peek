...
- **정찰(3트랙) 용어 한눈에** — 대시보드의 정찰 구역은 하나의 흐름입니다(각 단계에 LLM 사용 여부 배지):
  ...
  | ② 영향지도 | 이 변경이 어디까지 번질지 정찰 AI가 미리보기(옛 표기 '영향지도 게시판') | ⚡ 호출 | 기본 정찰(Claude)=별도 과금 없음(쓰던 사용량 범위) / DeepSeek 정찰은 키 |
...
- **검증 통계 탭** (대시보드 상단 `[현황]` / `[검증 통계]` 토글): 이 폴더의 검증 활동을 한 화면에 — 최근 7일 **검증 횟수·완전통과율·보완이상 비율·전환(실패·보류 뒤 통과)**, 28일 **verdict 분포**(도넛 + 색별 가로막대, ‘판정표지 누락’=Codex가 통과/실패 결론 줄을 안 적은 답은 통과율 분모서 제외), 14일 **추이**(완전통과/통과보완/보류/실패/표지누락 5색 스택), 4주 **활동 히트맵**(요일×시간), **토큰**(연결 코덱스 세션 누적 · 이 폴더 클로드 작업 토큰+턴수 · 모델·추론강도별 · 검증모드별), **프로젝트별 비교**(이건 전체 폴더 28일 group-by — ‘이 폴더’ 통계와 별개 섹션), **3트랙 기여(관찰 신호)**(지도가 발견·재동봉·확인된 결합과 틀림/복권 — 일지 이벤트 합계·추가 LLM 0·3트랙일 때만) + **정찰(3트랙) 비용**(28일 — DeepSeek 지도·연결 점검은 실측 토큰, 기본 정찰(Claude)은 글자 수만[토큰 미제공·별도 결제 없음] · 지도 10장 보관과 무관한 60일 장부). ‘이 폴더’ 통계는 모두 **연 폴더 기준**(작업 cwd가 흔들려도 일관)·**메타만 기록**(프롬프트/답 원문 미저장)·로컬 집계. 모델·1회 토큰은 rollout 마지막 턴 기준 *근사*로 정직하게 표기합니다.
...
- **자체 서버 전송 없음**: 이 확장·브릿지가 따로 데이터를 보내는 서버는 없습니다. 외부로 나가는 경로는 두 갈래 — ⑴ **DeepSeek 키 등록 시**: ① 탐색(3트랙)의 DeepSeek 지도 생성이 **실행될 때** 자료 꾸러미(직접 실행하거나, 3트랙 자동 지시를 받은 Claude가 실행 — 키 등록=동의 · 확장·훅 자체는 꾸러미를 전송하지 않음) ② 3트랙을 켤 때 **연결 점검 요청 1회**(꾸러미 아님) ⑵ **기본 정찰(키 불필요) 실행 시**: 같은 꾸러미가 **쓰시던 Claude CLI를 통해 Claude 서비스로** 전달됩니다(별도 결제 없음 — 검증이 사용자의 codex CLI로 Codex에 가는 것과 같은 성격). 무엇을 보내고 무엇을 자동 제외하는지는 [PRIVACY.md](PRIVACY.md)에 명시돼 있습니다.
...
- [PRIVACY.md](PRIVACY.md) — 정확히 어떤 파일을 읽고 쓰는지, 외부 전송 경로 두 갈래(DeepSeek 키 시 꾸러미+연결 점검 / 기본 정찰 시 Claude CLI 경유 꾸러미)의 내용·제외·조건, 데이터 삭제
...
- **자체 서버 전송 없음 / 읽기 전용 원칙**: 확장·브릿지가 따로 보내는 곳은 없고, 외부로 나가는 것은 정찰 관련뿐입니다 — 정찰 실행 시의 꾸러미(DeepSeek 키 등록 시 DeepSeek API로 / 기본 정찰은 쓰시던 Claude CLI 경유)와 DeepSeek 키 등록 시 3트랙 켤 때의 연결 점검 1회([PRIVACY.md](PRIVACY.md)에 전송·제외·조건 명시). 코덱스 원본 기록은 *직접 "영구삭제"를 누른 한 파일* 외엔 건드리지 않습니다.
