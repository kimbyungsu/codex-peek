...
- **외부 전송은 기본적으로 없음**: codex 호출은 사용자의 codex CLI를 로컬에서 실행할 뿐, 별도 서버로 데이터를 보내지 않습니다. **유일한 예외**는 탐색(3트랙)의 DeepSeek 지도 생성이 **실행될 때**입니다 — 직접 실행하거나, 3트랙 자동 지시를 받은 Claude가 실행(키 등록=동의 · 확장·훅 자체는 전송하지 않음). 무엇을 보내고 무엇을 자동 제외하는지는 [PRIVACY.md](PRIVACY.md)에 명시돼 있습니다.
...
- [PRIVACY.md](PRIVACY.md) — 정확히 어떤 파일을 읽고 쓰는지, 외부 전송(기본 없음·예외 1건 명시), 데이터 삭제
...
- **외부 전송 기본 없음 / 읽기 전용 원칙**: 기본 동작의 모든 데이터는 로컬에 머물고(유일한 예외: DeepSeek 지도 생성 실행 시 — 직접 또는 3트랙 자동 지시 경유, [PRIVACY.md](PRIVACY.md)에 전송·제외·조건 명시), 코덱스 원본 기록은 *직접 "영구삭제"를 누른 한 파일* 외엔 건드리지 않습니다.
