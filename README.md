...
- **체크(ON)** — 파일을 변경한 턴은 `verify-guard`(Stop 훅)가 Claude의 종료를 막고, Claude가 `codex-bridge ask`로 Codex 검증을 받아 그 결과(통과/실패+근거)를 반영해 보고하도록 강제합니다. 사람이 두 모델 사이를 중계하지 않습니다.
- **해제(OFF)** — 2트랙 미발동. 평소 Claude 그대로(파일을 바꿔도 Codex 호출 안 함).
