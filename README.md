...
1. **[codex-peek-0.1.100-installer.zip 내려받기](https://github.com/kimbyungsu/codex-peek/releases/download/v0.1.100/codex-peek-0.1.100-installer.zip)** → 압축 풀기.
...
이 경로는 확장·브릿지·훅과 정찰(3트랙) 스크립트까지 함께 둡니다. 이미 `0.1.100`이 설치된 PC에서도 설치기가 기존 확장 폴더를 강제 삭제하지 않고, 바뀐 런타임만 백업한 뒤 같은 버전에 안전하게 갱신합니다. 비표준 `extensions` 폴더를 쓰는 환경만 `CODEX_BRIDGE_EXTENSIONS_DIR`로 그 폴더를 지정하면 됩니다.

확장과 2트랙 검증만 빠르게 설치하려면 **[codex-bridge-0.1.100.vsix](https://github.com/kimbyungsu/codex-peek/releases/download/v0.1.100/codex-bridge-0.1.100.vsix)**를 받아 VS Code의 `확장: VSIX에서 설치...`로 여세요. 같은 `0.1.100`을 이미 쓰는 PC는 VSIX 강제 재설치 대신 위 전체 설치 묶음을 사용해야 안전하게 최신 코드가 반영됩니다. 파일 지문은 [v0.1.100 릴리스 페이지](https://github.com/kimbyungsu/codex-peek/releases/tag/v0.1.100)의 `SHA256SUMS.txt`에서 확인할 수 있습니다.
...
5. **알아둘 것(현 단계 한계)**: 마켓 설치본에는 정찰(3트랙) 실행 스크립트(`scripts/`)가 포함되지 않습니다 — 기본(Claude) 정찰 실행과 그 '준비됨' 표시는 레포 설치(`git clone` 후 `node install.js`)에서만 가능하고, 마켓 설치본의 준비 점검은 이 상태를 사유와 함께 정직하게 '미준비'로 보여줍니다. 검증(2트랙)과 대시보드는 마켓 설치본만으로 완전 동작합니다.
