...
5. **정찰(3트랙)도 마켓 설치본만으로 동작합니다**(2026-09-16): 정찰 실행층(실행기 3종·꾸러미 수집·보관·대상 지정·게이트·지도 명령·일지 도구)이 브릿지 엔진과 함께 브릿지 홈에 배치되고, 훅·대시보드의 실행 안내도 그 경로를 가리킵니다 — 예: `node "<브릿지 홈>/scope-scout-self.js" <프로젝트>`. 저장소에만 있는 것은 개발용 도구(회고·재조정·동기화)뿐입니다. 기본 정찰 담당(Claude)은 `claude` 명령줄이 있어야 하며, 없으면 정찰 실행이 'cli-not-found'로 실패해 그대로 보고됩니다(코덱스-코덱스 모드의 기본 담당과 정찰 전용 준비 표시는 다음 묶음에서 정리).
