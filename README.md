...
- **대시보드**(클릭): 연결 상태(Claude⇄Codex 모노그램·연결 라인), **검증 진행 스트립**([Claude]⟷[Codex] 방향·활성 박스), **무결성 배너**(빨강=검증 미완 / 노랑=근거 의심·근거 미확인, ‘확인함’으로 해제), **검증 대화**(사용자 말풍선 + Codex 카드·통과/실패 칩·긴 글 "펼치기"), 후보 세션 목록(첫 발화로 식별)+`[연결]`·숨김/복원, 고정 계약 편집칸+체크박스, **검증 모드 세그먼트 토글**, **두뇌 설정**(모델·생각 강도, 계정 캐시 기반), **검증 대기시간**(기본 8분·1~60분, 전역), 접힌 **단계별 기본 원칙** 보기/수정/기본값 복원. 섹션마다 색 막대·구분선으로 경계 구분(장식 이모지 없음). `links.json`·`~/.codex/sessions`·진행/무결성 변경 자동 감지(여러 VS Code 창은 워크스페이스로 격리).
