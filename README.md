...
- [PRIVACY.md](PRIVACY.md) — 정확히 어떤 파일을 읽고 쓰는지, 외부 전송 경로 세 갈래(DeepSeek 키 시 꾸러미+연결 점검 / 기본 정찰 시 Claude CLI 경유 꾸러미 / Codex 정찰 선택 시 codex CLI 경유 꾸러미)의 내용·제외·조건, 데이터 삭제
...
- **자체 서버 전송 없음 / 읽기 전용 원칙**: 확장·브릿지가 따로 보내는 곳은 없고, 외부로 나가는 것은 정찰 관련뿐입니다 — 정찰 실행 시의 꾸러미(DeepSeek 키 등록 시 DeepSeek API로 / 기본 정찰은 쓰시던 Claude CLI 경유 / Codex 정찰 선택 시 쓰시던 codex CLI 경유)와 DeepSeek 키 등록 시 3트랙 켤 때의 연결 점검 1회([PRIVACY.md](PRIVACY.md)에 전송·제외·조건 명시). 코덱스 원본 기록은 *직접 "영구삭제"를 누른 한 파일* 외엔 건드리지 않습니다.
