...
- **정찰(3트랙) 용어 한눈에** — 대시보드의 정찰 구역은 하나의 흐름입니다(각 단계에 LLM 사용 여부 배지):
  ...
  | ② 영향지도 | 이 변경이 어디까지 번질지 정찰 AI가 미리보기(옛 표기 '영향지도 게시판') | ⚡ 호출 | 기본 정찰(Claude)=별도 과금 없음(쓰던 사용량 범위) / DeepSeek 정찰은 키 / Codex 정찰은 쓰는 Codex 계정 사용량 범위 |
...
5. **정찰(3트랙)도 마켓 설치본만으로 동작합니다**(2026-09-16): 정찰 실행층(실행기 3종·꾸러미 수집·보관·대상 지정·게이트·지도 명령·일지 도구)이 브릿지 엔진과 함께 브릿지 홈에 배치되고, 훅·대시보드의 실행 안내도 그 경로를 가리킵니다 — 예: `node "<브릿지 홈>/scope-scout-self.js" <프로젝트>`. 저장소에만 있는 것은 개발용 도구(회고·재조정·동기화)뿐입니다. 정찰 담당의 기본값은 모드를 따릅니다 — 클로드-코덱스는 기본 정찰(Claude·무과금), 코덱스-코덱스는 Codex 정찰. 담당의 명령줄(또는 DeepSeek 키)이 없으면 대시보드가 '준비 안 됨'과 사유를 보여 주고, 플랜 게이트와 자동 지시는 따라 할 수 없는 실행을 요구하지 않고 통과합니다(2026-09-17).
...
- [PRIVACY.md](PRIVACY.md) — 정확히 어떤 파일을 읽고 쓰는지, 외부 전송 경로 네 갈래(Codex 검증 시 요청문+자동 동봉 / DeepSeek 키 시 꾸러미+연결 점검 / 기본 정찰 시 Claude CLI 경유 꾸러미 / Codex 정찰 선택 시 codex CLI 경유 꾸러미)의 내용·제외·조건, 데이터 삭제
...
- **자체 서버 전송 없음 / 읽기 전용 원칙**: 확장·브릿지가 따로 보내는 곳은 없고, 외부로 나가는 것은 Codex 검증 전송(요청문+자동 동봉)과 정찰 관련뿐입니다 — 정찰 실행 시의 꾸러미(DeepSeek 키 등록 시 DeepSeek API로 / 기본 정찰은 쓰시던 Claude CLI 경유 / Codex 정찰 선택 시 쓰시던 codex CLI 경유)와 DeepSeek 키 등록 시 3트랙 켤 때의 연결 점검 1회([PRIVACY.md](PRIVACY.md)에 전송·제외·조건 명시). 코덱스 원본 기록은 *직접 "영구삭제"를 누른 한 파일* 외엔 건드리지 않습니다.
