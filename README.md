...
1. **[codex-peek-0.1.101-installer.zip 내려받기](https://github.com/kimbyungsu/codex-peek/releases/download/v0.1.101/codex-peek-0.1.101-installer.zip)** → 압축 풀기.
...
이 경로는 확장·브릿지·훅과 개발용 스크립트(회고·재조정·동기화)까지 함께 둡니다 — 정찰(3트랙) 실행층은 마켓/VSIX 설치본에도 똑같이 들어 있습니다(2026-09-16 이관). 이미 `0.1.101`이 설치된 PC에서도 설치기가 기존 확장 폴더를 강제 삭제하지 않고, 바뀐 런타임만 백업한 뒤 같은 버전에 안전하게 갱신합니다. 비표준 `extensions` 폴더를 쓰는 환경만 `CODEX_BRIDGE_EXTENSIONS_DIR`로 그 폴더를 지정하면 됩니다.

확장만 빠르게 설치하려면(검증·대시보드·정찰 3트랙 모두 동작) **[codex-bridge-0.1.101.vsix](https://github.com/kimbyungsu/codex-peek/releases/download/v0.1.101/codex-bridge-0.1.101.vsix)**를 받아 VS Code의 `확장: VSIX에서 설치...`로 여세요. 같은 `0.1.101`을 이미 쓰는 PC는 VSIX 강제 재설치 대신 위 전체 설치 묶음을 사용해야 안전하게 최신 코드가 반영됩니다. 파일 지문은 [v0.1.101 릴리스 페이지](https://github.com/kimbyungsu/codex-peek/releases/tag/v0.1.101)의 `SHA256SUMS.txt`에서 확인할 수 있습니다.
