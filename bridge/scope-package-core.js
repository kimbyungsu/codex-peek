...
exports.PKG_DEFAULTS = void 0;
exports.isSensitivePath = isSensitivePath;
exports.redactSensitiveDiff = redactSensitiveDiff;
exports.extractDiffTokens = extractDiffTokens;
exports.buildPackage = buildPackage;
exports.scoutDirectiveText = scoutDirectiveText;
exports.scoutLedgerNotes = scoutLedgerNotes;
exports.renderPackageMarkdown = renderPackageMarkdown;
exports.PKG_DEFAULTS = {
...
function redactSensitiveDiff(diffText) {
    ...
    for (const part of parts) {
        ...
        if (m) {
            ...
            if (paths.some((p) => isSensitivePath(p))) {
                excluded.push(paths[paths.length - 1] || m[1]);
                continue;
            }
        }
        kept.push(part);
...
function extractDiffTokens(diffText, opts) {
    const o = { ...exports.PKG_DEFAULTS, ...(opts || {}) };
    const counts = {};
    for (const raw of String(diffText).split(/\r?\n/)) {
        ...
        if (!m)
            continue;
        for (const t of m) {
            if (t.length < o.minTokenLen)
                continue;
            if (STOP.has(t.toLowerCase()))
                continue;
...
function buildPackage(input, opts) {
    ...
    if (input.sensitiveExcluded && input.sensitiveExcluded.length)
        truncations.push(`민감 범주 파일 ${input.sensitiveExcluded.length}개의 diff 제외(외부 전송 안전 — 파일명도 생략)`);
    let diff = String(input.diffText || "");
    if (diff.length > o.maxDiffChars) {
        diff = diff.slice(0, o.maxDiffChars);
        truncations.push(`diff ${input.diffText.length}→${o.maxDiffChars}자로 절단`);
    }
    let map = input.mapContent;
    if (map && map.length > o.maxMapChars) {
        map = map.slice(0, o.maxMapChars);
        truncations.push(`MAP ${input.mapContent.length}→${o.maxMapChars}자로 절단`);
    }
    const failures = (input.recentFailures || []).slice(0, o.maxFailures);
    if ((input.recentFailures || []).length > o.maxFailures)
        truncations.push(`최근 실패 ${input.recentFailures.length}→${o.maxFailures}건`);
    return {
        meta: { repo: input.repo, head: input.head, generatedBy: "scope-package v1 (deterministic)", caps: o, truncations, ...(input.mapContentBlocked ? { mapContentBlocked: input.mapContentBlocked } : {}), ...(input.mapContentBlockedKey ? { mapContentBlockedKey: input.mapContentBlockedKey } : {}), ...(input.mapContentBlockedEn ? { mapContentBlockedEn: input.mapContentBlockedEn } : {}) },
        seeds: input.seeds,
        diff,
...
function scoutDirectiveText(lang) {
    return lang === "en"
...
function scoutLedgerNotes(lang) {
    return lang === "en"
        ? {
...
function renderPackageMarkdown(p, lang) {
    const L = [];
    const maskList = (paths) => {
        const shown = [];
        let hidden = 0;
        for (const s of paths) {
            if (isSensitivePath(s))
                hidden++;
            else
                shown.push(s);
        }
    ...
    const HIDDEN = (n) => `[민감 범주 파일 ${n}개 — 이름 생략(전송 보호)]`;
    ...
    const maskText = (s) => String(s || "").replace(/[A-Za-z0-9_.\\/-]{3,}/g, (t) => (isSensitivePath(t) ? "[민감 범주 — 이름 생략]" : t));
    ...
    if (p.tokenHits.length)
        for (const h of p.tokenHits) {
            const m = maskList(h.files);
            L.push(`- \`${h.token}\` → ${m.shown.concat(m.hidden ? [HIDDEN(m.hidden)] : []).join(", ")}${h.truncated ? " (…더 있음)" : ""}`);
        }
    else
        L.push("(seed 밖에서 참조되는 바뀐 식별자 없음)");
    if (p.droppedTokens.length)
        L.push(`\n(제외된 편재 토큰 — 참조가 너무 흔해 결합 증거가 안 됨: ${maskText(p.droppedTokens.join(", "))})`);
    L.push(`\n## 4. 과거 '함께 변경' 통계 (이 저장소 git 이력)`);
    if (!p.coChange)
        L.push("(git 이력 없음)");
    else if (p.coChange.sparse)
        L.push(`(표본 부족 — seed 관측 ${p.coChange.seedObservations}회 <3 → 통계는 침묵. 신생 영역이면 정상)`);
    else
        L.push(p.coChange.candidates.map((c) => `- ${isSensitivePath(c.file) ? HIDDEN(1) : c.file} (함께 변경 ${c.n}회)`).join("\n") || "(문턱 이상 후보 없음)");
    ...
    if (p.map)
        L.push(`\n## 7. stable MAP(확정 지식층)\n${maskText(p.map)}`);
    else if (p.meta.mapContentBlocked)
        L.push(lang === "en"
