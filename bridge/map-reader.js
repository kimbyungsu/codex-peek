...
function gateResult(state, why, active, reasonKey) {
  return {
    ...
    notice: {
      ko: state === "stale" ? "플랜 확정 전에 Project MAP부터 — 지도가 이 변경을 아직 모릅니다. `" + CL.bridgeCmd("scope-map.js") + " <저장소> refresh` 후 계속하세요."
        : state === "no-map" ? "이 프로젝트에 Project MAP projection이 아직 없습니다 — `" + CL.bridgeCmd("scope-map.js") + " <저장소> bootstrap`으로 시작할 수 있어요."
        : "Project MAP 기준 진행 가능(" + state + ").",
      en: state === "stale" ? "Map first, before finalizing the plan — the Project MAP does not know this change yet. Run `" + CL.bridgeCmd("scope-map.js") + " <repo> refresh`, then continue."
        : state === "no-map" ? "This project has no Project MAP projection yet — start with `" + CL.bridgeCmd("scope-map.js") + " <repo> bootstrap`."
