...
function cleanupOldState(now) {
  ...
  sweep(path.join(STATS_DIR, "selector-usage"), PROOF_TTL_MS); // 선별 영수증 — 같은 무유실 계보(v7 §3)
...
// [UI 개편 2차] 검증 요청에 실제 동봉된 지도 조각의 구조화 기록 — 대시보드 '이번 검증에 실린 기억' 카드 전용.
// 주입문 원문에는 이미 문자열로 존재하지만 항목 목록으로는 안 남아(사용자 질문 2026-08-07) ask마다 한 줄씩 남긴다.
const ATTACH_USAGE_FILE = path.join(STATS_DIR, "attach.jsonl");
function trimAttachUsage(maxLines) {
  try {
    const lines = fs.readFileSync(ATTACH_USAGE_FILE, "utf8").split("\n").filter(Boolean);
    if (lines.length <= maxLines) return;
    fs.writeFileSync(ATTACH_USAGE_FILE, lines.slice(-maxLines).join("\n") + "\n", "utf8");
  } catch { /* best-effort */ }
}
function appendAttachUsage(ev) {
  try {
    fs.mkdirSync(STATS_DIR, { recursive: true });
    fs.appendFileSync(ATTACH_USAGE_FILE, JSON.stringify(ev) + "\n", "utf8");
    trimAttachUsage(200);
    return true;
...
// [4a f-3c20a5da] 진행 표기 워크스페이스 분리: 기록은 ws별 파일(phase-<wsKey>.json)+legacy 전역(구형 확장
// 호환 미러) 이중, 판독은 ws 지정 시 ws별 파일 우선(부재=legacy 폴백). 다중 프로젝트가 동시에 진행돼도
// 서로의 라이브 표시를 덮어쓰지 못하고, 병합 승계도 같은 ws 기록만 본다(타 프로젝트 round·session 오염 차단).
function phaseFileFor(ws) { return path.join(BRIDGE_DIR, "phase-" + wsKeyFor(ws) + ".json"); }
function readPhase(ws) {
  if (ws) { try { return JSON.parse(fs.readFileSync(phaseFileFor(ws), "utf8")) || {}; } catch { /* 미기록·구형=legacy 폴백 */ } }
  try { return JSON.parse(fs.readFileSync(PHASE_FILE, "utf8")) || {}; } catch { return {}; }
}
function writePhase(phase, extra) {
  const data = Object.assign({ round: 0 }, readPhase(extra && extra.workspace ? extra.workspace : undefined), { phase, ts: new Date().toISOString() }, extra || {});
  let ok = atomicWrite(PHASE_FILE, JSON.stringify(data)); // legacy 미러 — 구형 확장(전역 단일 판독)도 최신 기록은 봄
  try { if (data.workspace) ok = atomicWrite(phaseFileFor(data.workspace), JSON.stringify(data)) && ok; } catch { /* ws 파일 실패=legacy만 */ }
  return ok;
}
...
function activeAskJobFor(ws) {
  try {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      if (!j || (j.state !== "queued" && j.state !== "selecting" && j.state !== "running")) continue; // selecting=서고 선별 진행(3b — 진행형)
...
function phaseBusy(ws, phases, maxAgeMs) {
  try {
    const p = readPhase(ws); // [4a] ws별 기록 우선 — 타 프로젝트의 legacy 덮어쓰기가 이 ws의 busy 판정을 가리지 않게
...
function loadContract(ws, lang) {
  ...
  return {
    ...
    verifierProvider: normVerifierProvider(o),
    ...
    archiveHash: typeof o?.archiveHash === "string" && /^[0-9a-f]{40}$/.test(o.archiveHash) ? o.archiveHash : null, // [Envelope Selector v6 §1] 서고 승인 도장 — 무효/부재=null(미도입)
  };
}
...
// [VerifierProvider Phase1] 검증자 공급자 — 기본 codex(미지정·미지값=무회귀). 설계 정본:
// docs/VERIFIER-PROVIDER-DESIGN.md(동결 v3).
const VERIFIER_PROVIDERS = ["codex", "claude"];
function normVerifierProvider(o) {
  return o && VERIFIER_PROVIDERS.includes(o.verifierProvider) ? o.verifierProvider : "codex";
}
...
// 원문(버퍼/문자열)→prefs 순수 해석 — 스냅샷 결속(재검증 blocker②: 지문과 실행 인자가 '같은 판독'에서
// 나와야 실행·증명 오결속이 없다)을 위해 판독과 해석을 분리.
function scoutCodexPrefsFrom(prefsRaw) {
  try {
    const j = JSON.parse(String(prefsRaw));
...
function readScoutCodexPrefs() {
  let raw = null; try { raw = fs.readFileSync(SCOUT_CODEX_FILE); } catch { /* 부재=기본 */ }
  return scoutCodexPrefsFrom(raw);
}
...
function scoutCodexArgsFrom(p) {
  const a = [];
  if (p.model) a.push("-c", `model=${p.model}`);
  if (p.reasoning) a.push("-c", `model_reasoning_effort=${p.reasoning}`);
  return a;
}
function scoutCodexArgs() { return scoutCodexArgsFrom(readScoutCodexPrefs()); }
...
function codexScoutExecArgsFrom(prefs, outFile) {
  return ["exec", "--ephemeral", "--sandbox", "read-only", "--skip-git-repo-check", ...scoutCodexArgsFrom(prefs), "-o", outFile];
}
function codexScoutExecArgs(outFile) { return codexScoutExecArgsFrom(readScoutCodexPrefs(), outFile); }
// probe가 지문과 '같은 스냅샷'으로 실행 인자를 만들 때 사용(재검증 blocker② — 실행·증명 결속)
function codexScoutExecArgsFromSnapshot(snap, outFile) { return codexScoutExecArgsFrom(scoutCodexPrefsFrom(snap && snap.prefsRaw), outFile); }
// 도구 자체 실행 표식(2026-07-28 실측 결함 봉합): --ephemeral 호출은 rollout(세션 기록)을 남기지 않으므로
// codex-hook의 세션 출처 판정이 항상 unknown이 되고, P-9의 fail-closed 규칙이 그 프롬프트를 차단한다
// (정찰·준비 점검·의미 보강·해소 질의 전부 동일). 하네스가 스스로 띄운 프로세스임을 자식 훅이 식별할 수
// 있도록 표식 env를 싣는다 — 훅은 이 표식을 'exec(확정 비대상)'과 동일하게 침묵 처리한다. 위조 가능성은
// 단일 사용자 로컬 전제(수칙서 지원 환경)에서 수용하며, 표식은 자동 전환 판정에만 쓰고 검증 증명에는 쓰지 않는다.
const TOOL_EXEC_ENV = "CODEX_PEEK_TOOL_EXEC";
function codexScoutExecEnv(base, kind) {
  return { ...(base || process.env), [TOOL_EXEC_ENV]: String(kind || "1") };
}
...
// 자동 재점검 전역 예약(2차 blocker③→3차 blocker①③ 개정): 창 메모리 Set은 창마다 따로라 다중 창이
// 같은 담당·같은 지문을 중복 과금 점검할 수 있다 — 전역 파일+잠금으로 '담당별 fp당 1회'를 기계 보증.
...
function autoReprobeLedgerOp(fn) {
  try { fs.mkdirSync(BRIDGE_DIR, { recursive: true }); } catch { /* 잠금이 실패 판정 */ }
  const lockP = AUTO_REPROBE_FILE + ".lock";
  let r = withFileLockStrict(lockP, fn);
  // 죽은 보유자(ESRCH 확정)만 격리 후 1회 재시도(3차 blocker③ 후단 — 잠금 보유 중 사망이 자동 경로를
  // 영원히 막지 않게). quarantineContractLock과 같은 계보: 사망 '증거' 기반만 회수, 나이 추측 탈취 금지
  // (pid 재사용으로 살아 있어 보이면 보수적으로 양보 — null 반환이라 다음 틱 재시도가 남는다).
  if (!r.ok && /dead-lock-holder/.test(String(r.error || ""))) {
    try { fs.renameSync(lockP, lockP + ".dead-" + Date.now()); } catch { /* 격리 실패=이번 틱 양보 */ }
    r = withFileLockStrict(lockP, fn);
  }
  return r;
}
function readAutoReprobeLedger() {
...
function claimAutoReprobe(key) {
  ...
  const now = Date.now();
  const r = autoReprobeLedgerOp(() => {
    let cur = readAutoReprobeLedger();
    const i = cur.findIndex((e) => e && e.sig === key);
    let tries = 1;
    if (i >= 0) {
      const e = cur[i];
      if (e.state === "done") {
    ...
    return atomicWrite(AUTO_REPROBE_FILE, JSON.stringify(cur)) ? "ok" : "io";
  });
  if (!r.ok) return null; // 잠금 일시 불가 — 실행 안 함(fail-closed)이되 영구 아님
  return r.result === "ok" ? true : r.result === "deny" ? false : null; // io 실패도 재시도 가능으로
}
// 실행 완료 확정 — 이걸 찍어야 예약이 영구가 된다(자식 정상 종료 시에만 호출·크래시는 TTL 승계).
function completeAutoReprobe(key) {
  if (!key || typeof key !== "string") return false;
  const r = autoReprobeLedgerOp(() => {
    let cur = readAutoReprobeLedger();
    const i = cur.findIndex((e) => e && e.sig === key);
    if (i >= 0) cur[i] = Object.assign({}, cur[i], { state: "done", doneTs: new Date().toISOString() });
    else { cur.push({ sig: key, ts: new Date().toISOString(), state: "done", tries: 1 }); if (cur.length > 40) cur = cur.slice(-40); } // 밀려난 뒤 완료 — 재등록
    return atomicWrite(AUTO_REPROBE_FILE, JSON.stringify(cur)) ? "ok" : "io";
  });
...
// A→B→A 경합(1차 blocker⑥·재검증 blocker②)은 '고지 수용'이 아니라 스냅샷 결속으로 봉합: probe가
// 설정을 정확히 1회 읽어 그 스냅샷으로 실행 인자와 지문을 함께 만든다(아래 read*Snapshot/*From 참조)
// — 실행 도중 파일이 몇 번 바뀌든, 기록된 지문은 '실제로 실행한 설정'과 항상 같다.
function bufSig(b) { return b === null || b === undefined ? "-" : crypto.createHash("sha1").update(b).digest("hex"); }
function contentSig(files) {
  return files.map((f) => { let b = null; try { b = fs.readFileSync(f); } catch { /* 부재="-" */ } return bufSig(b); }).join(",");
}
// probe 실행·지문 결속용 스냅샷(경합 창의 자식 deepseek-bridge가 파일 대신 이 원문을 쓰도록 env로 전달)
const DS_SNAPSHOT_ENV = "CODEX_PEEK_DS_SNAPSHOT";
function readEconomySnapshot() {
  let raw = null; try { raw = fs.readFileSync(path.join(BRIDGE_DIR, "deepseek.json")); } catch { /* 부재 */ }
  let fileJson = null; try { fileJson = JSON.parse(String(raw)); } catch { /* 파손=env만 */ }
  return { raw, fileJson };
}
function economyConfigFpFrom(snap) {
  try {
    const db = require(path.join(__dirname, "deepseek-bridge.js"));
    const cfg = db.resolveDeepseekConfig(process.env, snap && snap.fileJson);
    if (!cfg.apiKey) return null; // 설정 부재(economyReady 1조건 미충족)
    return sha1Of(cfg.baseUrl + "|" + cfg.model + "|" + sha1Of(cfg.apiKey).slice(0, 12) + "|" + bufSig(snap && snap.raw));
  } catch { return null; }
}
function economyConfigFp() { return economyConfigFpFrom(readEconomySnapshot()); }
...
function readPrecisionConfigSnapshot() {
  let prefsRaw = null, binRaw = null;
  try { prefsRaw = fs.readFileSync(SCOUT_CODEX_FILE); } catch { /* 부재=기본 */ }
  try { binRaw = fs.readFileSync(path.join(BRIDGE_DIR, "codex-bin.txt")); } catch { /* 미기록 */ }
  return { prefsRaw, binRaw };
}
function precisionExecFpFrom(inv, snap) {
  ...
  // env CODEX_BIN 문자열 포함(blocker③ 해석 일치). env 자체의 A-B-A는 창 프로세스별이라 실행 중 교체
  // 가능성이 파일 대비 현저히 낮음 — inv 문자열이 지문에 직접 들어가 실행 결속은 유지된다.
  const prefs = scoutCodexPrefsFrom(snap && snap.prefsRaw);
  return sha1Of(inv.file + "|" + (inv.args || []).join(" ") + "|" + home + "|" + (process.env.CODEX_BIN || "") + "|" + codexScoutExecArgsFrom(prefs, "<out>").join(" ") + "|" + CODEX_SCOUT_ADAPTER_VER + "|" + bufSig(snap && snap.prefsRaw) + "," + bufSig(snap && snap.binRaw));
}
function precisionExecFp(inv) { return precisionExecFpFrom(inv, readPrecisionConfigSnapshot()); }
...
// ── verifier 세션 전역 lease(재확인 설계 §7 · 증분 3) ──────────────────────────────────
// 유일 키=정규화한 verifier session ID 전역 1개. ws·mode·jobId·PID·token은 소유자 메타데이터다.
// 목적: 서로 다른 창/프로젝트가 '같은' verifier 세션을 동시에 resume하는 경합 차단(ask-active는
// ws 단위라 교차 ws 같은 세션을 못 막는다 — v6 설계 blocker 계보). 일반 검증 resume와(증분 4부터)
// challenge resume가 같은 lease를 확인하고, 원 검증의 lease는 같은 token으로 challenge까지 이관한다.
const SESSION_LEASES_DIR = path.join(BRIDGE_DIR, "session-leases");
function sessionLeaseFileFor(sid) { return path.join(SESSION_LEASES_DIR, safeStateName(String(sid || "").trim()) + ".json"); }
function readSessionLease(sid) { try { return JSON.parse(fs.readFileSync(sessionLeaseFileFor(sid), "utf8")); } catch { return null; } }
...
// 기록을 섞는다(확인 검증 blocker). 이 경우 ask-active와 같은 철학으로 수동 정리만 허용:
// `session-lease clear <session> --confirm` (busy 응답에 staleOwner 힌트를 실어 안내).
function acquireSessionLease(sid, meta) {
  const s = String(sid || "").trim();
  if (!s) return { ok: false, reason: "no-session" };
  fs.mkdirSync(SESSION_LEASES_DIR, { recursive: true });
  const file = sessionLeaseFileFor(s);
  const token = require("crypto").randomBytes(8).toString("hex");
  const rec = {
    v: 1, session: s, token, ownerPid: process.pid, childPid: null,
    ws: String((meta && meta.ws) || ""), mode: String((meta && meta.mode) || ""), jobId: String((meta && meta.jobId) || ""),
    deadlineAt: String((meta && meta.deadlineAt) || ""), createdAt: new Date().toISOString(),
  };
  const tryCreate = () => {
    const tmp = file + "." + token + ".tmp";
    try { fs.writeFileSync(tmp, JSON.stringify(rec, null, 1)); fs.linkSync(tmp, file); }
    catch { try { fs.unlinkSync(tmp); } catch { /* 무해 */ } return false; }
  ...
  const held = withFileLockStrict(file + ".reclaim.lock", () => {
    ...
    // childPid 미기록=자동 회수 금지(고아 codex 생존 가능 — 수동 clear만). 기록됨=owner·child 모두
    // 확정 사망일 때만 회수.
    const childGone = cur.childPid === null || cur.childPid === undefined ? false : !leasePidAlive(Number(cur.childPid));
    if (ownerDead && childGone) {
      try { fs.unlinkSync(file); } catch { return { ok: false, reason: "busy", holder: cur }; }
      if (tryCreate()) return { ok: true, token, session: s, reclaimedFrom: cur };
    }
    return { ok: false, reason: "busy", holder: cur, staleOwner: ownerDead };
  });
  return held.ok ? held.result : { ok: false, reason: "busy", holder: readSessionLease(s) };
}
...
// 기억 v3는 대상 파일 안에서 호출자(연 폴더+언어 슬롯)별 {state,base,buckets,arm}을 분리한다. 같은 대상을
// 서로 다른 계약이 공유해도 담당값을 번갈아 덮어 무한 재안내하지 않는다. 판독·판정·기록은 한 잠금 안에서 수행.
// 구버전 단일 기억은 현재 호출자 슬롯으로 1회 마이그레이션한다.
...
function buildScoutDirective(ws, c) {
  ...
  const langSnap = loadLang();
  const armV = scoutArmView(ws, c); // 실효 담당이 바뀌면 같은 실패 상태도 새 실행 기회이므로 억제 서명에 포함
  const callerKey = langSnap + "-" + wsKeyFor(ws);
  try { fs.mkdirSync(path.dirname(f), { recursive: true }); } catch { return null; }
  const claimed = withFileLockStrict(f + ".lock", () => {
    let raw = null;
    try { raw = JSON.parse(fs.readFileSync(f, "utf8")); } catch { /* 첫 지시 */ }
    let callers = {};
    let prev = null;
    if (raw && raw.schema === "scout-advice-v3" && raw.callers && typeof raw.callers === "object" && !Array.isArray(raw.callers)) {
      callers = { ...raw.callers };
      const p = callers[callerKey];
      if (p && typeof p === "object" && typeof p.state === "string") prev = { state: p.state, base: p.base || null, buckets: p.buckets && typeof p.buckets === "object" ? p.buckets : null, arm: typeof p.arm === "string" ? p.arm : null };
    } else if (raw && typeof raw === "object" && typeof raw.state === "string") {
      prev = { state: raw.state, base: raw.base || null, buckets: raw.buckets && typeof raw.buckets === "object" ? raw.buckets : null, arm: typeof raw.arm === "string" ? raw.arm : null };
    } else if (raw && typeof raw.sig === "string") { // v0 형식 — 성분·담당 배정 불가 → 현재 호출자에게 1회 재알림
      prev = raw.sig === "no-map" ? { state: "no-map", base: null, buckets: null, arm: null }
        : raw.sig.startsWith("stale:") ? { state: "stale", base: raw.sig.slice(6), buckets: null, arm: null }
        : raw.sig.startsWith("legacy:") ? { state: "legacy-no-seeds", base: raw.sig.slice(7), buckets: null, arm: null }
        : null;
    }
    const sameState = prev && prev.state === st.state && prev.base === st.base;
    const sameArm = sameState && prev.arm === armV.eff;
    if (sameArm) {
      if (st.state !== "stale") return false; // no-map/legacy/invalid/unknown은 호출자·담당·상태당 1회
      if (prev.buckets && ["seed", "commits", "dirty", "history"].every((k) => comp[k] <= ((prev.buckets[k] | 0) || 0))) return false;
    }
    const mergedBuckets = {};
    for (const k of ["seed", "commits", "dirty", "history"]) mergedBuckets[k] = Math.max(comp[k], sameState && prev.buckets ? ((prev.buckets[k] | 0) || 0) : 0);
    const ts = new Date().toISOString();
    callers[callerKey] = { state: st.state, base: st.base, buckets: mergedBuckets, arm: armV.eff, ts };
    const keys = Object.keys(callers).sort((a, b) => String((callers[a] || {}).ts || "").localeCompare(String((callers[b] || {}).ts || "")));
    for (const k of keys.slice(0, Math.max(0, keys.length - 32))) delete callers[k];
    return atomicWrite(f, JSON.stringify({ schema: "scout-advice-v3", callers }));
  });
...
// 영향지도 형식(legacy/Project MAP/판독 불가)과 독립된 관찰 일지 재확인 블록. 모든 동봉 표면이 같은 id·문구를 쓴다.
function scoutCouplingAttach(target, en) {
  let couplings = [];
...
function buildScoutAttach(ws, c, lang) {
  ...
  const en = (LANGS.includes(lang) ? lang : loadLang()) === "en";
  const health = scoutHealthLine(target, en); // 프로젝트별 관찰 신호 — 실패해도 지도/장부 재확인 동봉 불침
  // 관찰 일지의 재확인은 영향지도 품질과 독립이다. 불량 지도 원문은 계속 배제하되, 과거 결합 후보는 다음
  // 검증에서 정확한 파일 근거로 다시 확인할 수 있어야 한다.
  const couplingAttach = scoutCouplingAttach(target, en);
  const couplings = couplingAttach.couplings;
  const coupleBlock = couplingAttach.text;
  const ledgerOnly = () => {
    if (!coupleBlock) return null;
    const head = en
      ? "[Field-journal recheck · independent of the impact map] No usable high-priority map material is included in this verification, but the earlier couplings below can still be checked on their own. Missing, invalid, or unreadable map text is not included."
      : "[관찰 일지 재확인 · 영향지도와 독립] 이번 검증에 쓸 수 있는 영향지도 high 자료는 싣지 않았지만, 아래 과거 결합은 별도로 다시 확인할 수 있다. 누락·불량·판독 실패 지도 원문은 포함하지 않았다.";
    return { text: [head, coupleBlock, ...(health ? [health] : [])].join("\n"), mapItems: [], couplings };
  };
  const st = scoutMapStatus(target);
  if (st.state === "no-map" || !st.base) return ledgerOnly();
  if (st.state === "invalid") return ledgerOnly(); // 형식 불명(빈/불량) 지도는 신뢰 입력으로 주입하지 않고 장부만 독립 동봉
  const dir = path.join(SCOUTS_DIR, wsKeyFor(target));
  let md = "", meta = {};
  try { md = fs.readFileSync(path.join(dir, st.base + ".md"), "utf8"); } catch { return ledgerOnly(); }
  ...
  if (!items.length) return ledgerOnly();
...
// 기계 확인 '승격 가능' 판정: 강한 명시 확인(claimed)은 1회, 약한 비-echoed 공동 인용은 서로 다른 askId 2회.
// 취급 흔적 검사 불가(seen=unknown)는 제외한다. 정본 machineConfirmSatisfied와 동형.
...
function miniMachineConfirmSatisfied(confs) {
  const claimed = new Set();
  const coCited = new Set();
  for (const e of confs) {
    if (!miniPromotableConfirm(e)) continue;
    const id = e.askId || e.ts || "";
    if (!id) continue;
    (e.grade === "claimed" ? claimed : coCited).add(id);
  }
  return claimed.size >= 1 || coCited.size >= 2;
}
...
function miniLedgerEntries(raw) {
  ...
  for (const e of per.values()) {
    ...
    const machineOk = miniMachineConfirmSatisfied(e.confs) || legacyRepeats(e.confs) >= 2; // DERIVE_V2와 동형(패리티 테스트 잠금)
    e.rehab = e.everDisp && !e.dead && (e.afterU >= 1 || miniMachineConfirmSatisfied(e.after) || legacyRepeats(e.after) >= 2);
...
const BASE_DEFAULTS = {
  ...
  verifyBaseline: [
    ...
    "4) 판정 기준은 '실질 영향'이다. 오작동·명세 불일치·회귀 위험·사용자/운영 판단을 오도할 표현·작아 보여도 반복·확장 시 결함으로 번질 구조는 사소하지 않으니 잡아라. 반대로 결과·동작·다음 판단을 바꾸지 않는 취향·형식·미세 문구만으로 통과를 막지 마라. 검증 경계의 절대 blocker 항목(ab-*)과 현재 구현이 직접 충돌하면 요청문의 요구를 이유로 낮추지 말고 blocker로 판정하라 — 단 현재 코드 증거가 그 제약을 무효화함을 직접 확인했다면 그 증거를 인용해 '[주의] 제약 재검토 필요'로 표기하라. ab 항목을 판정에 반영했으면 행 단독 '제약적용/제약기각/제약대체 ab-N' 한 줄을 남겨라.",
  ].join("\n"),
...
const BASE_CORE = {
  verifyBaseline: [
    ...
    "1) 논리 구조만으로 단정하지 말고, 코드·파일을 실제로 열어 확인해 검증하라.",
    "2) 검증 대상은 '요청이 선언한 목표·인수조건과 그 직접 영향'이다. 지정 파일·범위는 시작점 — 범위 밖 확장은 직접 의존·안전 불변식 위반 의심 같은 구체 사유가 있을 때만, 그 사유를 항목에 표기하라.",
    "3) 판정 규약(핵심 프로필): '검증: 실패'는 미해결 blocker가 최소 1개일 때만. blocker는 종류가 아니라 실질 영향으로 판정한다 — ①인수조건 안의 재현·신뢰 가능한 오작동 ②데이터·보안·역할·증명 무결성 훼손(희귀 경합이라도 해당하면 blocker) ③명시 요구 위반 ④핵심 인수조건의 실행 증거 부재 ⑤직접 연관 고위험 회귀 ⑥경계 ab-* 항목과의 직접 충돌(요청문이 요구했다는 이유로 낮추지 마라 — 단 현재 코드 증거가 제약을 무효화함을 직접 확인했다면 blocker 대신 증거 인용+'[주의] 제약 재검토 필요'. 간접 연관=기존 세 갈래. 반영한 ab는 행 단독 '제약적용|제약기각|제약대체 ab-N' 한 줄로 표기). 그 외 지적은 세 갈래로 표기하라: '[주의]'=보안·개인정보·데이터 손상·복구 불능·운영 오판 위험 '인접'(그 위험까지의 구체 경로 1줄 필수 — 경로를 못 대면 다른 갈래로. 처리는 구현모델이 재판단 — 침묵 이관 금지) / '[보완]'=구체·국소·새 설계 선택 없음(이번 루프 일괄 반영 기대) / '[백로그]'=범위 밖 제안(인수조건 밖·재현 가능한 명세 위반 아님·불변식 훼손 경로 없음·채택=새 범위 — 추가 방어·커버리지 확장 포함). 이름만 '저확률·경합·테스트 보강'이라고 [백로그]로 보내지 마라 — 무결성 훼손 저확률 경합과 핵심 입증 테스트 부재는 blocker다. 지적이 하나도 없으면 판정은 '검증: 통과', blocker 없이 비차단만 있으면 '검증: 통과(보완)'.",
  ].join("\n"),
  transmit: [
  ...
  rejudge: [
    ...
    "- 모든 지적(blocker 포함)을 [수용/반박/보류] + 근거(파일·라인)로 재판단하라. 수용에는 직접 확인한 근거가, 반박에는 실측 반례가 필요하다(성립 시 그 근거를 다음 검증 요청에 동봉). blocker(실패 사유)는 반박이 성립하지 않는 한 반드시 고치고, '[주의]'는 이번 루프 처리로 결정한 항목만 함께 고쳐 재검증하라. 완료 보고는 '통과'/'통과(보완)' 후에만 — 검증 후 수정했으면 최종본을 재검증하라.",
...
const BASE_CORE_EN = {
  verifyBaseline: [
    ...
    "2) The verification target is the declared goal/acceptance criteria and their direct impact. Given files/scope are a starting point — expand beyond them only with a concrete reason (direct dependency, suspected safety-invariant violation), noted on the item.",
    "3) Verdict rule (core profile): use 'Verdict: fail' only when at least one unresolved blocker exists. A blocker is judged by real impact, not category — (1) a reproducible/credible malfunction within the declared acceptance criteria, (2) damage to data/security/role/proof integrity (even a rare race, if it qualifies), (3) explicit-requirement violation, (4) missing executable evidence for a core criterion, (5) directly related high-risk regression, (6) direct conflict with an envelope ab-* item (do not downgrade because the request asked for it — unless current code evidence you verified invalidates the constraint: then cite it as '[caution] constraint needs review' instead of a blocker; indirect relations keep the three classes. Mark each ab you weighed on its own line: 'constraint-used|constraint-rejected|constraint-superseded ab-N'). Mark other findings in three ways: '[caution]' = adjacent to security/privacy/data-loss/unrecoverable/operational-misjudgment risk (a one-line concrete path is mandatory — no path, use another class; the implementer re-judges — never silently deferred) / '[notes]' = concrete, local, no new design choice (batch-applied in this loop) / '[backlog]' = out-of-scope proposal (outside the criteria, not a reproducible spec violation, no invariant-damaging path, adoption opens new scope — incl. extra hardening/coverage). Never send an item to '[backlog]' merely by the name 'low-probability/race/test-hardening' — an integrity-damaging rare race and a missing core-proof test are blockers. With zero findings the verdict is 'Verdict: pass'; with no blocker but non-blocking findings, 'Verdict: pass (notes)'.",
  ].join("\n"),
  transmit: [
  ...
  rejudge: [
    ...
    "- Re-judge every point (blockers included) as [accept/rebut/hold] + evidence (file·line). Acceptance needs evidence you checked yourself; a rebuttal needs a measured counterexample (carry it into the next request when it stands). Fix every blocker unless a rebuttal stands, plus only the '[caution]' items you decided to handle in this loop, then re-verify. Report completion only after 'pass' or 'pass (notes)' — and re-verify the final state if you edited after verification.",
...
const BASE_DEFAULTS_EN = {
  verifyBaseline: [
    ...
    "4) Judge by REAL impact. Malfunctions, spec mismatches, regression risks, wording that could mislead users/operations, and structures that look small but will grow into defects when repeated or extended are not minor — catch them. Conversely, do not block a pass over taste, formatting, or micro-wording that changes no outcome, behavior, or next decision. If the current implementation directly conflicts with an always-blocker item (ab-*) of the Verification Envelope, do not downgrade because the request asked for that behavior — it is a blocker; unless you directly verified that current code evidence invalidates the constraint, in which case mark '[caution] constraint needs review' citing that evidence. When an ab item informed your verdict, leave a one-line handling mark on its own line: 'constraint-used/constraint-rejected/constraint-superseded ab-N'.",
  ].join("\n"),
  transmit: [
...
// [프로필 문안 편집 개방 — 기계 서식 분리] 판정 줄·지적 블록 요구는 '문안'이 아니라 기계 계약이다.
// 어느 프로필의 어떤 편집과도 무관하게 코드가 소유하고 검증자 프롬프트에 항상 동봉된다(verifierBaselineFor).
// 문안 카드에서 서식 문구를 지워도 판정 판독·기계 지적 블록·대시보드 색 표시는 깨지지 않는다.
// (v2 서식 상향은 종전대로 v2DirectiveFor 담당 — 여기는 상시 최소 계약 v1.)
const VERIFIER_FORMAT_KO = [
  "[응답 서식 · 기계 판독 — 코드 고정(문안 편집과 무관하게 항상 적용)]",
  "1) 본문에 검토 내용·항목별 근거(경로·라인)를 '먼저' 상세히 작성하라. 판정 결론은 반드시 '맨 마지막 한 줄'에만 다음 4가지 중 정확히 하나로 출력하라: '검증: 통과' / '검증: 통과(보완)' / '검증: 보류' / '검증: 실패'. 마지막 줄 외에는 '검증:'으로 시작하는 줄을 쓰지 마라.",
  "2) 판정 줄 '바로 앞'에 기계 판독용 지적 블록을 붙여라: '[지적 목록 v1]' 한 줄(그 문자열 그대로 행 전체) → 본문에서 제기한 모든 지적을 줄당 JSON 1개로 {\"tag\":\"blocker|주의|보완|백로그\",\"title\":\"지적 1줄(비밀값·개인정보·절대경로 원문 금지)\",\"file\":\"관련 파일 경로(선택)\"} → '[지적 목록 끝]' 한 줄. 지적이 없으면 마커 두 줄만(빈 블록). 종료 마커와 마지막 판정 줄 사이에는 빈 줄 외에 아무것도 쓰지 마라. 블록이 없거나 손상되거나 판정과 모순이면(예: blocker를 나열하고 '통과' 선언) 하네스가 판정을 '보류'로 강등한다(fail-closed).",
].join("\n");
const VERIFIER_FORMAT_EN = [
  "[Response format · machine-readable — code-fixed (always applies regardless of text edits)]",
...
// [약속 발화 포착 부품 B §2·§7-3 '프롬프트 범주 규칙 1줄'] 범주 수준 안내(문구 패턴 목록 아님 — 판단은 검증자
// LLM). 선택 절이라 블록 부재=정상, 회수 게이트(원문 스냅샷 대조·형태 방어)는 하네스가 별도 수행.
const VERIFIER_FORMAT_CONSTRAINT_KO = "3) (선택) 사용자 발화 원문에 '앞으로 계속 지킬 약속'으로 보이는 구절이 있으면, 지적 블록보다 앞에 '[제약 후보 v1]' 한 줄 → 줄당 {\"quote\":\"<사용자 원문 연속 구절>\",\"why\":\"<한 줄>\"}(최대 4행) → '[제약 후보 끝]' 한 줄로 제출하라. 원문 그대로만 — 요약·작문·계약/지도 문구 인용은 하네스가 거부한다.";
const VERIFIER_FORMAT_CONSTRAINT_EN = "3) (Optional) If the user's own words contain a lasting promise, submit BEFORE the findings block: a line '[제약 후보 v1]' → per line {\"quote\":\"<verbatim contiguous span of the user's words>\",\"why\":\"<one line>\"} (max 4 rows) → a line '[제약 후보 끝]'. Verbatim only — summaries, rewrites, or quotes from contracts/maps are rejected by the harness.";
function verifierFormatDirective(lang, blockMode) {
  const en = (LANGS.includes(lang) ? lang : loadLang()) === "en";
  const tail9 = "\n" + (en ? VERIFIER_FORMAT_CONSTRAINT_EN : VERIFIER_FORMAT_CONSTRAINT_KO); // 두 blockMode 공통(위임 모드에서 소실 금지)
  if (blockMode !== "delegated") return (en ? VERIFIER_FORMAT_EN : VERIFIER_FORMAT_KO) + tail9;
  // 조각 재조립(단일 출처): 전문 상수의 헤더+1)판정 줄 항목을 그대로 쓰고 2)블록 항목만 위임 줄로 교체.
  const full = en ? VERIFIER_FORMAT_EN : VERIFIER_FORMAT_KO;
  const cut = full.indexOf("\n2) ");
  return (cut > 0 ? full.slice(0, cut) : full) + "\n" + (en ? VERIFIER_FORMAT_DELEGATE_EN : VERIFIER_FORMAT_DELEGATE_KO) + tail9;
}
...
function baseDefaultsFor(lang, profile) {
  const en = (LANGS.includes(lang) ? lang : loadLang()) === "en";
  return profile === "core" ? (en ? BASE_CORE_EN : BASE_CORE) : (en ? BASE_DEFAULTS_EN : BASE_DEFAULTS);
}
function baseDirectiveFileFor(lang, profile) {
  const l = LANGS.includes(lang) ? lang : loadLang();
  if (profile === "core") return path.join(BRIDGE_DIR, l === "ko" ? "base-directive.core.json" : `base-directive.core.${l}.json`);
  return l === "ko" ? BASE_DIRECTIVE_FILE : path.join(BRIDGE_DIR, `base-directive.${l}.json`);
}
// 기본 지침 로드: 오버라이드 파일의 비지 않은 항목만 그 프로필의 기본값을 대체(두 프로필 동일 규칙 —
// 편집 개방). lang=언어 슬롯(오버라이드·기본값 모두 그 언어 것).
function loadBaseDirective(lang, profile) {
  let o = {};
  try { o = JSON.parse(fs.readFileSync(baseDirectiveFileFor(lang, profile), "utf8")); } catch { o = {}; }
  const D = baseDefaultsFor(lang, profile);
...
function saveBaseDirective(obj, lang, profile) {
  const D = baseDefaultsFor(lang, profile);
  const file = baseDirectiveFileFor(lang, profile);
  const out = {};
  for (const k of ["verifyBaseline", "transmit", "rejudge"]) {
    const v = obj && typeof obj[k] === "string" ? obj[k] : "";
    if (v.trim() && v.trim() !== D[k].trim()) out[k] = v;
  }
  // [결정 블록 서식 2026-08-30] 같은 파일의 decisionBlock 오버라이드(사용자 편집 서식)는 세 축 저장이 지우지 않는다(ab-2)
  try { const prev = JSON.parse(fs.readFileSync(file, "utf8")); if (prev && prev.decisionBlock && typeof prev.decisionBlock === "object") out.decisionBlock = prev.decisionBlock; } catch { /* 없음 */ }
...
function resetBaseDirective(lang, profile) {
  // 그 프로필·언어의 오버라이드 파일만 삭제 = 기본값 복원(다른 프로필·언어 오버라이드 보존).
  try { fs.unlinkSync(baseDirectiveFileFor(lang, profile)); } catch (e) { if (e && e.code !== "ENOENT") return false; }
  return true;
}
...
const ARCHIVE_FILE = "verify-envelope-archive.json";
const ARCHIVE_ITEM_MAX = 96;
function validEnvelopeArchiveInner(inner) {
  if (!inner || typeof inner !== "object" || Array.isArray(inner) || inner.schema !== "verify-envelope-archive-v1") return false;
  const allowed = new Set(["schema", "note", "alwaysBlocker", "alwaysBlockerEn", "alwaysBlockerEx"]);
  const strAxis = (arr, mustLen) => Array.isArray(arr) && (mustLen === undefined || arr.length === mustLen) && arr.length <= ARCHIVE_ITEM_MAX && !arr.some((x) => typeof x !== "string" || !x.trim() || x.trim().length > ENVELOPE_CHAR_MAX);
  if (!strAxis(inner.alwaysBlocker)) return false;
  for (const suf of ["En", "Ex"]) { const opt = inner["alwaysBlocker" + suf]; if (opt !== undefined && !strAxis(opt, inner.alwaysBlocker.length)) return false; }
...
function readVerifyEnvelopeArchive(repo) {
  let raw = null;
  try { raw = fs.readFileSync(path.join(repo, ARCHIVE_FILE)); } catch (e) { return { st: e && e.code === "ENOENT" ? "absent" : "corrupt" }; }
  let o = null;
  try { o = JSON.parse(String(raw)); } catch { return { st: "corrupt" }; }
  if (!validEnvelopeArchiveInner(o)) return { st: "corrupt" };
  const ab = o.alwaysBlocker.map((s) => s.trim());
  const opt9 = (k) => Array.isArray(o[k]) && o[k].length === ab.length ? o[k].map((s) => s.trim()) : null;
  return { st: "ok", data: { alwaysBlocker: ab }, dataEn: { alwaysBlocker: opt9("alwaysBlockerEn") }, dataEx: { alwaysBlocker: opt9("alwaysBlockerEx") }, sha1: crypto.createHash("sha1").update(raw).digest("hex") };
}
// [v6 §1 target 차원] 전이·도장·제안본의 대상 표 — '재사용'은 매개변수화(설계검증 1차 blocker①): 파일·계약
// 필드·strict 판독기를 이 표 하나가 정의하고, WAL·제안본의 target(부재=core legacy)이 행을 고른다.
function normEnvelopeTarget(tg) { return tg === "archive" ? "archive" : "core"; }
function envelopeTargetDef(tg) {
  return normEnvelopeTarget(tg) === "archive"
    ? { target: "archive", file: ARCHIVE_FILE, hashField: "archiveHash", validInner: validEnvelopeArchiveInner, label: "서고" }
    : { target: "core", file: ENVELOPE_FILE, hashField: "envelopeHash", validInner: validEnvelopeProposalInner, label: "코어" };
}
...
function envelopeInjectionFor(repo, approvedHash, lang) {
  ...
  const axes = {}; for (const ax of ENVELOPE_AXES) axes[ax] = ev.data[ax].length;
  return { text: L.join("\n"), warn: ev.truncated ? "truncated" : null, st: "ok", sha1: ev.sha1, axes };
}
...
function readEnvelopeProposal(ws, repo) {
  ...
  const tgt9 = envelopeTargetDef(o.target); // [v6 §1] target 부재=core(legacy 제안본 호환)
  if (!tgt9.validInner(inner)) return { st: "corrupt" }; // 대상별 정본 reader와 같은 축의 strict(재검증 blocker① 계보)
  // [v6 §1 이중 결속(설계검증 2차 blocker①)] targetBaseHash=대상 파일 drift 검사용 · candidateGeneration=
...
function writeEnvelopeProposal(ws, repo, proposalText, note, meta) {
  ...
  try { const inner = JSON.parse(proposalText); if (!tgt9.validInner(inner)) return { ok: false, error: tgt9.target === "archive" ? "제안 전문이 서고 계약(verify-envelope-archive-v1·ab축·96항·항목 200자)에 맞지 않음" : "제안 전문이 정본 수칙서 계약(3축 문자열 배열·축 12·항목 200자)에 맞지 않음" }; } catch { return { ok: false, error: "제안 전문 JSON 파싱 실패" }; }
  // [v6 §1 이중 결속] targetBaseHash=대상 파일 현행 지문(부재=null — 최초 서고 생성 허용) ·
  // candidateGeneration=core 승인 세대(후보 장부 축 — 복원 조회는 항상 이 값). legacy 필드 baseHash는
  // targetBaseHash와 동치로 유지(구판 판독기 호환).
  let baseHash = null;
  try { const raw9 = fs.readFileSync(path.join(repo, tgt9.file), "utf8"); baseHash = sha1Of(raw9); } catch { baseHash = null; }
  let candidateGeneration = null; try { candidateGeneration = loadContract(ws).envelopeHash || null; } catch { candidateGeneration = null; }
  ...
  if (pr && pr.st === "ok" && cids.length) {
    ...
    try {
      ...
      for (const cid of cids) {
        const rec9 = latest.get(cid + "@" + gen0);
...
// [승인 지문 언어 공유 2026-08-22 사용자 실보고] 수칙서 파일은 병렬 축(En/Ex)을 가진 '한 문서'인데
// 승인 지문이 언어 슬롯별 계약 파일에 따로 저장돼, ko에서 도장 찍으면 en은 미승인 취급(카드 '승인 대기'·
// en 검증 무주입)이 되던 비의도 분리 봉합 — 도장은 문서 전문에 찍히므로 지문 기록도 양 슬롯 동시.
// 반환: 성공 슬롯 수(2=완전·1=부분[호출자가 경고]·0=실패).
function setContractHashAllSlots(ws, field, hash) { // [v6 §1] 필드 매개변수화(envelopeHash|archiveHash) — 양 슬롯 동시 계보 공유
  let okN = 0;
  for (const lg of LANGS) { try { if (updateContractPatch(ws, lg, { [field]: hash }).ok) okN++; } catch { /* 부분 실패=okN에 반영 */ } }
  return okN;
}
function setEnvelopeHashAllSlots(ws, hash) { return setContractHashAllSlots(ws, "envelopeHash", hash); }
function applyEnvelopeTransition(ws, repo, lang, wal) {
  ...
  try { return applyEnvelopeTransitionLocked(ws, repo, lang, wal); } finally { releaseEnvelopeTransLock(ws, lk.token); }
}
// 잠금 보유형 본체(확인검증 blocker f-6c81a2d4): 호출자가 전이 잠금을 이미 보유한 구간 전용 — 모듈 밖 공개 금지.
// 도장 래퍼가 '확인→WAL→전이'를 한 잠금 아래 묶을 수 있게 분리(잠금 밖 확인=TOCTOU로 병행 초안 삭제 위험).
function applyEnvelopeTransitionLocked(ws, repo, lang, wal) {
  {
    ...
    if (!wal) { // 신규 전이: 제안본에서 WAL 구성(복구 재개는 기존 WAL 전달)
      ...
      const tgtN = envelopeTargetDef(pr.target); // [v6 §1] 대상=제안본의 target(부재=core legacy)
      let oldText = null, oldHash = null;
      try { oldText = fs.readFileSync(path.join(repo, tgtN.file), "utf8"); oldHash = sha1Of(oldText); } catch (e) { if (!e || e.code !== "ENOENT") return { ok: false, reason: "old-unreadable" }; }
      // [기억 권위 A-4·구현검증 1차 blocker②] 승인 시점 base 재검사 — 제안 생성 이후 대상 파일이 직접
      // 편집됐으면 그 편집이 '구본'으로 오인돼 덮여 사라진다. 검사 기준=targetBaseHash(v6 이중 결속 —
      // archive 초안이면 archiveHash·legacy=baseHash 겸용). 거부=WAL 미구성(아무것도 안 바꿈).
      if ((pr.targetBaseHash || null) !== (oldHash || null)) return { ok: false, reason: "base-drift" };
      wal = { schema: "env-trans-wal-v1", target: tgtN.target, ws: String(ws), repo: String(repo), lang: lang || null, oldText, oldHash, newText: pr.proposalText, newHash: pr.newHash, ts: new Date().toISOString() };
      if (!atomicWrite(walFile, JSON.stringify(wal, null, 1))) return { ok: false, reason: "wal-write" };
    }
    const tgtW = envelopeTargetDef(wal.target); // WAL의 target(부재=core legacy) — 복구도 같은 행(교차 덮침 원천 차단)
    // ① 원본 교체(원본=신 해시면 멱등 통과 — 복구 재개 분기)
    let curHash = null; try { curHash = sha1Of(fs.readFileSync(path.join(repo, tgtW.file), "utf8")); } catch { curHash = null; }
    if (curHash !== wal.newHash) {
      if (curHash !== wal.oldHash) return { ok: false, reason: "drift" }; // 전이 중 제3 변경=중단(WAL 보존 — 사용자 재확인)
      if (!atomicWrite(path.join(repo, tgtW.file), wal.newText)) return { ok: false, reason: "replace-failed" };
    }
    // ② 계약 해시 기록(멱등) — 대상 필드(envelopeHash|archiveHash)만. 다른 대상 필드는 무접촉(교차 반례 검수).
    // [재검증 blocker f-71d4c2a8] 부분 성공(1슬롯)을 완료로 확정하면 반대 언어 검증이 수칙서를 계속 누락한다
    // — 전 슬롯 미만=실패 반환·WAL 보존(복구 스캐너의 멱등 재시도가 전 슬롯 수렴할 때까지 정리 금지).
    // [경계 통일 2026-08-29 · 검증 2회차 blocker③] 승인본 사본은 전이의 "성공 조건" — 도장(②)보다 먼저 기록하고, 실패면
    // 전이를 실패로 되돌린다(WAL 보존 → recover-needed → 복구 스캐너 멱등 재시도). 사본 없이 도장만 찍히면 이후 승인 없는
    // 변경을 "마지막 승인 목록으로 복구"할 길이 없다. 재시도 멱등: 같은 지문 사본이 이미 있으면 통과.
    {
      const cpF = envelopeApprovedCopyFileFor(ws, tgtW.target);
      let have = false; try { const o9 = JSON.parse(fs.readFileSync(cpF, "utf8")); have = !!(o9 && o9.hash === wal.newHash && o9.text === wal.newText); } catch { have = false; }
      if (!have) {
        try { fs.mkdirSync(VERIFY_FINDINGS_DIR, { recursive: true }); } catch { /* atomicWrite가 판정 */ }
        let okCp = false; try { okCp = atomicWrite(cpF, JSON.stringify({ schema: "env-approved-copy-v1", target: tgtW.target, hash: wal.newHash, text: wal.newText, ts: new Date().toISOString() }, null, 1)); } catch { okCp = false; }
        if (!okCp) return { ok: false, reason: "approved-copy-write" };
      }
    }
    const upN = setContractHashAllSlots(ws, tgtW.hashField, wal.newHash); // 양 슬롯(언어 공유 — 2026-08-22)
    if (upN < LANGS.length) return { ok: false, reason: upN >= 1 ? "contract-write-partial" : "contract-write" };
    // ③ 정리 — WAL 완료. 제안본 폐기는 '이 WAL이 소비한 그 제안본'만: 도장 WAL(kind:stamp)은 proposal과 무관
...
function envelopeApprovedCopyFileFor(ws, target) { return path.join(VERIFY_FINDINGS_DIR, wsKeyFor(ws) + "." + normEnvelopeTarget(target) + ".approved.json"); }
function readEnvelopeApprovedCopy(ws, target) {
  let o = null;
  try { o = JSON.parse(fs.readFileSync(envelopeApprovedCopyFileFor(ws, target), "utf8")); } catch (e) { return { st: e && e.code === "ENOENT" ? "absent" : "corrupt" }; }
  if (!o || o.schema !== "env-approved-copy-v1" || typeof o.text !== "string" || !/^[0-9a-f]{40}$/.test(String(o.hash || "")) || sha1Of(o.text) !== o.hash) return { st: "corrupt" };
  return { st: "ok", hash: o.hash, text: o.text, ts: String(o.ts || "") };
}
// [경계 통일 2026-08-29] 승인 없이 바뀐 파일의 차이 — 승인본 사본(도장 지문 일치) vs 현재 파일. 항목 단위(축별)·정규화 문안 대조.
// hasCopy=false(구 승인·사본 소실)면 added/removed 없이 currentItems만(복구 불가·승인만 가능).
function envelopeDriftView(ws, repo, target) {
  const tg = envelopeTargetDef(target);
  let approvedHash = null; try { approvedHash = loadContract(ws)[tg.hashField] || null; } catch { approvedHash = null; }
  if (!approvedHash) return { ok: false, reason: "not-approved" };
  let curText = null, curAbsent = false; try { curText = fs.readFileSync(path.join(repo, tg.file), "utf8"); } catch (e) { curAbsent = !!(e && e.code === "ENOENT"); curText = curAbsent ? "" : null; }
  const currentHash = curText === null ? null : sha1Of(curText);
  const axes = tg.target === "archive" ? ["alwaysBlocker"] : ENVELOPE_AXES.slice();
  const itemsOf = (txt) => { let o = null; try { o = JSON.parse(txt); } catch { return null; } if (!o || typeof o !== "object") return null; const out = []; for (const ax of axes) if (Array.isArray(o[ax])) for (const x of o[ax]) if (typeof x === "string" && x.trim()) out.push({ axis: ax, text: x.trim() }); return out; };
  const curItems = curText === null ? null : (curAbsent ? [] : itemsOf(curText)); // 부재=빈 목록(복구 가능)·0바이트/손상=판독 불가
  const copy = readEnvelopeApprovedCopy(ws, target);
  const hasCopy = copy.st === "ok" && copy.hash === approvedHash;
  const out = { ok: true, target: tg.target, approvedHash, currentHash, currentReadable: curItems !== null, currentAbsent: curAbsent, currentItems: curItems || [], hasCopy, added: [], removed: [], drift: currentHash !== approvedHash }; // currentAbsent=파일 부재(승인 불가·복구만 — 3회차 보완)
  if (!hasCopy || curItems === null) return out;
  const appItems = itemsOf(copy.text) || [];
  const key = (it) => it.axis + "|" + normBacklogTitle(it.text);
  const appSet = new Set(appItems.map(key)), curSet = new Set(curItems.map(key));
  out.added = curItems.filter((it) => !appSet.has(key(it)));
  out.removed = appItems.filter((it) => !curSet.has(key(it)));
  return out;
}
// 마지막 승인 목록으로 복구 — 현재 파일을 백업 폴더에 복사한 뒤(실패=중단·파괴 0) 승인본 사본을 그대로 쓴다. 전이 잠금 아래·WAL/초안 잔존=거부.
function restoreEnvelopeApproved(ws, repo, target) {
  const tg = envelopeTargetDef(target);
  const lk = acquireEnvelopeTransLock(ws);
  if (!lk.ok) return { ok: false, reason: lk.reason };
  try {
    try { fs.accessSync(envelopeTransWalFileFor(ws)); return { ok: false, reason: "recover-needed" }; } catch { /* WAL 없음 */ }
    let prSt = "corrupt"; try { prSt = readEnvelopeProposal(ws, repo).st; } catch { /* fail-closed */ }
    if (prSt !== "absent") return { ok: false, reason: "proposal-pending" };
    let approvedHash = null; try { approvedHash = loadContract(ws)[tg.hashField] || null; } catch { approvedHash = null; }
    if (!approvedHash) return { ok: false, reason: "not-approved" };
    const copy = readEnvelopeApprovedCopy(ws, target);
    if (copy.st !== "ok" || copy.hash !== approvedHash) return { ok: false, reason: copy.st === "ok" ? "stale-copy" : "no-approved-copy" };
    const file = path.join(repo, tg.file);
    const bdir = path.join(BRIDGE_DIR, "envelope-backup");
    const backup = path.join(bdir, wsKeyFor(ws) + "-" + tg.target + "-" + new Date().toISOString().replace(/[:.]/g, "-") + ".json");
    let cur = null; try { cur = fs.readFileSync(file); } catch (e) { if (!e || e.code !== "ENOENT") return { ok: false, reason: "file-unreadable" }; }
    if (cur !== null) { try { fs.mkdirSync(bdir, { recursive: true }); fs.writeFileSync(backup, cur); if (!fs.readFileSync(backup).equals(cur)) return { ok: false, reason: "backup-failed" }; } catch { return { ok: false, reason: "backup-failed" }; } }
    if (!atomicWrite(file, copy.text)) return { ok: false, reason: "write-failed" };
    let after = null; try { after = sha1Of(fs.readFileSync(file, "utf8")); } catch { after = null; }
    if (after !== approvedHash) return { ok: false, reason: "verify-failed" };
    return { ok: true, backup: cur === null ? "" : backup };
  } finally { releaseEnvelopeTransLock(ws, lk.token); }
}
// [직접 승인 도장 2026-08-22 확인검증 blocker f-71d4c2a8 연속분] 파일 무변 도장도 전이 계약(WAL)을 재사용 —
// 슬롯 직접 기록은 부분 성공(ko만 영속)이 '조용한 반쪽 승인'으로 남는다: 성공 슬롯은 즉시 승인 표시·주입되고
// 승인 버튼이 사라져 재시도 수렴도 없다. WAL 경유면 부분 실패=WAL 잔존 → 검증 시작·주입 차단(recover-needed)
// +복구 스캐너 멱등 재시도로 전 슬롯 수렴. old==new라 원본 교체는 멱등 통과, 순수 지문 기록 트랜잭션이 된다.
function stampEnvelopeAllSlots(ws, repo, sha, target) {
  // [f-6c81a2d4] 확인(proposal 부재·WAL 부재)→WAL 기록→전이를 '한 전이 잠금' 아래 수행 — 잠금 밖 확인이면
  // draft·수동 propose writer(같은 잠금 사용)가 사이에 초안을 만들고, 정리 단계가 그 초안을 삭제하는 TOCTOU.
  const tgtS = envelopeTargetDef(target); // [v6 §1] 재도장도 대상 표를 탄다(서고 소실 복구 재도장 포함)
  const lk = acquireEnvelopeTransLock(ws);
  if (!lk.ok) return { ok: false, reason: lk.reason };
  try {
    let txt = null;
    try { txt = fs.readFileSync(path.join(repo, tgtS.file), "utf8"); } catch { return { ok: false, reason: "file-unreadable" }; }
    if (sha1Of(txt) !== sha) return { ok: false, reason: "sha-drift" }; // 도장 대상=사용자가 본 전문(경합 변경=거부)
    // [1단계 재검증 blocker f-d9c9930d] sha 일치만으론 '도장 대상이 아닌 전문'(손상·96항 초과·타 축)에
    // 지문이 찍힌다 — 도장 전 대상별 strict 검증(전이의 제안본 검증과 같은 판독기·실패=거부 fail-closed).
    let inner9 = null; try { inner9 = JSON.parse(txt); } catch { return { ok: false, reason: "target-corrupt" }; }
    if (!tgtS.validInner(inner9)) return { ok: false, reason: "target-corrupt" };
    let prSt = "corrupt"; try { prSt = readEnvelopeProposal(ws, repo).st; } catch { /* 판독 불가=fail-closed */ }
    if (prSt !== "absent") return { ok: false, reason: "proposal-pending" }; // 초안 폐기 오발 방지(corrupt 포함 거부)
    try { fs.accessSync(envelopeTransWalFileFor(ws)); return { ok: false, reason: "recover-needed" }; } catch { /* WAL 없음=진행 */ }
    const wal = { schema: "env-trans-wal-v1", kind: "stamp", target: tgtS.target, ws: String(ws), repo: String(repo), lang: null, oldText: txt, oldHash: sha, newText: txt, newHash: sha, ts: new Date().toISOString() };
    const walFile = envelopeTransWalFileFor(ws);
    try { fs.mkdirSync(path.dirname(walFile), { recursive: true }); } catch { /* atomicWrite가 실패 반환 */ }
    if (!atomicWrite(walFile, JSON.stringify(wal, null, 1))) return { ok: false, reason: "wal-write" };
    return applyEnvelopeTransitionLocked(ws, repo, null, wal);
  } finally { releaseEnvelopeTransLock(ws, lk.token); }
}
...
// [ab-1 후보 파티션] 후보가 태어난 정찰 대상(저장소)의 정규화 지문 — 같은 작업공간(wsKey)이라도 정찰 대상이 바뀌면 다른 프로젝트.
function repoKeyOf(repo) { return crypto.createHash("sha1").update("repo:" + normWs(String(repo || ""))).digest("hex").slice(0, 16); }
// [ab-1] 약속 발화 후보의 저장소 결속값 — 훅이 원문 스냅숏을 기록하는 그 시점의 정찰 대상(계약 스냅샷 우선). 판독 불가=null.
...
// ── 결정 장부(HARNESS-REALIGNMENT-2026-08-29 §4 개선 2 — 2026-08-30 구현 1단계) ──
// 원칙(사용자 방향 2026-08-30 재확정): 결정 행은 **구현자의 판단**으로만 생긴다 — 검증자 지적·하네스 상태가 자동으로 사용자
// 결정을 만들지 않는다(그러면 쓸데없는 항목이 계속 사용자에게 빠짐 — 구현자 판단을 빼 버린 설계는 고치려던 병의 재발).
// 구현자는 지적마다 수용/반박/보관함/수칙서 후보/추가 검증을 스스로 정하고, "사용자가 방향을 정해야 하는 것"만 이 장부에 구조 채널
// (decisions raise: 종류·질문·왜·기본값이 없는 이유·선택지·권장)로 올린다. 하네스는 진위를 가리지 않고(뜻을 읽는 일) 형식·기록·
// 개수(지표)만 맡는다. 사용자 역할 모델(사용자=철학·방향, 개입=승인·빼기·"네가 정해라"). 장부 계약(후보 장부 규약 재사용):
// 추가 전용 jsonl · 생성 행(open)/결과 행(chosen|delegated) 분리 · 같은 id는 마지막 행이 이김 · 결과가 원항목을 덮지
// 않음 · 기록 뒤 read-back · 선택 시 대상 지문(targetFp) 재대조(다른 창·다른 세대의 항목을 종결하지 못함) · wsKey 결속(ab-1).
const DECISIONS_DIR = path.join(BRIDGE_DIR, "decisions");
...
function decisionRowValid(r) {
  if (!r || typeof r !== "object" || r.schema !== "decision-v1") return false;
  if (typeof r.decisionId !== "string" || !/^[a-f0-9]{16}$/.test(r.decisionId)) return false;
  if (!DECISION_STATUSES.includes(r.status)) return false;
  if (typeof r.wsKey !== "string" || !r.wsKey) return false;
  if (typeof r.ts !== "string" || !r.ts) return false;
  if (r.status === "open") {
    if (!DECISION_ORIGINS.includes(r.origin)) return false;
    if (!DECISION_KINDS.includes(r.kind)) return false;
    if (typeof r.noDefault !== "string" || r.noDefault.trim().length < DECISION_NO_DEFAULT_MIN) return false; // 기본값 없는 이유 필수(형식)
    if (typeof r.campaignId !== "string" || typeof r.sourceAsk !== "string" || typeof r.targetFp !== "string") return false;
    if (typeof r.question !== "string" || !r.question.trim()) return false;
    if (!Array.isArray(r.choices) || !r.choices.length) return false;
    if (r.choices.some((c) => !c || typeof c !== "object" || typeof c.key !== "string" || !c.key || c.key === DECISION_DELEGATE_KEY || typeof c.label !== "string" || !c.label)) return false;
...
function appendDecisionRows(ws, recs) {
  if (!Array.isArray(recs) || !recs.length) return { ok: false, reason: "empty" };
  const wk = wsKeyFor(ws);
  for (const r of recs) if (!decisionRowValid(r) || r.wsKey !== wk) return { ok: false, reason: "invalid-row" };
  const file = decisionsFileFor(ws);
  const chunk = recs.map((r) => JSON.stringify(r)).join("\n") + "\n";
  try { fs.mkdirSync(DECISIONS_DIR, { recursive: true }); fs.appendFileSync(file, chunk); } catch { return { ok: false, reason: "write-failed" }; }
  try { if (!String(fs.readFileSync(file, "utf8")).endsWith(chunk)) return { ok: false, reason: "read-back-mismatch" }; } catch { return { ok: false, reason: "read-back-failed" }; }
  return { ok: true };
}
// 결정 행 생성 — 멱등(같은 ws·캠페인·출처·ask·대상 지문=같은 id, 이미 열려 있으면 재생성 없음)
function openDecision(ws, spec) {
  spec = spec || {};
...
function resolveDecision(ws, decisionId, choiceKey, opts) {
  ...
  if (d.targetFp) {
    if (typeof opts.currentFp !== "string") return { ok: false, reason: "current-fp-required" };
    if (opts.currentFp !== d.targetFp) return { ok: false, reason: "target-drift" };
  }
...
// ── 결정 블록 서식(사용자 편집 가능 — 기본 원칙과 같은 편집 계층 · 2026-08-30 사용자 지적 "서식을 코드에 박지 마라") ──
// 서식은 코드 상수가 아니라 기본지침 파일(core·언어별)의 `decisionBlock` 키에 사는 편집 가능한 값이다. 렌더(CLI decisions render·마감문)와
// 마감 검사기(정본 대조)가 같은 loadDecisionTemplate을 읽으므로 사용자가 낱말을 고치면 둘 다 함께 따라간다. 필수 자리표시자가 빠진
// 서식은 fail-visible: 기본 서식으로 렌더하고 templateWarn을 남긴다(정보를 떨어뜨리는 서식으로 조용히 바뀌지 않음).
const DECISION_TEMPLATE_DEFAULTS = {
  ko: {
    header: "- 결정 {{id}}: {{question}}",
    why: "  왜: {{why}}",
...
// 필수 자리표시자(정보 무손실 계약): header={{id}}{{question}} · why={{why}} · noDefault={{noDefault}} · choice={{key}}{{label}}{{ifChosen}}(고르면 어떻게 되는지는 뺄 수 없다 — 3회차 blocker ab-3) · recommend={{recommend}} · answer={{id}}
const DECISION_TEMPLATE_SLOTS = { header: ["{{id}}", "{{question}}"], why: ["{{why}}"], noDefault: ["{{noDefault}}"], choice: ["{{key}}", "{{label}}", "{{ifChosen}}"], recommend: ["{{recommend}}"], answer: ["{{id}}"] };
function validateDecisionTemplate(tpl) {
  if (!tpl || typeof tpl !== "object") return { ok: false, reason: "not-object" };
  for (const [k, slots] of Object.entries(DECISION_TEMPLATE_SLOTS)) {
    const v = tpl[k];
    if (typeof v !== "string" || !v.trim()) return { ok: false, reason: "missing:" + k };
    if (/\r|\n/.test(v)) return { ok: false, reason: "multiline:" + k };
    for (const s of slots) if (!v.includes(s)) return { ok: false, reason: "slot:" + k + ":" + s };
  }
  // 줄 구분 계약(4회차 blocker): 검사기는 줄 머리말로 헤더/본문을 가르므로 ① 여섯 줄 모두 자리표시자 앞에 리터럴 머리말이 있어야 하고
  // ② 헤더 머리말과 다른 줄 머리말이 서로의 접두어면 안 된다(공백 정규화·대소문자 무시) — 모호한 서식은 저장 단계에서 거부(렌더=검사 보장).
  const pre = (s) => String(s).split("{{")[0].replace(/\s+/g, " ").trim().toLowerCase();
  const heads = {};
  for (const k of Object.keys(DECISION_TEMPLATE_SLOTS)) { heads[k] = pre(tpl[k]); if (!heads[k]) return { ok: false, reason: "prefix-required:" + k }; }
  for (const k of Object.keys(heads)) {
    if (k === "header") continue;
    if (heads[k].startsWith(heads.header) || heads.header.startsWith(heads[k])) return { ok: false, reason: "prefix-ambiguous:" + k };
  }
  return { ok: true };
}
function loadDecisionTemplate(lang) {
  const L = lang === "en" ? "en" : "ko";
  const D = DECISION_TEMPLATE_DEFAULTS[L];
  // 파일 상태 3분류(fail-visible — 3회차 blocker): 없음=기본(경고 없음) / 문법 손상=기본+warn / decisionBlock 비객체=기본+warn
  let raw = null; try { raw = fs.readFileSync(baseDirectiveFileFor(L, "core"), "utf8"); } catch (e) { if (!e || e.code !== "ENOENT") return { template: D, overridden: false, warn: "decision-template-file-unreadable" }; raw = null; }
  if (raw === null) return { template: D, overridden: false, warn: "" };
  let o = null; try { o = JSON.parse(raw); } catch { return { template: D, overridden: false, warn: "decision-template-file-corrupt" }; }
  if (!o || typeof o !== "object" || !("decisionBlock" in o)) return { template: D, overridden: false, warn: "" };
  const tpl = o.decisionBlock;
  if (!tpl || typeof tpl !== "object" || Array.isArray(tpl)) return { template: D, overridden: false, warn: "decision-template-invalid:not-object" };
  const v = validateDecisionTemplate(tpl);
  if (!v.ok) return { template: D, overridden: false, warn: "decision-template-invalid:" + v.reason };
  return { template: Object.assign({}, D, tpl, { empty: Object.assign({}, D.empty, tpl.empty && typeof tpl.empty === "object" ? tpl.empty : {}) }), overridden: true, warn: "" };
}
// 저장: 기본 서식과 같으면 오버라이드 제거. 검증 실패=저장 거부(정보 무손실). 다른 세 축(verifyBaseline·transmit·rejudge) 무접촉.
function saveDecisionTemplate(tpl, lang) {
  const L = lang === "en" ? "en" : "ko";
  const file = baseDirectiveFileFor(L, "core");
  let o = {}; try { o = JSON.parse(fs.readFileSync(file, "utf8")) || {}; } catch { o = {}; }
  if (tpl === null) { delete o.decisionBlock; }
  else {
    const v = validateDecisionTemplate(tpl); if (!v.ok) return { ok: false, reason: v.reason };
    const D = DECISION_TEMPLATE_DEFAULTS[L];
    const same = Object.keys(DECISION_TEMPLATE_SLOTS).every((k) => String(tpl[k]).trim() === D[k].trim()) && !(tpl.empty && JSON.stringify(tpl.empty) !== JSON.stringify(D.empty));
    if (same) delete o.decisionBlock; else o.decisionBlock = tpl;
  }
  fs.mkdirSync(BRIDGE_DIR, { recursive: true });
  if (!Object.keys(o).length) { try { fs.unlinkSync(file); } catch (e) { if (e && e.code !== "ENOENT") return { ok: false, reason: "unlink-failed" }; } return { ok: true }; }
  return atomicWrite(file, JSON.stringify(o, null, 2)) ? { ok: true } : { ok: false, reason: "write-failed" };
}
// 검사기용: 서식 줄 → 정규식(리터럴은 이스케이프·공백은 느슨·자리표시자는 캡처). 자리표시자를 어디에 두든(재배치 서식) 렌더와 검사가 같은 서식을 따른다.
function decisionTemplateRegex(line, slots) {
  ...
  for (const part of parts) {
    if (!part) continue;
    const m = /^\{\{(\w+)\}\}$/.exec(part);
    if (m) { order.push(m[1]); re += (m[1] === "id" ? "([a-f0-9]{16})" : (m[1] === "n" ? "(\\d+)" : "(.+?)")); continue; }
    re += part.replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/\s+/g, "\\s*");
  }
  re += "\\s*$";
  return { re: new RegExp(re, "i"), order };
}
function decisionTemplateLabels(lang) {
  const cur = loadDecisionTemplate(lang);
  const T = cur.template;
  const build = (k) => decisionTemplateRegex(T[k]);
  return { header: build("header"), why: build("why"), noDefault: build("noDefault"), choice: build("choice"), recommend: build("recommend"), answer: build("answer"), warn: cur.warn };
}
function fillTpl(s, vars) { return String(s).replace(/\{\{(\w+)\}\}/g, (m, k) => (k in vars ? String(vars[k]) : m)); }
// 결정 항목의 정본 렌더(쉬운 말 블록) — 마감문 [사용자 판단 필요] 절·CLI decisions render·화면이 전부 이 한 곳에서 조립한다.
// 마감 검사기는 붙여 넣은 블록을 이 렌더와 글자 단위(공백 정규화)로 대조한다 — 질문·선택지·권장을 바꿔치기한 블록은 위조로 거부(ab-3).
function renderDecisionBlock(d, en) {
  const { template: T } = loadDecisionTemplate(en ? "en" : "ko");
  const L = [];
  L.push(fillTpl(T.header, { id: d.decisionId, question: d.question }));
  L.push(fillTpl(T.why, { why: d.why || T.empty.why }));
  L.push(fillTpl(T.noDefault, { noDefault: d.noDefault }));
  (d.choices || []).forEach((ch, i) => L.push(fillTpl(T.choice, { n: i + 1, key: ch.key, label: ch.label, ifChosen: ch.ifChosen || T.empty.ifChosen })));
  const rec = (d.choices || []).find((ch) => ch.key === d.recommend);
  L.push(fillTpl(T.recommend, { recommend: rec ? rec.key + " — " + rec.label : T.empty.recommend }));
  L.push(fillTpl(T.answer, { id: d.decisionId }));
  return L.join("\n");
}
...
function constraintTurnContext() {
  ...
  if (csid) { // 판독 권위=claudeCampaignAnchor와 동형(세션별 active 우선·전역은 같은 세션일 때만)
    ...
    return { ok: true, ...idc, turnAnchor: a.constraintAnchor, sourceHash: a.constraintSourceHash, repoKey: (typeof a.constraintRepoKey === "string" && /^[0-9a-f]{16}$/.test(a.constraintRepoKey)) ? a.constraintRepoKey : null }; // [ab-1] 원문 시점 저장소=훅 기록값만
  }
  ...
  if (xsid) {
    ...
    return { ok: true, ...idx, turnAnchor: a.constraintAnchor, sourceHash: a.constraintSourceHash, repoKey: (typeof a.constraintRepoKey === "string" && /^[0-9a-f]{16}$/.test(a.constraintRepoKey)) ? a.constraintRepoKey : null }; // [ab-1] 원문 시점 저장소=훅 기록값만
  }
  return { ok: false, reason: "no-session" };
}
// ── [부품 B §2] 검증자 회수 그물 — 규격 블록 파서(문법 동결분) ────────────────────────
// 여는/닫는 마커는 언어 무관 단일 세트(동결). 위치=지적 블록보다 '앞'(지적 파서가 블록 뒤 잔여를 손상 처리).
// 줄당 JSON {"quote","why"}만·최대 4행·행당 1KB. 손상 하나=블록 전량 거부(부분 수용 없음 — fail-closed).
const CONSTRAINT_BLOCK_MARKERS = { start: "[제약 후보 v1]", end: "[제약 후보 끝]" };
const CONSTRAINT_BLOCK_MAX_ROWS = 4;
const CONSTRAINT_BLOCK_LINE_MAX = 1024;
// [확인검증 blocker] 중복 키를 원문 정규식 출현 횟수로 세면 이스케이프 키("quo\\u0074e")가 우회한다 —
// JSON 문자열 토큰을 실제로 해독해 최상위 키 목록을 회수하는 스캐너(구조 이상=null → 상위가 전량 거부).
function jsonTopLevelKeys(ln) {
  let i = 0; const n = ln.length; const keys = [];
  const ws = () => { while (i < n && /\s/.test(ln[i])) i++; };
  const str = () => { const s0 = i; i++; while (i < n) { const ch = ln[i]; if (ch === "\\") { i += 2; continue; } if (ch === '"') { i++; return ln.slice(s0, i); } i++; } return null; };
  const skipValue = () => {
    ws();
    if (ln[i] === '"') return str() !== null;
    let depth = 0;
    while (i < n) {
      const ch = ln[i];
  ...
  ws(); if (ln[i] !== "{") return null; i++;
  ws(); if (ln[i] === "}") return keys;
  while (i < n) {
    ws(); if (ln[i] !== '"') return null;
    const tok = str(); if (tok === null) return null;
    let key; try { key = JSON.parse(tok); } catch { return null; }
    keys.push(key);
    ws(); if (ln[i] !== ":") return null; i++;
...
function parseConstraintBlock(text) {
  const out = { present: false, ok: false, items: [], reason: "" };
  const lines = String(text || "").split(/\r?\n/);
  let s = -1, e = -1, extra = false;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i] === CONSTRAINT_BLOCK_MARKERS.start) { if (s >= 0) extra = true; s = i; }
    else if (lines[i] === CONSTRAINT_BLOCK_MARKERS.end) { if (e >= 0) extra = true; e = i; }
  }
  if (s < 0 && e < 0) return out; // 부재=정상(선택 절)
  out.present = true;
  if (extra || s < 0 || e < 0 || e < s) { out.reason = "marker"; return out; }
  for (let i = 0; i < lines.length; i++) { // 위치 계약: 지적 블록 시작 마커보다 앞
    const ln = lines[i];
    if (ln === FINDINGS_MARKERS.ko.start || ln === FINDINGS_MARKERS.en.start || ln === FINDINGS_MARKERS_V2.ko.start || ln === FINDINGS_MARKERS_V2.en.start) {
      if (e > i) { out.reason = "position"; return out; }
      break;
    }
  }
  const rows = [];
  for (let i = s + 1; i < e; i++) {
    const ln = lines[i];
    if (!ln.trim()) continue;
    if (Buffer.byteLength(ln, "utf8") > CONSTRAINT_BLOCK_LINE_MAX) { out.reason = "line-too-long"; return out; } // 상한=UTF-8 바이트(코드 단위 아님 — 한글 행 우회 봉합)
    // 중복 키는 JSON.parse가 마지막 값으로 삼켜 검출 불가 — 이스케이프 키까지 해독하는 최상위 키 스캐너로
    // 검사(정규식 출현 횟수는 "quo\\u0074e" 우회가 실측됨 — 확인검증 blocker).
    const keys9 = jsonTopLevelKeys(ln);
    if (!keys9 || new Set(keys9).size !== keys9.length) { out.reason = "dup-or-missing-key"; return out; } // 구조 이상·중복 키(이스케이프 해독 후)
    if (keys9.length !== 2 || !keys9.includes("quote") || !keys9.includes("why")) { out.reason = "unknown-field"; return out; } // 키 집합={quote,why} 정확히
    let o; try { o = JSON.parse(ln); } catch { out.reason = "bad-json"; return out; }
    if (!o || typeof o !== "object" || Array.isArray(o)) { out.reason = "not-object"; return out; }
    if (Object.keys(o).length !== 2 || typeof o.quote !== "string" || typeof o.why !== "string") { out.reason = "unknown-field"; return out; }
    if (/[\r\n\u2028\u2029]/.test(o.quote) || /[\r\n\u2028\u2029]/.test(o.why)) { out.reason = "newline-value"; return out; }
    rows.push({ quote: o.quote, why: o.why });
    if (rows.length > CONSTRAINT_BLOCK_MAX_ROWS) { out.reason = "too-many-rows"; return out; }
  }
  out.items = rows; out.ok = true; return out; // 빈 블록(0행)=ok(무해)
}
// [부품 B §2] 회수 본체 — 대조 권위=부품 A와 같은 훅 원문 스냅샷뿐(검증 요청문·주입문 대조 금지: 구현자
// 작문·계약/지도 문구 위장 차단). ctx=ask 생성 시 job에 동결된 {provider, sessionId, turnAnchor, sourceHash}.
// ctx 부재(직접 ask — 시작 시점 동결 carrier 없음)=블록 전량 무시+direct-ask 거부 영수증(§2 의식적 한정).
function constraintHarvestFromAnswer(ws, answerText, ctx) {
  const out = { present: false, ok: false, accepted: 0, rejected: 0, ignored: false, reason: "" };
  let pb; try { pb = parseConstraintBlock(answerText); } catch { return out; }
  if (!pb.present) return out;
  out.present = true;
  const base = { ts: new Date().toISOString(), provider: (ctx && ctx.provider) || "", wsKey: wsKeyFor(ws), sessionId: (ctx && ctx.sessionId) || "", turnAnchor: (ctx && ctx.turnAnchor) || "", quoteFp: "", quoteLen: 0, whyLen: 0, via: "verifier-block" };
  if (!ctx || !ctx.turnAnchor || !ctx.sourceHash) {
    out.ignored = true; out.reason = "direct-ask";
    appendConstraintUsage({ ...base, outcome: "rejected", reason: "direct-ask" });
    return out;
  }
  if (!pb.ok) { out.reason = pb.reason; appendConstraintUsage({ ...base, outcome: "rejected", reason: "block-" + pb.reason }); return out; } // 손상=전량 거부
  // job 결속 선검사(§6): 스냅샷 부재·지문 불일치=전량 거부 단일 영수증(행별 게이트가 재확인하지만 사유를 블록 단위로 명확화).
  let snap9 = null; try { snap9 = fs.readFileSync(constraintTurnFileFor(ws, ctx.turnAnchor), "utf8"); } catch { snap9 = null; }
  if (snap9 === null) { out.reason = "snapshot-missing"; appendConstraintUsage({ ...base, outcome: "rejected", reason: "block-snapshot-missing" }); return out; }
  if (sha1Of(snap9) !== ctx.sourceHash) { out.reason = "snapshot-mismatch"; appendConstraintUsage({ ...base, outcome: "rejected", reason: "block-snapshot-mismatch" }); return out; }
  out.ok = true;
  for (const it of pb.items) {
    const r = constraintAdd(ws, it.quote, it.why, "", { ok: true, provider: (ctx.provider || ""), sessionId: (ctx.sessionId || ""), turnAnchor: ctx.turnAnchor, sourceHash: ctx.sourceHash, repoKey: ctx.repoKey, via: "verifier-block" }); // repoKey=job 동결값(ab-1)
    if (r.ok) out.accepted++; else out.rejected++; // duplicate=구현자 상신과 자연 합류(지문 dedupe — 거부 영수증이 근거)
  }
  return out;
}
...
// ctx.via="verifier-block"(부품 B 회수)=턴당 상한 비적용(블록 자체가 4행 유계·§2)·영수증에 via 표기.
function constraintAdd(ws, quote, why, scope, ctx) {
  ...
  const base = () => ({
    ...
    ...(cid.via ? { via: String(cid.via) } : {}), // 부품 B 회수 경로 표기(출처 감사)
  });
  ...
  // [ab-1 후보 파티션 · 5회차 blocker] 태어난 정찰 대상 지문은 원문 결속 문맥(ctx.repoKey — 원문 스냅샷과 같은 턴에
  // 동결된 값)에서만 읽는다. 회수 시점의 현재 계약 재판독은 금지 — 검증 응답을 기다리는 사이 정찰 대상을 바꾸면
  // A 원문에 B 표식이 붙는다(정상 전환 경합). 부재·형식 이상=기록하지 않음(무표기 생산 금지·fail-closed).
  const repoKeyC = (typeof c.repoKey === "string" && /^[0-9a-f]{16}$/.test(c.repoKey)) ? c.repoKey : null;
  if (!repoKeyC) return reject("repo-unknown");
  // candidateId=전체 원문 지문(200자 절단 정규화 아님 — §1·2차 blocker③: scope는 ID에 넣지 않는다)+저장소 파티션.
  const qNorm = q.normalize("NFC").replace(/\s+/g, " ").trim();
  const candidateId = envelopeCandidateId("user-constraint", wsKeyFor(ws) + "|" + repoKeyC + "|" + sha1Of(qNorm));
  const { rows, latest } = readEnvelopeCandidates(ws);
  ...
  // 턴당 상한(§1-⑦): 같은 turnAnchor로 등록 성공한 distinct 후보 수. 부품 B 회수(via=verifier-block)는
  // 블록 자체가 4행 유계라 비적용(§2 — A 상한과 별개 채널·둘 다 pending 12 합산 상한은 아래에서 공유).
  if (c.via !== "verifier-block") {
  ...
  const rec = {
    candidateId, envelopeHash: gen, status: "proposed", kind: "user-constraint", repoKey: repoKeyC,
...
// Envelope 활성(승인 지문 존재) 프로젝트 한정 — 비활성은 ab 전달 경로 자체가 없어 공급 무의미(부트스트랩은 기존 흐름).
const MEMORY_CANDIDATE_PENDING_MAX = 12; // pending 상한 = ab축 상한과 동일 값(별도 상수 — 승인 요청 폭주 방지)
// [부품 C §3-3 draft allowlist] '채택=병합 초안 생성과 결속'이 강제되는 kind — 계산기 합류·대시보드 채택 분기·
// mark 우회 가드가 이 한 목록을 공유한다(목록 불일치=한쪽만 초안 없이 adopted가 기록되는 고아 재발).
...
function envelopeMarkGuard(ws, candidateId, status) {
  if (status !== "adopted") return { ok: true, kind: "" };
  let kind = "";
  try { for (const r of readEnvelopeCandidates(ws).rows) if (r && r.candidateId === candidateId && r.kind) { kind = r.kind; break; } } catch { /* 판독 실패=비 draftable 취급(기존 계약 유지) */ }
  if (ENVELOPE_DRAFTABLE_KINDS.includes(kind)) return { ok: false, reason: "draftable-adopt-via-draft", kind };
  return { ok: true, kind };
}
function reconcileMemoryCandidates(ws, repo, approvedHash) {
  const out = { scanned: 0, appended: 0, suppressed: [], legacyUnbound: 0 };
  if (!approvedHash) return out; // 비활성 — A-6 경계
  let env; try { env = readVerifyEnvelope(repo); } catch { return out; }
  if (!env || env.st !== "ok" || env.sha1 !== approvedHash) return out; // 미승인 변경/손상=공급 보류(주입 규칙과 동형)
  // 잠금: 전이 잠금 1회 시도 — 실패=이번 트리거 스킵(멱등이라 다음 트리거가 처리). 전이와의 원자성만 필요.
  const lk = acquireEnvelopeTransLock(ws);
  if (!lk.ok) return out;
  try {
    const { rows: candRows, latest } = readEnvelopeCandidates(ws); // 지적 장부 판독은 폐지된 본 스캔과 함께 제거(재편 A)
    ...
    // [A-5 stale 생명주기 → 부품 C §3-2 kind 공통 확장] 승인 세대 변경·고아를 kind별 규칙으로 정리:
    // · resolved-blocker 구세대 proposed=declined 정리(원천이 지적 장부라 아래 본 스캔이 새 세대 재제안).
    // · user-constraint 구세대 proposed/adopted-but-unstamped=carry-forward(원천 재스캔 없음 — 사용자 판단 전
    //   소실 금지·candidateId별 새 세대에 정확히 1회 재발급·이미 등재된 문안은 declined 정리).
    // · 같은 세대 adopted 고아(draftable 공통)=proposed 복원 — 초안 덮어쓰기·소실로 proposal에 미결속된 채
    //   남은 adopted는 재채택 불가 고아(§3-2 — 진행 중 초안에 결속된 것·이미 등재된 것은 무접촉).
    const staleRecs = [];
    let boundIds = null; // null=proposal corrupt(소유 불명 — 복원 보류가 보수) / Set=결속 후보 목록(absent=빈 Set)
    try { const pr9 = readEnvelopeProposal(ws, repo); if (pr9.st === "ok") boundIds = new Set([pr9.candidateId, ...(pr9.candidateIds || [])].filter(Boolean)); else if (pr9.st === "absent") boundIds = new Set(); } catch { boundIds = null; }
    // [재검증 blocker] '정확히 1회'는 latest.has(새 세대)만으론 부족 — append-only 장부라 같은 후보가 H1·H2 등
    // 여러 구세대 키로 공존하면 한 스캔에서 각 키가 새 세대 행을 push해 중복 재발급+pending 허위 소진.
    // 이번 스캔에서 이미 새 세대 행을 만든 candidateId를 기록해 두 번째 구세대 키부터 건너뛴다.
    const issued9 = new Set();
    for (const [, rec] of latest) {
      if (!rec) continue;
      const kindR = rec.kind || (metaAll.get(rec.candidateId) || {}).kind || "";
      if (!ENVELOPE_DRAFTABLE_KINDS.includes(kindR)) continue;
      const meta = metaAll.get(rec.candidateId) || {};
      const title = typeof rec.title === "string" && rec.title ? rec.title : String(meta.title || "");
      if (String(rec.envelopeHash || "") !== approvedHash) { // ── 구세대 처리
        if (rec.status !== "proposed" && rec.status !== "adopted") continue;
        if (latest.has(rec.candidateId + "@" + approvedHash)) continue; // 새 세대 행 이미 존재=1회 규칙
        if (issued9.has(rec.candidateId)) continue; // 이번 스캔에서 이미 발급(다중 구세대 공존 반례)
        if (kindR === "resolved-blocker") {
          // [재편 A §5-3 2026-08-27] 본 스캔(자동 재제안) 폐지 — 구세대 proposed·미도장 adopted 모두
          // 한정 정리(재상신 경로는 rule-propose 뿐 · append-only라 기록 보존·복원 가능).
          staleRecs.push({ candidateId: rec.candidateId, envelopeHash: rec.envelopeHash || "", status: "declined", kind: "resolved-blocker", note: "공급 정책 개정(v2) — 자동 상신 폐지·필요 시 rule-propose로 재상신", findingId: rec.findingId || "", ts: new Date().toISOString() });
          continue;
        }
        ...
        if (kindR === "rule-manual" || kindR === "user-direct") {
          ...
          staleRecs.push({ candidateId: rec.candidateId, envelopeHash: approvedHash, status: "proposed", kind: kindR, origin: rec.origin || meta.origin || (kindR === "user-direct" ? "dashboard" : "manual"), policyVersion: 2, ...(rec.repoKey || meta.repoKey ? { repoKey: rec.repoKey || meta.repoKey } : {}), title, why: rec.why || meta.why || "", findingId: rec.findingId || meta.findingId || "", campaignId: rec.campaignId || meta.campaignId || "", ...(rec.askId || meta.askId ? { askId: rec.askId || meta.askId } : {}), ...(rec.oosId || meta.oosId ? { oosId: rec.oosId || meta.oosId } : {}), note: "승인 세대 이월(carry-forward — 사용자 판단 전 소실 금지)", ts: new Date().toISOString() });
          continue;
        }
        staleRecs.push({ candidateId: rec.candidateId, envelopeHash: approvedHash, status: "proposed", kind: "user-constraint", repoKey: rec.repoKey || meta.repoKey, title, why: rec.why || meta.why || "", provider: rec.provider || meta.provider || "", sessionId: rec.sessionId || meta.sessionId || "", turnAnchor: rec.turnAnchor || meta.turnAnchor || "", sourceHash: rec.sourceHash || meta.sourceHash || "", note: "승인 세대 이월(carry-forward — 사용자 판단 전 소실 금지)", ts: new Date().toISOString() });
        continue;
      }
      // [재편 A §5-3 2026-08-27] 같은 세대 legacy 자동 상신 잔여(resolved-blocker proposed·정책 버전 부재)
      // 한정 정리 — 같은 전이 잠금 안에서 proposal 결속·최신 상태를 재검사(경합 차단·설계 v4). proposal
      // 손상(boundIds null)=소유 판정 불가라 보수적으로 건너뛴다. user-constraint·rule-manual 무접촉.
      if (rec.status === "proposed" && kindR === "resolved-blocker" && !rec.policyVersion) {
        if (boundIds === null || boundIds.has(rec.candidateId)) continue; // 손상·진행 초안 결속=무접촉(보수)
        staleRecs.push({ candidateId: rec.candidateId, envelopeHash: approvedHash, status: "declined", kind: "resolved-blocker", note: "공급 정책 개정(v2) — 자동 상신 폐지·필요 시 rule-propose로 재상신", ...(rec.findingId ? { findingId: rec.findingId } : {}), ts: new Date().toISOString() });
        continue;
      }
      // [ab-1] 같은 세대 무표기(repoKey 없음) 대기=정책 정리 — 어느 저장소 제안인지 결정 불가한 행은 화면·빌더가
      // 이미 fail-closed로 막으므로 장부도 같은 결론으로 닫는다(사용자에게는 '다시 넣기'가 유일 경로).
      if (rec.status === "proposed" && ENVELOPE_DRAFTABLE_KINDS.includes(kindR) && !(rec.repoKey || meta.repoKey)) {
        if (boundIds === null || boundIds.has(rec.candidateId)) continue;
        staleRecs.push({ candidateId: rec.candidateId, envelopeHash: approvedHash, status: "declined", kind: kindR, note: "저장소 표식 없음(정책 v2.1) — 어느 프로젝트의 제안인지 판정 불가라 정리·필요하면 다시 넣기", ...(rec.findingId ? { findingId: rec.findingId } : {}), ts: new Date().toISOString() });
        continue;
      }
    ...
    // [재편 A §2-1 2026-08-27] 본 스캔(해소 blocker 전원 자동 상신) 폐지 — 헌법: 상신 자격은 성격(관통
    // 지침)이지 이력(고쳐졌음)이 아니다. 공급 경로는 ruleProposeCandidate(구현모델 마감 판단)뿐.
    // 생명주기 정리(위 stale·이월·스윕·고아 복원)는 유지 — out 형태(scanned/appended/suppressed/
    // legacyUnbound)는 호출자 호환으로 보존(신규 공급이 없으므로 항상 0).
  } catch { /* 스캔 실패=이번 트리거 무공급(멱등 — 다음 트리거 재시도) */ }
  finally { releaseEnvelopeTransLock(ws, lk.token); }
  return out;
}
// [재편 A §2-1 2026-08-27] 명시 상신 — 캠페인 마감 판단(관통 지침)만 후보로 올리는 유일한 공급 경로.
// 자격: 이번 캠페인에서 닫힌 blocker — resolved(close askId 결속) 또는 demoted 닫힘(원천 oosId 결속·
// 강등 과정상 askId 부재 허용). why 필수(왜 관통 지침인지 1줄) — user-constraint와 동형 방어
// (120자·단일 행·민감정보 형태). 중복은 코어(ab·oos)∪도장 서고 정규화 문안(원문·절단 실물 양쪽) 거부.
// pending 12칸은 경고로만(수동 판단 산출이라 차단하지 않음 — 설계 v4 §2-1).
function ruleProposeCandidate(ws, repo, opts) {
  const o = opts || {};
  const findingId = String(o.findingId || "").trim();
  const why = String(o.why || "").trim();
  const campaignId = String(o.campaignId || "").trim();
  const reject = (reason) => ({ ok: false, reason });
  if (!findingId) return reject("finding-missing");
  if (!campaignId) return reject("campaign-missing");
  // [검증 blocker: 세대 결속 fail-open] 승인 세대는 계약 슬롯이 유일 권위 — 호출자 지문은 "일치 확인"에만 쓴다
  // (부재=비활성·불일치=거부). 시험도 계약 슬롯을 먼저 세팅해야 한다(주입만으로 우회 불가).
  let gen = null; try { gen = loadContract(ws).envelopeHash || null; } catch { gen = null; }
  if (!gen) return reject("envelope-inactive");
  if (typeof o.approvedHash === "string" && o.approvedHash && o.approvedHash !== gen) return reject("gen-mismatch");
  // [확인검증 blocker ab-1] repo 결속 — 전달된 repo가 계약의 현재 정찰 대상과 다르면 거부(대상 변경 구간의 타 프로젝트 오귀속 차단).
  try { const cur = resolveScoutRepo(ws, loadContract(ws)).repo; if (normWs(String(cur || "")) !== normWs(String(repo || ""))) return reject("repo-mismatch"); } catch { return reject("repo-mismatch"); }
  if (!why) return reject("why-missing");
  ...
  if (String(f.campaignId || "") !== campaignId) return reject("other-campaign");
  const title = typeof f.title === "string" && f.title ? f.title : "";
  if (!title) return reject("legacy-unbound");
  let closeRec = null, demoteClose = null;
  for (const r of rows) if (r && r.type === "close" && r.findingId === findingId) {
    if (r.closeReason === "resolved") closeRec = r;
    else if (r.closeReason === "demoted") demoteClose = r;
  }
  // 강등 종결 두 형태(1차 검증 blocker①): ①생성형 — finding 자체가 demoted+oosId로 태어남(직접 강등 등재)
  // ②인용형 — 기존 open blocker가 후속 판정에서 close(closeReason demoted)로 닫힘. 인용형의 close 행에는
  // oos 결속이 기록되지 않으므로 oosId는 finding 실물에 있을 때만 결속한다(없음=결속 생략이 정직).
  const demotedByCreation = !!(f.demoted && f.oosId);
  if (closeRec) {
    if (f.tag !== "blocker") return reject("not-blocker");
    if (typeof closeRec.askId !== "string" || !closeRec.askId) return reject("legacy-unbound");
  } else if (demoteClose) {
    if (f.tag !== "blocker" && !f.demoted) return reject("not-blocker"); // 원 blocker의 강등만(설계 §2-1: '닫힌 blocker')
  } else if (!demotedByCreation) return reject("not-eligible");
  ...
  const candidateId = envelopeCandidateId("rule-manual", wsKeyFor(ws) + "|" + campaignId + "|" + findingId);
  const { latest } = readEnvelopeCandidates(ws);
  const cur = latest.get(candidateId + "@" + gen);
  if (cur && (cur.status === "proposed" || cur.status === "adopted")) return reject("duplicate");
  if (cur && (cur.status === "declined" || cur.status === "failed")) return reject("declined-suppressed"); // 거부권=같은 세대 억제(user-constraint와 동형)
  let pending = 0;
  for (const [, rec] of latest) if (rec && rec.status === "proposed" && String(rec.envelopeHash || "") === gen) pending++;
  const rec = {
    candidateId, envelopeHash: gen, status: "proposed", kind: "rule-manual", origin: "manual", policyVersion: 2, repoKey: repoKeyOf(repo),
    title, why, findingId, campaignId,
    ...(closeRec ? { askId: closeRec.askId } : {}),
    ...(!closeRec && f.oosId ? { oosId: f.oosId } : {}), // 인용형 강등은 oos 결속 실물이 없을 수 있음(위 주석)
    ts: new Date().toISOString(),
  };
  if (!appendEnvelopeCandidates(ws, [rec])) return reject("ledger-write");
  return { ok: true, candidateId, ...(pending >= MEMORY_CANDIDATE_PENDING_MAX ? { warn: "pending-cap" } : {}) };
}
// [넣기 입력칸 2026-08-28] 사용자가 대시보드에서 직접 친 수칙 → 후보(user-direct·proposed). 사용자 행위가 권위이므로
// 대화 원문 결속(user-constraint)이 필요 없다. 형식 방어는 user-constraint 문안 검사와 동형(12~200자·단일 행·규약 어휘·
// 민감 형태). 중복=코어 3축∪도장 서고 거부·같은 세대 duplicate/declined-suppressed. 효력은 승인(도장)부터.
function directRuleCandidate(ws, repo, opts) {
  const o = opts || {};
  const raw0 = String(o.text || "");
  const reject = (reason) => ({ ok: false, reason });
  if (/[\r\n\u2028\u2029]/.test(raw0)) return reject("multiline"); // 정규화(공백 접기) 전에 개행 검사 — 접은 뒤엔 개행이 사라져 검사가 무력화됨(시험 반례)
  const q = raw0.normalize("NFC").replace(/\s+/g, " ").trim(); // 유니코드 공백 전부 접음(normBacklogTitle과 동형 — NBSP 우회 반례)
  // [검증 blocker: 세대 결속 fail-open] 승인 세대는 계약 슬롯이 유일 권위 — 호출자 지문은 "일치 확인"에만 쓴다
  // (부재=비활성·불일치=거부). 시험도 계약 슬롯을 먼저 세팅해야 한다(주입만으로 우회 불가).
  let gen = null; try { gen = loadContract(ws).envelopeHash || null; } catch { gen = null; }
  if (!gen) return reject("envelope-inactive");
  ...
  // [확인검증 blocker ab-1] repo 결속 — 전달된 repo가 계약의 현재 정찰 대상과 다르면 거부(대상 변경 구간의 타 프로젝트 오귀속 차단).
  try { const cur = resolveScoutRepo(ws, loadContract(ws)).repo; if (normWs(String(cur || "")) !== normWs(String(repo || ""))) return reject("repo-mismatch"); } catch { return reject("repo-mismatch"); }
  if (q.length < CONSTRAINT_QUOTE_MIN) return reject("too-short");
  if (q.length > CONSTRAINT_QUOTE_MAX) return reject("too-long");
  const vocab = [FINDINGS_MARKERS.ko, FINDINGS_MARKERS.en, FINDINGS_MARKERS_V2.ko, FINDINGS_MARKERS_V2.en].flatMap((m) => [m.start, m.end]);
  if (vocab.some((v) => q.includes(v))) return reject("protocol-vocab");
  const qs = safeBacklogAutoTitle(q);
  ...
  for (const ax of ["supportedEnv", "alwaysBlocker", "outOfScope"]) for (const x of env.data[ax] || []) normSet.add(normBacklogTitle(x));
  try { const ah = (loadContract(ws) || {}).archiveHash; if (typeof ah === "string" && ah) { const ar = readVerifyEnvelopeArchive(repo); if (ar.st === "ok" && ar.sha1 === ah) for (const x of ar.data.alwaysBlocker) normSet.add(normBacklogTitle(x)); } } catch { /* 서고 판독 실패=코어만(보수) */ }
  if (normSet.has(normBacklogTitle(q))) return reject("already-in-envelope");
  const repoKey = repoKeyOf(repo); // [ab-1] 정찰 대상 파티션 — 같은 문안이라도 다른 저장소면 다른 후보
  const candidateId = envelopeCandidateId("user-direct", wsKeyFor(ws) + "|" + repoKey + "|" + sha1Of(q));
  const { latest } = readEnvelopeCandidates(ws);
  const cur = latest.get(candidateId + "@" + gen);
  if (cur && (cur.status === "proposed" || cur.status === "adopted")) return reject("duplicate");
  if (cur && (cur.status === "declined" || cur.status === "failed")) return reject("declined-suppressed");
  const rec = { candidateId, envelopeHash: gen, status: "proposed", kind: "user-direct", origin: "dashboard", policyVersion: 2, repoKey, title: q, why: "사용자가 직접 입력한 수칙", sourceHash: sha1Of(q), ts: new Date().toISOString() };
  if (!appendEnvelopeCandidates(ws, [rec])) return reject("ledger-write");
  return { ok: true, candidateId };
}
// [개정 작업대 2026-08-22] 개정판 초안: 추가(후보 N건 병합)+빼기(승인 항목 M건 제거)를 한 개정판으로 —
// 도장 1번. 변경은 개별·수량 자유(1건 단독도 가능), 서명은 결과 전문(권위 모델 불변). 전이 잠금 아래
// (draft·폐기·수동 propose·승인 전이와 직렬화). removes=[{axis, index}] — index는 '현행 파일' 기준·축별
// 내림차순 제거(선행 제거로 인덱스가 밀리는 오류 차단). 병렬 축(En/Ex)은 길이 일치 시 같은 index 제거·
// 같은 원문 복제(결정론 — 작문 금지).
function draftEnvelopeRevision(ws, repo, opts) {
  const o9 = opts || {};
  // [4a] 목적지=core(기본)|archive — 후보 파이프의 세대 축(approvedHash=core 승인 세대)은 목적지와 무관하게
  // 무변(설계 §1: 채택 시점에만 분기). archive 개정은 서고 파일을 대상으로 하되 후보 자격·복원은 core 축 그대로.
  const tgt9 = envelopeTargetDef(o9.target);
  const approvedHash = String(o9.approvedHash || "");
  ...
  if (!adds.length && !removes.length) return { ok: false, error: "선택된 변경이 없음(올림·빼기 0건)" };
  const lk9 = acquireEnvelopeTransLock(ws);
  if (!lk9.ok) return { ok: false, error: "수칙서 작업 잠금 경합 — 잠시 후 재시도" };
  try {
    // 코어 원문은 목적지와 무관하게 판독·세대 결속(후보 자격이 core 세대에 묶여 있음+union 중복 판정 재료).
    let coreRaw9, coreCur9;
    try { coreRaw9 = fs.readFileSync(path.join(repo, ENVELOPE_FILE), "utf8"); coreCur9 = JSON.parse(coreRaw9); } catch { return { ok: false, error: "현행 수칙서 판독 실패" }; }
    if (crypto.createHash("sha1").update(coreRaw9).digest("hex") !== approvedHash) return { ok: false, error: "수칙서가 승인 세대와 다름(재승인 후 재시도)" };
    // 서고 실물·도장 결속 — archiveHash 활성이면 파일이 도장 시점과 일치해야 어떤 목적지든 진행(소실≠미도입).
    const arcHash9 = (() => { try { const h = loadContract(ws).archiveHash; return typeof h === "string" && h ? h : null; } catch { return null; } })();
    const arcRead9 = readVerifyEnvelopeArchive(repo);
    if (arcHash9) {
      if (arcRead9.st !== "ok" || arcRead9.sha1 !== arcHash9) return { ok: false, error: "서고가 도장 시점과 다름(" + (arcRead9.st === "ok" ? "내용 불일치" : arcRead9.st) + ") — 대시보드에서 서고를 확인·재승인 후 재시도" };
    }
    let cur;
    if (tgt9.target === "archive") {
      if (!arcHash9 && arcRead9.st !== "absent") return { ok: false, error: "서고 파일이 도장 없이 존재(또는 손상) — 무단 파일을 덮어쓰지 않습니다. 파일을 정리한 뒤 재시도" };
    ...
    const removedTitles = [];
    for (const [axis, set9] of byAxis) {
      const arr0 = next[axis];
      if (!Array.isArray(arr0)) return { ok: false, error: "수칙서 축 형식 이상(" + axis + ")" };
      const idxs = [...set9].sort((a, b) => b - a);
      if (idxs[0] >= arr0.length) return { ok: false, error: "빼기 번호가 범위 밖(" + axis + " #" + (idxs[0] + 1) + " — 화면 새로고침 후 재시도)" };
      const mirror = ["En", "Ex"].filter((sfx) => Array.isArray(next[axis + sfx]) && next[axis + sfx].length === arr0.length);
      const arr = [...arr0];
      const mirrors = Object.fromEntries(mirror.map((sfx) => [sfx, [...next[axis + sfx]]]));
      for (const i of idxs) {
        removedTitles.push(axis + "#" + (i + 1));
        arr.splice(i, 1);
        for (const sfx of mirror) mirrors[sfx].splice(i, 1);
      }
      next[axis] = arr;
      for (const sfx of mirror) next[axis + sfx] = mirrors[sfx];
    }
    ...
    // 원문만 비교하면 절단 저장된 기존 항목과 어긋나 같은 절단 문안이 중복 등재된다(장문 후보=파서에 길이 상한 없음).
    const tgtSet9 = new Set(), othSet9 = new Set();
    if (tgt9.target === "archive") {
      for (const x of Array.isArray(next.alwaysBlocker) ? next.alwaysBlocker : []) tgtSet9.add(normBacklogTitle(String(x)));
      for (const ax of ENVELOPE_AXES) for (const x of (Array.isArray(coreCur9[ax]) ? coreCur9[ax] : [])) othSet9.add(normBacklogTitle(String(x)));
    } else {
      for (const ax of ENVELOPE_AXES) for (const x of (Array.isArray(next[ax]) ? next[ax] : [])) tgtSet9.add(normBacklogTitle(String(x)));
    ...
    for (const cid of adds) {
      ...
      if (!cand.repoKey) return bail9("저장소 표식이 없는 이전 제안이라 올릴 수 없어요 — 다시 넣어 주세요(" + cid + ")"); // [ab-1] 무표기=판정 불가=거부(fail-closed)
...
function draftEnvelopeCandidate(ws, repo, candidateId, approvedHash) {
  if (!approvedHash) return { ok: false, error: "승인된 수칙서 없음(Envelope 비활성) — draft 대상 아님" };
  // R3(f-abbe97fd): draft(proposal 쓰기+adopted 기록)도 같은 전이 잠금 아래 — 폐기·승인 전이와 직렬화.
  const lk9 = acquireEnvelopeTransLock(ws);
  if (!lk9.ok) return { ok: false, error: "수칙서 작업 잠금 경합 — 잠시 후 재시도" };
  try { return draftEnvelopeCandidateLocked(ws, repo, candidateId, approvedHash); } finally { releaseEnvelopeTransLock(ws, lk9.token); }
}
function draftEnvelopeCandidateLocked(ws, repo, candidateId, approvedHash) {
  const fail = (error) => { try { appendEnvelopeCandidates(ws, [{ candidateId, envelopeHash: approvedHash || "", status: "failed", note: String(error).slice(0, 200), ts: new Date().toISOString() }]); } catch { /* 기록 실패=반환 오류가 정본 */ } return { ok: false, error }; };
  const { latest } = readEnvelopeCandidates(ws);
  const cand = latest.get(candidateId + "@" + approvedHash);
  if (!cand) return { ok: false, error: "이 승인 세대에 해당 후보 없음(조정 스캔 후 재시도)" };
  if (cand.status !== "proposed") return { ok: false, error: "후보 상태가 proposed 아님(현재: " + cand.status + ")" };
  if (typeof cand.title !== "string" || !cand.title) return fail("후보에 문안(title) 없음");
  let raw, o;
  try { raw = fs.readFileSync(path.join(repo, ENVELOPE_FILE), "utf8"); o = JSON.parse(raw); } catch { return fail("현행 수칙서 판독 실패"); }
  if (crypto.createHash("sha1").update(raw).digest("hex") !== approvedHash) return fail("수칙서가 승인 세대와 다름(재승인 후 재시도)"); // baseHash 결속(설계 A-4)
  if (!Array.isArray(o.alwaysBlocker)) return fail("수칙서 ab축 형식 이상");
  if (o.alwaysBlocker.length >= ENVELOPE_ITEM_MAX) return fail("ab축 12항 상한 — 어떤 항목을 뺄지는 사용자 결정(자동 삭제 금지)");
  const tn = normBacklogTitle(cand.title);
  if (o.alwaysBlocker.some((x) => normBacklogTitle(x) === tn)) return fail("동일 정규화 문안이 이미 ab축에 존재");
  // [기억 권위 A-2·구현검증 1차 blocker③] 200자 초과=절단 '표식과 함께' 보존 — 사용자가 문장이 잘린 사실을
  // 모른 채 의미 끊긴 제약을 승인하는 경로 차단. 표식 포함 총장은 상한 준수(표식이 상한을 뚫지 않음).
  const MARK9 = "…[절단]";
  const t9 = String(cand.title);
  const item = t9.length > ENVELOPE_CHAR_MAX ? t9.slice(0, ENVELOPE_CHAR_MAX - MARK9.length) + MARK9 : t9;
  const next = { ...o, alwaysBlocker: [...o.alwaysBlocker, item] };
  let dup = false; // 병렬 축 결정론 복제 — 길이 일치 정본 검사(writeEnvelopeProposal) 충족
  for (const sfx of ["En", "Ex"]) {
    const k = "alwaysBlocker" + sfx;
    if (Array.isArray(o[k]) && o[k].length === o.alwaysBlocker.length) { next[k] = [...o[k], item]; dup = true; }
  }
  const proposalText = JSON.stringify(next, null, 1);
  const note = "후보 병합 " + candidateId + (dup ? " · 병렬 축 복제됨 — 사용자 편집 필요(번역·예시)" : "");
  const w = writeEnvelopeProposal(ws, repo, proposalText, note, { candidateId }); // 폐기 복원 결속(2026-08-21)
  if (!w.ok) return fail("제안본 저장 거부: " + w.error);
  try { appendEnvelopeCandidates(ws, [{ candidateId, envelopeHash: approvedHash, status: "adopted", note: "draft " + w.newHash, ts: new Date().toISOString() }]); } catch { /* 상태 기록 실패해도 제안본은 유효 */ }
  return { ok: true, newHash: w.newHash, parallelCopied: dup };
}
...
// ── 지적 처분 관문(2026-08-01 실사고): '판단 없는 자동 수용'으로 왕복이 새는 경로 차단 ────────────
// 배경: 상한 5회 캠페인에서 다섯 지적 전부를 판단 없이 수용·수정하며 회차를 소진(blocker 4→2→2→2 정체).
// 네 갈래 재판단은 상한 소진 뒤 마감문에서 1회만 요구돼 중간 회차에는 판단을 강제하는 지점이 없었다.
// 계약: 같은 캠페인의 열린 지적마다 처분 1건이 장부에 있어야 다음 ask 허용(예약 전 거부=왕복 미소모).
// fix-fact=사실 오류 인정(실측으로 반박됨) / fix-gap=보강 요구 수용(틀린 건 아님) / rebut=반박 종결(근거
// 필수) / park=맞지만 이 묶음 밖(보관함 자동 등록). fix-gap 누적(FIX_GAP_NOTICE_AT)은 '틀려서 고치는 게
// 아니라 더-자세히 요구가 반복되는 상태' — 종이 단계 수확 체감 신호로 집계·고지한다(차단 아님).
// findingId가 캠페인·세대를 이미 지문에 포함하므로 처분 대조는 (campaignId, findingId)로 충분하다.
const FINDING_DISPOSITIONS = ["fix-fact", "fix-gap", "rebut", "park", "escalate"]; // escalate=사용자 방향 질문 — 결정 장부 항목 id 필수(스크립트 출력 강제·장치화 2026-08-30)
const FIX_GAP_NOTICE_AT = 3;
// 1차 검증 blocker① 반영: 처분은 '그 시점까지의 등장'에만 유효하다(asOfRound 결속). 같은 지적이 후속
// 라운드에 재등장(occurrence)하면 과거 처분은 낡은 것 — 다시 판단해야 한다. 결속 없는 처분(legacy·수동
// append)은 유효 처분으로 인정하지 않는다(무근거 1회 기록으로 영구 통과하는 우회 차단).
// 확인 검증 blocker①(계보) 반영: occurrence는 루트 id에 기록되고 prevId에 인용된 자식 id가 실린다
// (machineFindingsLayer rootOf 정규화). 자식이 열린 채 재등장하면 활동이 루트에만 쌓여 자식 처분이 영구
// 유효해지는 우회 — prevId 일치도 그 자식의 활동으로 센다(실행 반례: child open·occurrence{root, prevId:child}).
// 3회차 확인 blocker(변형 ③) 반영: 재지적이 id(자식)와 다른 prevId를 함께 실으면 occurrence가
// {findingId: rootOf(cited), prevId: f.prevId}가 되어 자식이 두 필드 모두에서 사라진다(실행 반례:
// childActivity=2·undisposed=[]). 생산자가 인용 대상을 subjectId로 별도 보존하고 여기서 그 축도 센다.
function findingActivityRound(rows, campaignId, findingId) {
  let r = 0;
  for (const x of rows) {
    if (!x || x.campaignId !== campaignId) continue;
    const hits = ((x.type === "finding" || x.type === "occurrence") && x.findingId === findingId)
      || (x.type === "occurrence" && (x.prevId === findingId || x.subjectId === findingId
        || (Array.isArray(x.subjectIds) && x.subjectIds.indexOf(findingId) >= 0))); // 4회차: 같은 라운드 다자식 전원 보존
    if (hits) r = Math.max(r, Number(x.round) || 0);
  }
  return r;
}
// 유효 처분 판정의 단일 출처(확인 검증 [보완]①② 동반 수정): 관문·현황 표시·fix-gap 집계가 같은 계산을
// 쓰지 않으면 '관문은 막는데 화면은 전부 판단됨'인 모순이 생긴다(실행 대조로 확인된 상태).
...
function readFindingsLedgerState(ws) {
  try {
    const rows = String(fs.readFileSync(findingsLedgerFileFor(ws), "utf8")).split(/\r?\n/).filter(Boolean).map((l) => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
...
// ── [Envelope Selector v7 §3] 선별 순수 계약 — 페이지·strict 출력·합집합 상한·예산 산식·영수증 서랍 ──
// 실행(spawn)은 selector-runner.js가, 배선(worker)은 3b가 담당 — 여기는 결정론 계산·판독만.
const SELECTOR_PAGE_ITEMS = 16;              // 페이지=항목 수 상수(완전성: 전 항목이 정확히 한 페이지에)
const SELECTOR_PAGE_TIMEOUT_MS = 8 * 60 * 1000; // 페이지당 timeout(정찰 팔 상한과 동형)
const SELECTOR_PARALLEL = 3;                 // 병렬 P(v7 §3 — 3b worker가 사용)
const SELECTOR_DEADLINE_MARGIN_MS = 60 * 1000;
const SELECTOR_UNION_MAX = 12;               // 합집합 항목 상한 K(코어와 동형 감각)
const SELECTOR_UNION_BYTES_MAX = 4000;       // 선별 전문 합계 바이트 상한
const SELECTOR_DIFF_MAX = 20000;             // diff 유계 절단(프롬프트·지문이 같은 절단분을 봄 — 3b)
function selectorDeadlineMsFor(pages) { return Math.ceil(Math.max(0, pages) / SELECTOR_PARALLEL) * SELECTOR_PAGE_TIMEOUT_MS + SELECTOR_DEADLINE_MARGIN_MS; }
// 서고 ab 항목 → 페이지 분할. id=arc-N(파일 순서·1기점) — 세대(archiveHash)에 결속된 위치 결정론.
function buildSelectorPages(abItems) {
  const items = (Array.isArray(abItems) ? abItems : []).map((x, i) => ({ id: "arc-" + (i + 1), text: String(x) }));
  const pages = [];
  for (let i = 0; i < items.length; i += SELECTOR_PAGE_ITEMS) pages.push(items.slice(i, i + SELECTOR_PAGE_ITEMS));
  return pages;
}
// 페이지 프롬프트 — 범주 지시(문구 패턴 목록 아님)·중립 재료만(사용자 원문 스냅샷+변경 파일+diff — 검증
// 요청문(구현자 작문)은 계약상 입력 금지: 호출자가 애초에 전달할 수 없게 인자에 없음).
function buildSelectorPagePrompt({ snapshotText, changedFiles, diffText, pageItems }) {
  const L = [];
  L.push("[서고 수칙 선별] 아래 '승인 수칙 후보 페이지'에서 '이번 작업'과 실질적으로 관련된 항목의 id만 고르라.");
  L.push("판단 기준: 이번 작업이 그 수칙을 위반할 수 있거나, 그 수칙이 이번 작업의 판정에 필요하면 관련이다. 확신이 없으면 포함하라(누락이 과선별보다 위험).");
  L.push("출력은 JSON 한 줄뿐: {\"relevant\":[\"arc-3\"]} — 코드 펜스(```)·다른 텍스트·설명·마크다운 금지. 관련 없음={\"relevant\":[]}.");
  if (typeof snapshotText === "string" && snapshotText) { L.push("", "[이번 작업 — 사용자 원문]", snapshotText.slice(0, 4000)); }
  if (Array.isArray(changedFiles) && changedFiles.length) { L.push("", "[변경 파일]", changedFiles.slice(0, 60).join("\n")); }
  if (typeof diffText === "string" && diffText) { L.push("", "[변경 내용(유계 절단)]", diffText.slice(0, SELECTOR_DIFF_MAX)); }
  L.push("", "[승인 수칙 후보 페이지]");
  for (const it of Array.isArray(pageItems) ? pageItems : []) L.push(it.id + ": " + it.text);
  return L.join("\n");
}
// 페이지 출력 strict 판독(§3): 전체가 JSON 하나·{"relevant": [유효 id...]}만 — 미지 필드·비배열·범위 밖
// id·중복·파싱 실패=실패(부분 수용 없음). 산문 섞임=실패(프롬프트가 JSON 한 줄을 지시 — fail-closed).
function parseSelectorPageOutput(raw, validIds) {
  const out = { ok: false, ids: [], reason: "" };
  let line = String(raw || "").trim();
  // [4b-2 실전 스모크 실측] 실제 모델이 '펜스 금지' 지시에도 코드 펜스로 감싸는 사례(```json\n{...}\n```) —
  // '전체가 펜스 하나로 감싼 정확히 한 줄'인 형태만 결정론 언랩. 그 외(산문 혼입·다중행·펜스 안 여러 줄)는
  // 여전히 거부 — strict 계약(부분 수용 없음) 무변, 포장 정규화일 뿐.
  const fm9 = /^```(?:json)?[ \t]*\r?\n([^\r\n]+)\r?\n```$/.exec(line); // [ \t]만 — \s는 개행을 삼켜 '펜스 안 선행 빈 줄' 다중행이 통과(4b-2 1차 blocker②)
  if (fm9) line = fm9[1].trim();
  if (/[\r\n]/.test(line)) { out.reason = "multiline"; return out; } // 'JSON 한 줄' 계약(3a 재검증 blocker — 다중행 수용 차단)
  // 중복 키는 JSON.parse가 후승 값으로 삼킨다 — 이스케이프 키까지 해독하는 기존 스캐너 재사용(제약 블록 계보).
  const keys9 = jsonTopLevelKeys(line);
  if (!keys9) { out.reason = "bad-json"; return out; } // 산문 섞임·배열 루트 등 구조 이상
  if (new Set(keys9).size !== keys9.length) { out.reason = "dup-key"; return out; } // 이스케이프 키 해독 후 중복
  if (keys9.length !== 1 || keys9[0] !== "relevant") { out.reason = "unknown-field"; return out; }
  let o;
  try { o = JSON.parse(line); } catch { out.reason = "bad-json"; return out; }
  if (!o || typeof o !== "object" || Array.isArray(o)) { out.reason = "not-object"; return out; }
  if (!Array.isArray(o.relevant)) { out.reason = "unknown-field"; return out; }
  const valid = new Set(Array.isArray(validIds) ? validIds : []);
  const seen = new Set();
  for (const id of o.relevant) {
    if (typeof id !== "string" || !valid.has(id)) { out.reason = "invalid-id"; return out; }
    if (seen.has(id)) { out.reason = "dup-id"; return out; }
    seen.add(id);
  }
...
const SELECTOR_USAGE_DIR = path.join(STATS_DIR, "selector-usage");
let selectorUsageSeq = 0;
function appendSelectorUsage(rec) {
  try {
    fs.mkdirSync(SELECTOR_USAGE_DIR, { recursive: true });
    for (let k = 0; k < 5; k++) {
      const name = String(Date.now()).padStart(14, "0") + "-" + String(process.pid).padStart(10, "0") + "-" + (selectorUsageSeq++).toString(36).padStart(6, "0") + "-" + Math.random().toString(36).slice(2, 6) + ".json";
      try { fs.writeFileSync(path.join(SELECTOR_USAGE_DIR, name), JSON.stringify(rec), { flag: "wx" }); return name; }
      catch (e) { if (!(e && e.code === "EEXIST")) return null; }
    }
    return null;
  } catch { return null; }
}
function readSelectorUsage() {
  let names;
  try { names = fs.readdirSync(SELECTOR_USAGE_DIR).filter((x) => x.endsWith(".json")).sort(); } catch { return []; }
  const out = [];
  for (const nm of names) { try { const o = JSON.parse(fs.readFileSync(path.join(SELECTOR_USAGE_DIR, nm), "utf8")); if (o && typeof o === "object") out.push({ ...o, _file: nm }); } catch { /* 손상=무시 */ } }
  return out;
}
// [3b] 변경물 꾸러미 재료 — 선별자 입력(§3 ②)과 archiveCtx 지문의 단일 출처. 선별 시작(worker)과 주입
// 직전(ask child)이 '같은 함수'로 재계산해 hash를 대조한다(드리프트=중단). 민감 경로는 목록·diff 모두
// 제외(src/scope-package.ts SENSITIVE_PATH_RE의 복사 — map-retrieval.js와 같은 관례·갈리면 잠금 테스트가 깨짐).
const SELECTOR_SENSITIVE_RE = /(^|\/)\.(env[^/]*|netrc|npmrc|pgpass|htpasswd)$|(^|[/._-])(secrets?|credentials?|tokens?|api[_-]?keys?|passwords?|passwd)([/._-]|$)|\.(pem|key|p12|pfx|jks|keystore|der|p8|ppk)$|(^|\/)id_(rsa|dsa|ecdsa|ed25519)|(^|\/)(node_modules|dist|build|vendor)\//i;
// 반환: {st:"ok", nonGit, files, diffText, hash, head, worktreeFp} | {st:"error", reason}.
// [3b 1차 blocker 봉합] '비-git 저장소(정상 축퇴 — 변경 추적 원천 자체가 없음)'와 'git 판독 실패(비정상 —
// timeout·권한·스폰 오류)'를 구분한다. 실패를 빈 재료로 축퇴시키면 '코드에만 드러나는 관련 수칙'이 선별에서
// 빠진 채 같은 빈 지문으로 주입 관문까지 통과한다(fail-open) — 실패=st:"error"로 호출자(worker·주입 재검사)가
// 중단한다. changedEntriesFor(공용·실패=[] 계약)는 다른 소비자용이라 여기서 쓰지 않고 strict 수집을 따로 한다.
function selectorScopeMaterial(repo) {
  const sh1 = (s) => crypto.createHash("sha1").update(String(s)).digest("hex");
  const norm9 = (p) => String(p || "").replace(/\\/g, "/");
  const run9 = (args) => { // {ok, out} | {ok:false, err} — 실패의 침묵 없음
    try {
      const r = require("child_process").spawnSync("git", ["-c", "safe.directory=" + String(repo).replace(/\\/g, "/"), "-C", repo, ...args], { encoding: "utf8", timeout: 5000, windowsHide: true, maxBuffer: 32 * 1024 * 1024 });
      if (r.error) return { ok: false, err: String((r.error && r.error.code) || r.error) };
      if (r.status !== 0) return { ok: false, err: "exit=" + r.status };
      return { ok: true, out: String(r.stdout || "") };
    } catch (e) { return { ok: false, err: String((e && e.message) || e) }; }
  };
  // 저장소 분류 — ①rev-parse가 성공적으로 true=git(strict) ②깨끗한 비0 종료+.git 부재=비-git(정상 축퇴)
  // ③그 외 전부(스폰 오류·timeout·.git이 있는데 git이 못 읽음)=판독 실패(fail-closed).
  const probe = run9(["rev-parse", "--is-inside-work-tree"]);
  const isGit = probe.ok && probe.out.trim() === "true";
  if (!isGit) {
    let dotGit = false; try { dotGit = fs.existsSync(path.join(repo, ".git")); } catch { dotGit = true; /* 검사 불가=보수(실패 취급) */ }
    if (probe.ok || dotGit || !/^exit=\d+$/.test(String(probe.err || ""))) return { st: "error", reason: "git-probe: " + (probe.ok ? "unexpected " + probe.out.trim() : String(probe.err)) };
    return { st: "ok", nonGit: true, files: [], diffText: "", hash: sh1("\u0000"), head: "no-git", worktreeFp: sh1("") };
  }
  // strict 수집 — git 저장소인데 어느 호출이든 실패=전체 실패(빈 재료 위장 금지)
  const stz = run9(["status", "--porcelain", "-z"]);
  if (!stz.ok) return { st: "error", reason: "git-status: " + stz.err };
  const files = [];
  { // changedEntriesFor와 같은 -z 파싱 규칙(rename 2필드 소비) — 단 실패는 위에서 이미 중단됨
    const toks = stz.out.split("\0");
    for (let i = 0; i < toks.length && files.length < 200; i++) {
      const tk = toks[i];
      if (!tk || tk.length < 4) continue;
      const rel = tk.slice(3);
      if (rel && !SELECTOR_SENSITIVE_RE.test(norm9(rel))) files.push(rel);
      if (/[RC]/.test(tk.slice(0, 2))) i++;
    }
  }
  const d1 = run9(["diff"]);
  if (!d1.ok) return { st: "error", reason: "git-diff: " + d1.err };
  const d2 = run9(["diff", "--cached"]);
  if (!d2.ok) return { st: "error", reason: "git-diff-cached: " + d2.err };
  const rawDiff = d1.out + "\n" + d2.out;
  // diff를 파일 구획 단위로 갈라 민감 경로 구획만 제거(부분 절단 없음 — 구획 통째 제외).
  const kept = [];
  for (const sec of rawDiff.split(/^(?=diff --git )/m)) {
    const m = /^diff --git a\/(.*?) b\/(.*)$/m.exec(sec);
    if (m && (SELECTOR_SENSITIVE_RE.test(norm9(m[1])) || SELECTOR_SENSITIVE_RE.test(norm9(m[2])))) continue;
    kept.push(sec);
  }
  const diffText = kept.join("").slice(0, SELECTOR_DIFF_MAX); // 지문은 '전송되는 절단분' 그대로에 결속
  // HEAD: 성공=sha. 실패는 '커밋 0개(미탄생 HEAD — 정상 no-head)'와 그 외(비정상=중단)를 실측으로 구분한다
  // (확인검증 blocker f-c4f1dc00 — timeout·스폰 오류·커밋이 있는데 HEAD 판독 불가를 no-head로 축퇴 금지).
  // 판별=rev-list --all: 유효 저장소면 항상 성공하고 커밋 0개=빈 출력. orphan HEAD(커밋 존재+HEAD 미탄생)는
  // 합법이지만 희귀 상태라 보수적으로 중단(fail-closed 우선 — reason이 사유를 밝힘).
  const hd = run9(["rev-parse", "HEAD"]);
  let head;
  if (hd.ok) head = hd.out.trim();
  else {
    if (!/^exit=\d+$/.test(String(hd.err || ""))) return { st: "error", reason: "git-head: " + hd.err };
    const rl = run9(["rev-list", "-n", "1", "--all"]);
    if (!rl.ok) return { st: "error", reason: "git-revlist: " + rl.err };
    if (rl.out.trim()) return { st: "error", reason: "git-head: unreadable-with-commits" };
    head = "no-head"; // 커밋 0개 확인됨 — 미탄생 HEAD 정상
  }
  return { st: "ok", nonGit: false, files, diffText, hash: sh1(files.join("\n") + "\u0000" + diffText), head, worktreeFp: sh1(stz.out) };
}

// [4c UX] 개정 초안 차이 요약 — 승인 화면이 '전문 벽'이 아니라 "그대로 유지 N·새로 올림 M·빼기 K"를 먼저
// 보여주기 위한 순수 계산(사용자 실보고 2026-08-25: '교체'라는 말에 기존 수칙 삭제로 오해). 대상 파일
// 부재=최초 생성(전부 추가). 판독 실패=null(호출자가 전문만 표시 — 요약 실패가 승인을 막지 않음).
function envelopeDraftDiff(repo, pr) {
  try {
    if (!pr || pr.st !== "ok") return null;
    const tgt = envelopeTargetDef(pr.target);
    // [1차 blocker①] 부재(ENOENT)=최초 생성만 — 권한 오류·JSON 손상 등 그 외 판독 실패는 null(전문-only 축퇴).
    // 판독 실패를 '첫 등재·전부 추가'로 위장하면 기존 데이터가 있는데 없다고 안심시키는 거짓 요약이 된다.
    let curInner = null, firstTime = false;
    try { curInner = JSON.parse(fs.readFileSync(path.join(repo, tgt.file), "utf8")); }
    catch (e) { if (e && e.code === "ENOENT") firstTime = true; else return null; }
    // [2차 확인검증 blocker 잔여] 문법은 JSON인데 정본 계약(스키마·축 형식)이 깨진 현행/초안=null —
    // strict 판독기(validInner)와 같은 눈으로 판정해 의미 손상 데이터를 정상처럼 요약하지 않는다.
    if (curInner !== null && !tgt.validInner(curInner)) return null;
    let nextInner = null;
    try { nextInner = JSON.parse(pr.proposalText); } catch { return null; }
    if (!tgt.validInner(nextInner)) return null;
    const axes = tgt.target === "archive" ? ["alwaysBlocker"] : ENVELOPE_AXES;
    const out = { target: tgt.target, label: tgt.label, firstTime, kept: 0, added: [], removed: [] };
    // [1차 blocker②] 다중집합(개수) 대조 — 같은 문안이 2회 있는데 1회로 줄어드는 초안(인덱스 빼기)을 집합
    // 비교는 '빼기 0'으로 오표시한다(거짓 안심). 문안별 개수 차이만큼 추가/빼기로 계상.
    const count9 = (arr) => { const m = new Map(); for (const x of arr) m.set(x, (m.get(x) || 0) + 1); return m; };
    for (const ax of axes) {
      const cur = Array.isArray(curInner && curInner[ax]) ? curInner[ax].map(String) : [];
      const nxt = Array.isArray(nextInner[ax]) ? nextInner[ax].map(String) : [];
      const mc = count9(cur), mn = count9(nxt);
      for (const [x, n] of mn) { const c = mc.get(x) || 0; for (let i = 0; i < n - c; i++) out.added.push({ axis: ax, text: x }); out.kept += Math.min(n, c); }
      for (const [x, c] of mc) { const n = mn.get(x) || 0; for (let i = 0; i < c - n; i++) out.removed.push({ axis: ax, text: x }); }
    }
    return out;
  } catch { return null; }
}

// [4b-2 §4-③] preview 관문 판정 — Claude(preview-gate.js)·Codex(codex-hook.js)가 '같은 함수'로 판정
// (설계: Claude와 동일 게이트 계약 공유 — 런타임별 분기 없음이 기본). 순수 판정(파일 판독은 영수증 폴백뿐).
// 반환: "allow-inactive"(미도입)|"allow-nosnapshot"(스냅샷 부재 — 뒤 관문 ask-start가 fail-closed)|
// "allow-readonly"(변경 가능 도구 아님)|"allow-preview-cmd"(예외=미리보기 실행 명령 전체 정확 일치)|
// "allow-receipt"(4중 결속 영수증 존재)|"block".
function previewGateDecision(inp) {
  const o = inp || {};
  if (!(typeof o.archiveHash === "string" && o.archiveHash)) return "allow-inactive";
  if (!o.turnAnchor || !o.snapshotHash) return "allow-nosnapshot";
  const name = String(o.toolName || "");
  if (!(/^(Bash|apply_patch|Edit|Write|MultiEdit|NotebookEdit)$/i.test(name) || /^mcp__/i.test(name))) return "allow-readonly";
  if (/^Bash$/i.test(name)) {
    // 예외=명령 '전체'가 <실행기> <스크립트> selector-preview 단독 호출이고 두 토큰의 basename이 정확 일치할
    // 때만(연결·리다이렉트·후행 인자·유사 이름 실행 전부 불허 — 4b-1 확인검증 2왕복 봉합분과 동일 규칙).
    const mx = /^\s*("[^"]+"|\S+)\s+("[^"]+"|\S+)\s+selector-preview\s*$/.exec(String(o.command || ""));
    if (mx) {
      const tok = (s) => String(s).replace(/^"|"$/g, "");
      const base = (p) => tok(p).replace(/[\\/]+$/, "").split(/[\\/]/).pop().toLowerCase();
      if (["node", "node.exe"].includes(base(mx[1])) && base(mx[2]) === "codex-bridge.js") return "allow-preview-cmd";
    }
  }
  let pass = false;
  try { pass = (Array.isArray(o.receipts) ? o.receipts : readSelectorUsage()).some((r) => r && r.purpose === "preview" && r.wsKey === o.wsKey && r.turnAnchor === o.turnAnchor && r.snapshotHash === o.snapshotHash && r.archiveHash === o.archiveHash); } catch { pass = false; } // 서랍 판독 실패=영수증 없음(차단 — fail-closed)
  return pass ? "allow-receipt" : "block";
}

// [4b 이중 배달 §4] 구현자 프롬프트 주입 — 훅(contract-inject·codex-hook)이 호출. 훅 안 LLM 호출 금지
// 계약 준수: 파일 판독·문자열 조립뿐. ①코어 ab 전문(수칙서 활성 시 상시·≤12항 — 구현하는 쪽도 판정 경계를
// 알고 시작) ②서고 활성 시: 이 턴 스냅샷+현행 서고에 결속된 자격 영수증(완료 preview 또는 proof 실존 ask의
...
function implementerEnvelopeInjectParts(ws, c, lang, snapshotHash, turnAnchor) {
  try {
    ...
    const ev = readVerifyEnvelope(target);
    if (ev.st !== "ok" || ev.sha1 !== c.envelopeHash) return null; // 미승인 변경=주입 생략(검증 측과 동일 태도)
    const L = [];
    const items = en && ev.dataEn.alwaysBlocker ? ev.dataEn.alwaysBlocker : ev.data.alwaysBlocker;
    ...
    if (arcActive && snapshotHash) { // turnAnchor 부재=아래 필터가 자격 0으로 수렴(안내 줄은 유지)
      ...
      if (arc.st === "ok" && arc.sha1 === c.archiveHash) {
        // [1·2차 blocker①] 캐시 자격=4중 결속(wsKey·turnAnchor·snapshotHash·archiveHash) — 설계 §4-②
        // '현재 턴 결속 캐시'. 같은 프로젝트라도 과거 턴(같은 문구) 영수증은 배제(타 작업=지문 불일치 자연 배제).
        let recs = [];
        try { const wk = wsKeyFor(ws); recs = turnAnchor ? readSelectorUsage().filter((r) => r && r.wsKey === wk && r.turnAnchor === turnAnchor && r.archiveHash === c.archiveHash && r.snapshotHash === snapshotHash) : []; } catch { recs = []; }
        const proofJobs = new Set();
        try { for (const f of fs.readdirSync(PROOFS_DIR)) { try { const p = JSON.parse(fs.readFileSync(path.join(PROOFS_DIR, f), "utf8")); if (p && typeof p.jobId === "string") proofJobs.add(p.jobId); } catch { /* skip */ } } } catch { /* 없음 */ }
        const q = recs.filter((r) => r.purpose === "preview" || (typeof r.askId === "string" && r.askId && proofJobs.has(r.askId)));
        const last = q.length ? q[q.length - 1] : null;
        if (last && Array.isArray(last.selectedIds)) {
          L.push("", en ? "[Archive rules selected for THIS task (reference — not a verdict authority · bound to this turn)]" : "[이번 작업 선별 수칙(참고 — 판정 권위 없음 · 이 턴 결속)]");
          let okIds = true;
          const tx = [];
          for (const idd of last.selectedIds) { const m = /^arc-(\d+)$/.exec(String(idd)); const t = m ? arc.data.alwaysBlocker[Number(m[1]) - 1] : undefined; if (typeof t !== "string") { okIds = false; break; } tx.push({ id: idd, t }); }
          if (okIds) { if (tx.length) tx.forEach((x) => L.push("> " + x.id + ": " + x.t)); else L.push(en ? "(none selected for this task)" : "(이번 작업 관련 선별 0건)"); }
          else L.push(en ? "(selection cache invalid — run selector-preview again)" : "(선별 사본 판독 불일치 — selector-preview를 다시 실행하세요)");
        } else {
          L.push("", en ? `[Archive active] Before changing anything this turn, run: node "${BRIDGE}" selector-preview — write tools stay gated until its receipt exists.` : `[서고 활성] 이번 턴에 변경 도구를 쓰기 전에 실행: node "${BRIDGE}" selector-preview — 이번 작업 선별 수칙을 받아야 변경 도구 관문이 열립니다.`);
        }
      } else {
...
// ── [잔여 재검증 강제 2026-08-29] 상한 마감문의 "즉시 재검증" 판단을 실행에 결속하는 마커(양 Stop 훅 공용 정본) ──
// 캠페인=사용자 턴이라 같은 턴에서는 새 검증을 못 여니, 판단을 마커로 남기고 다음 턴부터 "그 캠페인 id를 요청문에 담은 통과 검증"이
// 결속될 때까지 종료를 막는다. 목록형(residual-now-v2 items[]) — 뒤 캠페인의 마감이 앞 캠페인의 미검증분을 덮어쓰지 못한다
// (1회차 blocker①). 소비 조건=같은 ws·succeeded·prompt에 campaignId 문자열·finishedAt(전용)>=이 턴 발화·판정 pass/pass-notes.
function residualFileFor(ws) { return path.join(VERIFY_FINDINGS_DIR, wsKeyFor(ws) + ".residual-now.json"); }
function readResidual(ws) {
  let o = null; try { o = JSON.parse(fs.readFileSync(residualFileFor(ws), "utf8")); } catch { return null; }
  if (!o || typeof o !== "object") return null;
  const okItem = (x) => x && typeof x.campaignId === "string" && x.campaignId;
  if (o.schema === "residual-now-v2" && Array.isArray(o.items)) { const items = o.items.filter(okItem); return items.length ? { items } : null; }
  if (o.schema === "residual-now-v1" && okItem(o)) return { items: [{ campaignId: o.campaignId, sourceAsk: o.sourceAsk || "", evidence: Array.isArray(o.evidence) ? o.evidence : [], ts: o.ts || "" }] }; // legacy 단건
  return null;
}
function addResidual(ws, rec) {
  if (!rec || typeof rec.campaignId !== "string" || !rec.campaignId) return false;
  const cur = readResidual(ws); const items = cur ? cur.items.slice() : [];
  if (!items.some((x) => x.campaignId === rec.campaignId)) items.push({ campaignId: rec.campaignId, sourceAsk: String(rec.sourceAsk || ""), evidence: Array.isArray(rec.evidence) ? rec.evidence.map(String) : [], ts: String(rec.ts || new Date().toISOString()) });
  try { fs.mkdirSync(VERIFY_FINDINGS_DIR, { recursive: true }); } catch { /* atomicWrite가 판정 */ }
  return atomicWrite(residualFileFor(ws), JSON.stringify({ schema: "residual-now-v2", ws: String(ws), items }, null, 1));
}
function removeResidualItems(ws, campaignIds) {
  const cur = readResidual(ws); if (!cur) return true;
  const drop = new Set((campaignIds || []).map(String));
  const items = cur.items.filter((x) => !drop.has(x.campaignId));
  if (!items.length) { try { fs.unlinkSync(residualFileFor(ws)); } catch (e) { if (!e || e.code !== "ENOENT") return false; } return true; }
  return atomicWrite(residualFileFor(ws), JSON.stringify({ schema: "residual-now-v2", ws: String(ws), items }, null, 1));
}
// 이 턴(turnTs ms 이후 finishedAt)에 마커 항목을 소비하는 통과 job — 식별자(campaignId 문자열)·판정·시각·ws 4중 결속. 없으면 null.
function residualJobFor(ws, item, turnTs) {
  const dir = path.join(BRIDGE_DIR, "ask-jobs");
  let names = []; try { names = fs.readdirSync(dir); } catch { return null; }
  const wsN = normWs(String(ws));
  for (const name of names) {
    const fm = /^(ask-[0-9a-z-]+)\.json$/i.exec(name); if (!fm) continue;
    let j = null; try { j = JSON.parse(fs.readFileSync(path.join(dir, name), "utf8")); } catch { continue; }
    if (!j || j.state !== "succeeded" || j.id !== fm[1] || normWs(String(j.workspace || "")) !== wsN) continue;
    if (typeof j.prompt !== "string" || !j.prompt.includes(item.campaignId)) continue;
    const fin = Date.parse(typeof j.finishedAt === "string" ? j.finishedAt : ""); if (!Number.isFinite(fin) || !(fin >= Number(turnTs || 0))) continue; // finishedAt 전용(startedAt 대체 금지 — 1회차 blocker③)
    let out = ""; try { out = fs.readFileSync(path.join(dir, fm[1] + ".out"), "utf8"); } catch { continue; }
    const v = authoritativeVerdict(out); if (v === "pass" || v === "pass-notes") return fm[1];
  }
  return null;
}
// 판독 전용: 남은 항목/이 턴에 소비된 항목. 호출자가 removeResidualItems로 소비 확정(기록 실패=fail-closed 유지).
function residualPending(ws, turnTs) {
  const cur = readResidual(ws); if (!cur) return { items: [], consumed: [] };
  const consumed = [], items = [];
  for (const it of cur.items) { const jid = residualJobFor(ws, it, turnTs); if (jid) consumed.push({ campaignId: it.campaignId, jobId: jid }); else items.push(it); }
  return { items, consumed };
}

// ── 판단 관문(HARNESS-REALIGNMENT §4 개선 2 · 장치화 2026-08-30) ──
// "네가 정하라"는 프롬프트는 흐려진다(사용자 실사고). 그래서 장치로 강제한다: 검증 판정이 '구현자 판단이 필요한 상태'(전량 강등 보류 등)가
// 되면 마커를 남기고, 종료 훅(Claude verify-guard·Codex codex-hook)이 그 판단(round-judge)이 장부에 기록되기 전에는 턴을 끝내지 못하게 한다.
// 판단 선택지: close-oos(범위 밖 종결) · re-verify(재검증 요청) · escalate(사용자 방향 질문 — 결정 장부의 실존 항목 id 필수 → 스크립트 출력 강제).
// 판단 자체(어느 것을 고를지)는 구현자 몫이며 하네스는 "기록됐는가"만 본다.
const JUDGE_CHOICES = ["close-oos", "re-verify", "escalate"];
function judgeFileFor(ws) { return path.join(VERIFY_FINDINGS_DIR, wsKeyFor(ws) + ".judge-required.json"); }
function readJudgeRequired(ws) {
  let o = null; try { o = JSON.parse(fs.readFileSync(judgeFileFor(ws), "utf8")); } catch { return null; }
  if (!o || o.schema !== "judge-required-v1" || !Array.isArray(o.items)) return null;
  const items = o.items.filter((x) => x && typeof x.askId === "string" && x.askId && typeof x.campaignId === "string");
...
function resolveJudgeRequired(ws, askId, choice, opts) {
  opts = opts || {};
  if (!JUDGE_CHOICES.includes(choice)) return { ok: false, reason: "unknown-choice" };
  const cur = readJudgeRequired(ws);
  const item = cur ? cur.items.find((x) => x.askId === askId) : null;
  ...
  return atomicWrite(judgeFileFor(ws), JSON.stringify({ schema: "judge-required-v1", ws: String(ws), items }, null, 1)) ? { ok: true, decisionId } : { ok: false, reason: "marker-remove-failed" };
}
