...
function buildVerifyDirective(mode) {
  ...
  return [
    ...
    `[전달 원칙] 검증 요청을 요약/생략하지 마라. 변경한 실제 파일 경로와 확인 지점(함수·라인·기대 동작)을 구체적으로 적어, Codex가 원본 파일을 직접 열어 검증하게 하라. "대충 됨" 식 요약 전달 금지.`,
    // (a) 재판단 강제 → 앵무새 금지
    `[재판단] Codex 답을 그대로 옮기지 마라. 그 지적이 타당한지 네가 다시 점검해 동의/반박을 근거와 함께 판단하고, 최종 결론(통과/실패 + 사유)을 네 책임으로 정리해 보고하라. Codex와 의견이 갈리면 그 이유를 명시하라.`,
    `검증을 건너뛴 완료 보고 금지.`,
  ].join("\n");
}
