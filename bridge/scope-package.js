...
const { parseGitLog, suggest } = require(path.join(__dirname, "scope-ledger-core.js"));
const { parseEventsJsonl, deriveLedger, selectForPackage } = require(path.join(__dirname, "ledger-events-core.js"));
const { readLedgerEventsText, appendLedgerEvent, loadLang, INTEGRITY_FILE, CONTRACTS_DIR } = require(path.join(__dirname, "contract-lib.js"));
...
function collectCommon(repo) {
  ...
  try {
    const MA = require(path.join(__dirname, "map-adapters.js"));
  ...
  const mapContentBlockedEn = mapContentBlockedKey
    ? (() => { try { return require(path.join(__dirname, "map-reader.js")).reasonTextFor(mapContentBlockedKey, null, true); } catch { return mapContentBlockedKey; } })()
    : null;
...
function collectPackageHistoryless(repo) {
  ...
  try { const { nonGitChangedSince, normWs } = require(path.join(__dirname, "contract-lib.js")); const seedAbs = new Set(seeds.map((f) => normWs(path.join(repo, f.rel)))); const inv = nonGitChangedSince(repo, Number.MAX_SAFE_INTEGER, seedAbs); baseline.nonGitFiles = { n: inv.files, complete: inv.complete }; } catch { /* 기준선 실패 — 삭제 감지만 비활성(무해) */ }
...
module.exports = { collectPackage, captureSeedBaseline, collectCommon, cliMain };

// CLI 본체 — 얇은 래퍼(scripts/scope-package.js)가 require.main 일 때 호출(이관 뒤에도 레포 명령 무회귀)
function cliMain() {
