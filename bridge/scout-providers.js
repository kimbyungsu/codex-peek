...
const { renderPackageMarkdown } = require(path.join(__dirname, "scope-package-core.js"));
...
const PROVIDERS = {
  ...
  deepseek: {
    ...
    probe: () => {
      const bridge = path.join(__dirname, "deepseek-bridge.js");
    ...
    invoke: ({ md, outFile, repo }) => {
      const bridge = path.join(__dirname, "deepseek-bridge.js"); // repo 정본 직접 실행(설치본 드리프트 회피 — 기존 계약)
