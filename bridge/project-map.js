...
exports.EVIDENCE_KINDS = ["ledger", "ask", "test", "code", "config", "doc", "git"]; // git: 하네스 사실 판독(커밋 실존 검사 · §B2 (2)) — 어휘 확장=추가만
...
exports.FACT_DETECTED_BY = ["git-name-status", "rotation", "rotation-revert"];
...
function isActiveFileNode(n) {
    return !!n && n.entityType === "file" && (!n.state || !n.state.lifecycle || n.state.lifecycle === "active");
}
function activeFileNodeCount(t) { return (t.nodes || []).filter((x) => isActiveFileNode(x)).length; }
...
function activeFileDelta(t, operation, payload, targetId) {
    if (operation === "add_node")
        return isActiveFileNode(payload ? payload.node : null) ? 1 : 0;
    if (operation === "split_node") {
        const src = (t.nodes || []).find((x) => x && x.id === targetId);
        const nn = payload && Array.isArray(payload.newNodes) ? payload.newNodes : [];
    ...
    if (operation === "set_state") {
        ...
        if (tr && tr.entityType === "file" && !isActiveFileNode(tr) && to && to.lifecycle === "active")
            return 1;
    }
    return 0;
}
...
function validateEvidence(e, who) {
    ...
    else if (e.kind === "git" && !/^[0-9a-f]{40}$|^[0-9a-f]{64}$/i.test(e.ref))
        errs.push(`${who}: git evidence ref 는 커밋 OID(40|64hex)여야`);
...
function validatePatchV2(p) {
    ...
    if (isPolicyOpV2(op)) {
        ...
        if (!Array.isArray(p.evidence) || !p.evidence.length)
            ...
            const factOk = exports.FACT_DETECTED_BY.includes(String(p.detectedBy || "")) && p.evidence.some((e) => e && typeof e === "object" && e.kind === "git");
            if (!factOk && !p.evidence.some((e) => e && typeof e === "object" && exports.CODE_EVIDENCE_KINDS.includes(e.kind)))
...
function semanticValidateV2(t, p, ctx) {
    ...
    switch (p.operation) {
        case "add_node": {
            ...
            if (n && n.entityType === "file" && activeFileNodeCount(t) + activeFileDelta(t, "add_node", pl) > exports.MAX_FILE_NODES)
                errs.push(`add_node: file 노드 전체 상한(${exports.MAX_FILE_NODES}) 도달(활성 file 노드 ${activeFileNodeCount(t)} — 내려간 칸 제외 · §B2) — 증분 세밀화 상한(해상도 설계 §2-3)`);
            break;
        }
        case "add_edge": {
            const e = pl.edge;
        ...
        case "tombstone_candidate": {
            ...
            if (p.operation === "set_state" && activeFileNodeCount(t) + activeFileDelta(t, "set_state", pl, p.targetId) > exports.MAX_FILE_NODES)
                errs.push(`set_state: file 노드 전체 상한(${exports.MAX_FILE_NODES}) 도달(활성 file 노드 ${activeFileNodeCount(t)} — 내려간 칸 제외 · §B2) — active 로 올릴 수 없음`);
            break;
        }
        case "add_anchor": {
            const tr = findEntity(t, p.targetId);
        ...
        case "split_node": {
            ...
            // split_node로 file 노드 60→62 적용이 실증됨). 사후 수 = 현재 − (원본이 file이면 1) + 신규 file 수.
            {
                ...
                const dF = activeFileDelta(t, "split_node", pl, p.targetId);
                if (dF > 0 && curF + dF > exports.MAX_FILE_NODES)
                    errs.push(`split_node: file 노드 전체 상한(${exports.MAX_FILE_NODES}) 초과 예정(${curF + dF}, 활성 기준 — 내려간 원본 분할도 활성 상한을 넘기지 못함 · §B2) — 증분 세밀화 상한(해상도 설계 §2-3)`);
            }
